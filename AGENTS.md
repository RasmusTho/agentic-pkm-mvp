...
- **Register the full worktree lifecycle.** After claim, register the dedicated checkout with `scripts/agent_worktree.py register`; heartbeat it during active work and review-fix waits, then record `release` or `complete` explicitly. Cleanup is report-only by default and may remove a worktree only when its matching lifecycle record is expired, the checkout is clean and unlocked, no active lease remains, and merge/closure eligibility is proven. Active, locked, dirty, mismatched, or unregistered worktrees are preservation evidence, never cleanup candidates. Cleanup must not hold the lifecycle lock across fetch or planning; it revalidates lifecycle authority under a short lock at each worktree-destructive boundary so a timely heartbeat can establish preservation authority.
