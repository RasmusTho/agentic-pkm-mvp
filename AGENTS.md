...
- Owner asks route through `.codex/skills/owner-decision-brief`, the thin Yggdrasil profile for the
  repo-local `decision-quality` method: discretionary escalations re-test this gate first (contractual
...
- **Late-change supersession is fail-closed and selective.** A late delivery change to the PR head, scope/authority body, governing contract, or required-check configuration supersedes the affected evidence, not unrelated evidence. Compare the digest/version of every mutable authority surface — PR body, governing contract, and required-check configuration — alongside the head SHA. Record the prior/current heads and digest/version snapshots, affected evidence, and required reruns in a `late_change_supersession_receipt.v1`; rerun every affected gate on the current head SHA before merge or closure. This receipt replaces relying on the advisory-only process-map rule as the sole enforcement; review its consolidation with existing exact-head evidence receipts by 2027-02-22. `pr-integration` owns detection during readiness, and `verification-and-closure` refuses stale evidence at the closing boundary.
...
- **Cross-stage simplicity loop.** Apply the canonical check in `docs/development/DELIVERY_FEEDBACK_LOOP.md :: Cross-stage simplicity check` at every handoff, including after feature breakdown. If implementation, review, CI, runtime, or user evidence shows that the slice is too large or uses the wrong abstraction, capture a BuilderOps `LearningSignal` and shrink or rescope it; do not add local complexity rules in each skill.
