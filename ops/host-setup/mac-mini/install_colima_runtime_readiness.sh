...
echo "Artifacts: helper, mount producer, mount boundary, substrate unit, containerd drop-in, docker drop-in"
...
for required_key in COLIMA_RUNTIME_PROVIDER COLIMA_EXPECTED_PERSISTENT_SOURCE COLIMA_EXPECTED_PERSISTENT_IDENTITY COLIMA_EXPECTED_PERSISTED_INVENTORY COLIMA_EXPECTED_PERSISTENT_FSTYPE COLIMA_PERSISTENT_DATA_PATH; do
...
install_guest_file "$ROOT/scripts/lib/colima_data_mount_provision.sh" \
  "$GUEST_LIBEXEC/yggdrasil-colima-data-mount" 0755
