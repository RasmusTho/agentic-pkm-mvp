...
builderops_valid_engine_id() {
  local engine_id="${1:-}"
  [[ "${engine_id}" =~ ^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$ ]] || return 1
...
builderops_product_project_listing_state() {
  local projects="${1:?Compose project listing required}"
  printf '%s' "${projects}" | python3 -c '
import json
import sys
...
if not isinstance(projects, list) or any(
    ...
    raise SystemExit(2)
print("present" if any(project["Name"].startswith("pkm-") for project in projects) else "absent")
'
}
...
builderops_assert_failure_domain() {
  ...
  local builder_id product_id builder_projects product_projects builder_project_state builder_product_project_state product_project_state
  ...
  builder_product_project_state="$(builderops_product_project_listing_state "${builder_projects}")" || {
    echo "BuilderOps Docker Compose project listing is invalid or unavailable" >&2
    export BUILDEROPS_FAILURE_DOMAIN_REASON="invalid_docker_project_listing"
    return 75
  }
  ...
  if [ "${builder_product_project_state}" = present ]; then
