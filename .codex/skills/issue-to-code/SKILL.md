...
15. Resolve `Suggested Validation` from the existing current-candidate results first; run only missing or invalidated checks. Apply `docs/development/GOVERNANCE_PROPORTIONALITY.md :: Evidence reuse and stop rule`. Confirm every AC's `Verify:` target is covered.
