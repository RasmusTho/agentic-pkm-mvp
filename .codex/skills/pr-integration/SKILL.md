...
- When waiting for CI to go green (or resolving the optional `--codex` verdict path, inactive as the default gate — the PR review gate runs locally per `verification-and-closure :: Running the local review gate`), follow `_shared/CI_WAIT_CONTRACT.md` (shared `app.dispatcher.poll_backoff` helper, REST check-runs only, interval + cap + exponential backoff, `Retry-After` and x-ratelimit-reset header honoring, calibrated backoff, `scripts/await_pr_checks.sh`). Never tight-poll `gh pr checks`/`gh pr view --json`, and never re-read verdict surfaces as separate per-attempt calls — they starve the shared API budget. [ci-wait-contract]
