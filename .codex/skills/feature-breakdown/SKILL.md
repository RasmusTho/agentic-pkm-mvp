...
- Register each new `docs/<CAPABILITY>/` specification directory in `docs/DOCS_INDEX.md` during the same publication pass — add rows for the `README.md` and every task/spec file, not just the directory itself. A spec directory that ships unregistered is invisible to the canonical index and to builder-agent source routing (the repeated miss recorded from PR #3154 / PR #3167 / PR #3193).
