...
5. Runs the repo smoke gate against prod config through the shared host lease if the environment
   supports it; records the result:
   `python3 scripts/run_with_host_lease.py --resource pytest-not-pg --execution-id verify-promotion:<sha> --wait-seconds 900 -- pytest -q -m "not pg and not alpha_llm"`.
