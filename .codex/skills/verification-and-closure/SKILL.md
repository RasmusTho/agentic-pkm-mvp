...
Only when the governing Issue or PR body carries an explicit
`Builder-Thread-Ref: <thread-id>`, use `builder-thread` for one exact read-only lookup. This
checkpoint never mutates or gates verification, merge, delivery, parent closure, or acceptance.
Missing, open, quarantined, conflicted, or unreadable thread context is reported separately and
cannot satisfy or invalidate a `Verify:` target.
