State: Conditional executor and in-flight verification procedure. Read only when SKILL.md routes here.

# Executor and In-Flight Verification
...
- Resolve `Suggested Validation` using current applicable results; run only missing or invalidated checks per `docs/development/GOVERNANCE_PROPORTIONALITY.md :: Evidence reuse and stop rule`
...
successor's dispatcher task/lease readback when dispatcher-backed, worktree registration, candidate
HEAD/file-set revalidation, branch/PR head, and review/receipt readback. A former lifecycle owner is read-only after that acknowledgment and cannot
...
path. Its full-path protocol is unchanged for dispatched execution, release operations, and
already-authenticated merge attempts. Review depth or Issue count alone does not select this file.
...
- After two formal independent `CHANGES_REQUESTED` review rounds, apply `docs/development/AUTONOMOUS_REVIEW_REPAIR_GATE_CONTRACTS.md :: PR-Level Scope Revalidation Gate` through the shared production gate before another expensive proof, publication, or repair cycle. Every publication declares `--publication-mode new|existing`; for an existing PR, invoke it with `--pr-scope-revalidation --github-repository <owner/repo> --pr-number <N>` and add `--governing-issue <N>` for issue-backed work. Issue-free docs/governance/direct-repair work binds its authenticated live PR-body contract and requires that contract's canonical classification to match the invocation lane. The gate must re-fetch content-bound live GitHub review/finding evidence and match the local receipt to a durable trusted-author PR comment rather than trust caller-supplied review history or classification coverage.
...
When all prerequisites are met, reject race-added closing refs and preserve a durable receipt,
including restoring the authenticated body:
...
- retain prior-head convergence comments as well. The shared convergence classifier authenticates
  ...
  merged-chain watchdog in every live-body mode; require a complete valid merged identity, and
  authenticate same-authority older body-edit phases before selection too. Preserve provenance-bound
  legacy terminal-LF compatibility through each authority's convergence/final observation and the
  existing no-history path. Prior-head proof never supplies current approval or resets repair accounting
