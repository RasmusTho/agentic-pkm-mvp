...
| `agent:ready` | bounded, testable, unblocked — external pickup qualifier; vault-backed work additionally requires an unclaimed `Ready` ticket |
...
- `agent:blocked` and `agent:needs-human` belong on non-active work; vault-backed tickets use
  `Blocked` or `Backlog` as appropriate.
