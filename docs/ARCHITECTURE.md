...
| Chat / Deep Agents | A read-only Chat cognition scaffold exists for planning/decomposition through the shared `ReasoningFacade`. Separately, a bounded canvas-session scaffold now exists behind `CANVAS_ENABLED`: session logs, body-scoped note edits, governance-intent routing, and a process-local API/CLI surface for dev/test validation. | Full Chat surface support is not yet accepted into the baseline. Canvas-session scaffolding remains gated, non-prod by default, and any future Chat or Deep Agent mutation remains governed execution work tied to owner-doc promotion on #597. |
