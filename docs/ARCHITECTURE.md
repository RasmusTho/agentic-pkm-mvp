...
- **Current (being replaced)**: for each object/uuid in the vault there is a matching `uuid.md` in the metadata mirror (`System/Metadata/VaultMirror/<vault-relative path>/`). The same file is both metadata mirror and per-note log: it collects agent runs, promotion history, provenance, and any future satellite sync evidence so the machine history follows the object regardless of backend. The Note Log is portable Markdown that can move via Git between instances even when SetDB/AMG or other Stores differ.
- **Planned replacement — Companion Note**: a flat, UUID-keyed file at `vault/_system/companions/<uuid>.md` replaces the path-mirrored VaultMirror layout. The companion holds a bounded field set (`uuid`, `source_ref`, `title` repair cache, `content_hash`, `ingest_state`, `last_ingested`, `created_by_instance`, `attachments` manifest) and explicitly excludes human-owned fields (`review_state`, `maturity`). Healing follows a 7-step authority matrix (frontmatter UUID → companion → DB → path match → content hash → semantic → new). Full implementation plan: `docs/plans/COMPANION_NOTE_AND_NOTE_CONTEXT.md`.
- **Planned addition — Note Context**: a runtime-assembled `NoteContext` struct (not a file) that aggregates companion + vault note frontmatter/body + runtime DB (relations, classification, history) for agent consumption, with per-agent `ContextBudget` controls. Eliminates the current 800-char snippet passed to Panel Agent (`app/agents/panel_agent/graph.py:240`).
