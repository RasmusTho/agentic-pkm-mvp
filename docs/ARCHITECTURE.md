...
3) **ASK API** — FastAPI endpoint returning answer text plus sources `{uuid, title, origin (vault/external), zone overlay if known, path/source_ref}` and latency; uses hybrid retrieval over both planes with an in-process HybridStore that is a cache-through of the durable `store_vector_index`, rebuilt via `rebuild_from_durable_index()` on first use per process (KERNEL-05, #2767 — audit invariant I-D3; full capability-promotion writeback tracked as a follow-up issue). `zone` is passed through when present on the hit payload, but derived zone coverage is not guaranteed baseline behavior.
...
- Runtime-gating settings deltas from `settings/local.md` now route through the same governed seam and emit `SettingsWriteReceipt`s for watcher-detected file edits to `enableVaultWatcher` / `enableAutoIndexing` (#2512).
- The first-vault authenticated bootstrap/initialize prerequisite (#5366) is delivered as an additive backend seam: it creates the first registry/default binding and returns a server-owned scoped selection, but does not activate the shipped client or claim full request-hot-path delivery; #3860 retains that integration authority.
8) **Active-context selection (WSP seam, MVR-03 #3857)** — `POST/PUT/GET/DELETE /api/companion/active-context/selection` drive a TTL-bound, process-local `ContextSelectionStore` keyed by an expiring server-minted bearer used *in addition to* the #2223 gate. Each resolution yields one immutable, generation-stamped `ActiveContextSetV1` (`app/vault/active_context_v1.py`) with server-derived workspace/scope/sphere/situated-identity, a principal resolved from the private delegated operator-role record (`app/instance/local_operator_principal.py`), and a per-binding GOV verdict (`app/governance/binding_authority.py`). Selection carries no authority: action, write class, and permission stay separate GOV inputs per call. **Not yet wired into the request hot path** — production carrier propagation (`X-Active-Context-Session` / `X-Active-Context-Override`) and binding-keyed retrieval/persistence are MVR-05A/05B. The `minimum_runtime_principal` floor and its auth-producer fence are implemented and fail-closed, but their deployment-wrapper activation is not yet automated; an operator runs the cutover commands explicitly inside the MVR-01B stopped window. The v0 `ActiveContextResolver` remains the transitional projection for scalar-`VaultContext` callers, and `/api/companion/vault/select` remains the named legacy-global adapter.
8) **Active-context selection and scoped reads (WSP seam, MVR-03/05B #3857/#3860)** — `POST/PUT/GET/DELETE /api/companion/active-context/selection` drive a TTL-bound, process-local `ContextSelectionStore` keyed by an expiring server-minted bearer used *in addition to* the #2223 gate. Scoped search, ASK, and vault-note routes resolve exactly one immutable, generation-stamped `ActiveContextSetV1` from `X-Active-Context-Session` or the higher-precedence `X-Active-Context-Override`; an explicit missing or stale carrier fails closed and cannot downgrade to a legacy read. Each scoped filesystem read revalidates registry binding revision/root and holds the existing shared binding-effect lease through I/O. Retrieval/cache provenance carries context generation and the non-reversible selection digest. Selection carries no operation authority: action, write class, and permission stay separate GOV inputs per call. Legacy scalar routes and the v0 `ActiveContextResolver` remain named compatibility adapters; MVR-05C still owns scoped governed writes.
9) **Governed capture and live-meeting client seam (CDLM)** — the Hub admits audio/image/video/document originals only on durable raw-write + event-commit receipts; persists meeting session/segment gaps; exposes revisable transcript/default-analysis projections; guards editor-owned `user_note` bytes from every derived writer; and finalizes transcript, analysis, and verbatim notes as three separate create-once artifacts. Bifrost retains originals and unacknowledged notes locally, renders the five durable transfer states, and keeps “your notes” separate from “AI keeps updating.” Hub remains the only entity-merge executor/journal authority. The shipped `main` capability is test-channel-proven by run `cdlm10-868e042e59`; this architecture claim does not imply prod activation or physical-device HCAP-09 acceptance.
The MVR-05B compatibility mutation bridge is separately identified: legacy vault mutations carrying either scoped bearer fail `capability_not_ready`, while callable `/capture/compatibility` and `/note/save/compatibility` routes require an authenticated opaque binding/revision precondition and never downgrade to the legacy route. The bridge is callable but not activated in shipped Companion HTML; aggregate MVR-05B integration and MVR-05C governed writes remain owned by #3860.
The bridge's current effect boundary also holds the shared `compatibility_ingress_window` from
final principal/binding/revision validation through the filesystem and authority-receipt effects;
the corresponding rebind transition takes the exclusive side of that window. Independently
mounted provisional-memory, Companion, Canvas, and Panel mutation routes fail closed on retained
`X-Active-Context-Session` or `X-Active-Context-Override` carriers before their vault effects.
This is the bounded pre-DecisionToken compatibility bridge, not the full MVR-05C explicit-target
governed-write floor.
...
| Cross-device capture + live meeting | Shipped on `main`: receipt-gated Bifrost custody and five-state queue; governed Hub media/meeting routes; revisable transcript/default analysis; fail-closed user-note ownership; separated final artifacts; test-channel composed receipt on #4383. | Prod-channel activation is release work, not implied. Physical-device locked-screen/call/Watch-haptic truth remains the operator receipt on bifrost#21. Rich templates, participant inference, public ingress, and client-side merge execution remain out of scope. |
...
| Reasoning/LangGraph | `ReasoningFacade` exists; ASK and PanelAgent use LangGraph in active runtime paths; reviewer and note-hygiene reasoning now route through the shared facade seam while preserving existing write-authority boundaries. Provider/model selection stays behind the shared registry/resolver seam: TCD chooses the lowest declared intelligence that adequately handles the task and spends deeper capability only when expected avoided cost justifies it, for both internal Yggdrasil reasoning and external harness invocations. | Broader Promotion/Hygiene graph adoption remains phased and gated; facade adoption does not imply full LangGraph baseline rollout or production Deep Agent expansion. |
| Model Access | A provider-neutral `ModelAccessRouter` facade and strict route/receipt provenance contract now sit above the existing LLM resolver. Product and Builder retain separate policy, registry, and credential authorities; routes preserve resolver-selected fallback lineage and reject capability claims outside the selected adapter descriptor's declared support, without exposing credentials. | Product/Builder runtime migration, first-class Codex CLI and Ollama adapters, dynamic provider catalogs, and preflight-only fallback execution are not delivered by this seam and remain separately gated. Model Inquiry retains its single-target, no-fallback policy. See `docs/LLM_ROUTING.md` and ADR-0066. |
...
| MCP/tools | Descriptor registry, registry-backed internal ToolProvider boundary, validation, deterministic/mock execution, internal tools, gated real `mcp.vault.append_note`, and an optional consumer-side remote multiplex seam with deterministic local fallback exist. The distinct producer-side `mimer-mcp` sidecar is shipped under ADR-0061 A2/B1/C1: it delegates only ask/governed-capture/retrieve-search/note-read/health to the governed HTTP API, uses stdio, and opens no listener or new auth surface. Its demonstrated support evidence is the hermetic JSON-RPC stdio client journey recorded on parent #3366; that evidence is not a claim of general third-party-client or production-runtime support. | Streamable HTTP, per-device auth, generic vault writes, receipt read-back, and support for any client not individually evidenced remain deferred, separately gated follow-ons. |
...
| Decision-history admission | A read-only Pre-mortem admission adapter (`app/calibration/premortem_admission.py`) accepts exactly one explicit canonical `decision_record`, reuses `resolve_vault_uuid` plus CAL-01 `iter_outcome_receipts`, excludes governance/cross-scope/unresolvable material before rendering, and emits only citation-resolved evidence with content-free coverage counts. | Reflective/model packets, recommendations, outcome inference, scheduling, profiles, writeback, and a user-facing Pre-mortem surface remain unshipped. |
| Temporal posture | A pure read-time adapter (`app/temporal/posture.py`) applies the versioned `config/temporal_posture.v1.json` allowlist to explicit policy-permitted timestamps and can emit only `unknown`, `historical`, or `review_due`, plus a content-free fixed-clock report. It renders source/index drift separately and never mutates artifacts or retrieval results. | Revalidation fetches, validity claims, per-artifact posture persistence, expiry, notification, ranking/filtering, and use beyond the explicit allowlist remain unshipped. |
| Context dimensions | Canonical payload contract delivered in `app/context_dimensions.py` (`ContextDimensions`: `scope: str` defaulting to `"default"`, `sphere_memberships: list[str]` defaulting to empty, `situated_identity: str \| None`), replacing the legacy single-`domain` field (`from_legacy_domain` maps old `domain` values into `scope` for back-compat). Threaded through runtime paths: the vault watcher extracts per-note dimensions from frontmatter (`app/watcher/vault_watcher.py::extract_context_dimensions_for_note`, `scope`/`domain` frontmatter keys with global-scope fallback); the orientation seam passes the dimensions through unchanged (`app/orientation/runtime.py::pass_through_context_dimensions`); the observability status service surfaces the most recent dimensions from the index outbox as `ContextDimensionsStatus` on the status payload (`app/observability/status_service.py`, `app/observability/status_model.py`). Orchestrator plan-context resolution (`app/orchestrator/runtime.py`, `app/orchestrator/v2_runtime.py`) and the event envelope/outbox schema (`app/events/models.py`, `app/events/schema.py`, `app/services/outbox.py`) also carry `context_dimensions` as a `scope`/`sphere_memberships`/`situated_identity` payload. Delivered per `docs/SCOPE_SPHERE_SITUATED_IDENTITY/` (#645, implementation #730/#731/#733, cross-surface tests #759). | Panel/Chat consumption of the payload, relation-aware use of `sphere_memberships` in retrieval/ranking, and richer `situated_identity` semantics beyond pass-through storage remain future work outside this capability seam. |
...
- **Model Inquiry bridge** — the repository provides a fail-closed remote launcher contract and
  repository-declared provider-API adapters for independent Fable and GPT/Codex turns. Neutral
  `xhigh` intent resolves through the Builder provider census and host-secret contract; a missing or
  malformed Keychain credential reaches the runner as a value-free logical identifier so it can
  persist a typed terminal receipt before any adapter call. Subscription CLIs remain interactive
  tools and are not reachable from headless entrypoints. Host credential values and deployment
  remain operator configuration outside Git. See `docs/BUILDEROPS_MODEL_INQUIRY/`; the current
  remote-host deployment is tracked separately in #3586.
