...
| Orchestrator | V1 is default. V2 is flag-selected via `ORCHESTRATOR_VERSION=v2` and includes dependency-aware parallel scheduling, event/trace compatibility, compensation/rollback, and retry metadata handling. Per-tool timeout via `tool_timeout_seconds` setting is supported on both V1 and V2 (see `docs/contracts/TIMEOUT_AND_SLA_CONTRACT.md`). | Broad V2 adoption is not baseline. Checkpoint/resume support, plan-level timeout budgets, and repo-wide A2A/runtime delivery SLA remain non-baseline; `CheckpointStore` plumbing exists but is not the supported execution path. |
