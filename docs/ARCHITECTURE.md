...
| Orchestrator | V1 is default. V2 is flag-selected via `ORCHESTRATOR_VERSION=v2` and includes dependency-aware parallel scheduling, event/trace compatibility, compensation/rollback, retry metadata handling, retry/backoff observability, and checkpoint/resume with configurable interval persistence. Per-tool timeout via `tool_timeout_seconds` setting is supported on both V1 and V2 (see `docs/contracts/TIMEOUT_AND_SLA_CONTRACT.md`); #456 tracks a V2 timeout discriminator mismatch for documented executor `tool_timeout` errors. | Broad V2 adoption is not baseline. Plan-level timeout budgets and repo-wide A2A/runtime delivery SLA remain non-baseline. |
