...
| Model Access | A provider-neutral `ModelAccessRouter` facade and strict route/receipt provenance contract now sit above the existing LLM resolver. Product and Builder retain separate policy, registry, and credential authorities; resolved routes bind the exact target and declared capabilities without exposing credentials. | Product/Builder runtime migration, first-class Codex CLI and Ollama adapters, dynamic provider catalogs, and preflight-only fallback are not delivered by this seam and remain separately gated. Model Inquiry retains its single-target, no-fallback policy. See `docs/LLM_ROUTING.md` and ADR-0066. |
