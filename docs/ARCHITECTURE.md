...
| MCP/tools | Descriptor registry, registry-backed internal ToolProvider boundary, validation, deterministic/mock execution, internal tools, gated real `mcp.vault.append_note`, and an optional consumer-side remote multiplex seam with deterministic local fallback exist. **No external Mimer MCP server is shipped.** | ADR-0061 accepts a distinct producer-side client adapter under A2/B1/C1: a constituent-owned sidecar over the governed HTTP API, stdio only, no network listener, no new auth, and exactly ask/governed-capture/retrieve-search/note-read/health. Implementation and composed client acceptance remain downstream. Streamable HTTP plus per-device auth are deferred, separately gated follow-ons. |
