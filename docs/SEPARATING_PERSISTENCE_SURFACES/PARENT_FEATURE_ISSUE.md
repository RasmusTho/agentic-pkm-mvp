...
There is active implementation work replacing VaultMirror with a companion-note surface on the `claude/inspiring-jackson` worktree. That work is the **reference per-note implementation of the system surface sub-lane this feature names**. This feature depends on the companion-note migration landing (or at least stabilizing its contract), and must not prescribe the companion-note implementation shape, field set, or migration behavior. Where the two overlap, this feature cites `COMPANION_NOTE_CONTRACT.md` and defers.
