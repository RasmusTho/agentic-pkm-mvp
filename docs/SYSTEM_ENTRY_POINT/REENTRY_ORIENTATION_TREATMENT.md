...
- **Cold (>7d)**: no overlay; the existing calm empty/cold copy with Browse-the-vault and System-map affordances. The leave-point cursor TTL is hard-capped at 7d (ADR-0008); re-entry beyond this window is always cold. The runtime resolver constant `_GAP_COLD_THRESHOLD` in `companion-ui/companion-app/companion_ui/workspace/entry_state.py:75` was reconciled to 7d by #2513 (closed); 8-14d vault-bound gaps now resolve to `cold_start`, not `long_mist`/`orienting`.
