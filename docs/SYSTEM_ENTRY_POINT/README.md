...
- [x] Cold start (vault-bound cold trajectory) and `no_vault` (runtime unreachable or no vault bound — including the first-contact picker) render no re-entry overlay. The decided cold threshold is 7d per ADR-0008/#2489; the runtime resolver was reconciled to 7d by #2513 (closed), so 8-14d vault-bound gaps now resolve to `cold_start` rather than `long_mist`/`orienting`.
