...
| Body edit to a canonical note — **allowlisted mechanical-hygiene class only** (broken wikilink w/ single unambiguous target, malformed-markdown deterministic repair, frontmatter format-only fix, misspelling, transcription artifact) | `governed_execution` (corrects transcription of existing meaning, adds none) | `act` — narrow carve-out under [ADR-0048](adr/ADR-0048-allowlisted-mechanical-hygiene-act-tier.md), all seven binding conditions mandatory (see below) |
| Body edit to a canonical note — **everything else** (all semantic prose) | `governed_execution` | `ask-you` (deliberate exception — see below) |
...
1. **Body edits stay human (`ask-you`) — except an allowlisted mechanical-hygiene carve-out.** Git makes body edits reversible, so the reversibility principle would otherwise place them at `agent-review`. Semantic prose is deliberately held at the human gate (the v1 human-save / canvas-suggestion model) because direct prose authorship is the human's primary creative surface. This is the principled exception to "human only for irreversible/external," and it was revisited under the evidence gate: owner ruling R1 (2026-07-05), ratified as [ADR-0048](adr/ADR-0048-allowlisted-mechanical-hygiene-act-tier.md) (Accepted 2026-07-06), carves **one narrow, closed class** out to `act` — allowlisted **mechanical hygiene** that corrects the transcription of existing meaning and adds none (broken wikilink with a single unambiguous target, deterministic malformed-markdown repair, frontmatter format-only fix, misspelling, transcription artifact). The carve-out is void where any of ADR-0048's **seven binding conditions** fails: (1) closed-class allowlist decided by class, never by confidence — `text.grammar` excluded initially; (2) never semantic — restores intended form only, demotes to propose on ambiguity; (3) deterministic application re-derived from file content, never LLM output as the direct source; (4) the Swedish safeguard (sv_SE lexicon veto, diacritic invariance, never cross-language, `mixed`/`unknown` demote to propose); (5) one fix = one atomic Git-visible diff = one receipt carrying the `finding_id`, with `curation revert`; (6) unchanged gates — WriteGuard, policy evaluation, event envelope, and receipts apply exactly as before; (7) the #1881 evidence gate governs any allowlist growth. **Every other body edit stays `ask-you`** — this is a bounded exception, not a general loosening. See ADR-0048 for the full condition prose. Enactment of the auto-apply flip itself is GRADUATED_CURATION slice G2-3; until it lands the engine remains propose-only.
