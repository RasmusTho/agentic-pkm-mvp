...
- [ ] AC2 (enforcement): retrieval of an artifact bound to a closed episode carries the closure-derived salience drop **on the production retrieval path** (`retrieve()` call site), and an open-episode artifact carries none. Verify: `tests/episodes/test_closure_decay.py::test_closed_episode_binding_derives_salience_drop_at_retrieval`
