...
2. **Closure event + projection sync**: emits outbox topic `episode.closed` (payload: `episode_id`, `closed_at`, `scope`, bound-artifact count; `context_dimensions` per SSI-01; idempotency-keyed per house producer rule). The note flip is SoR; the event is plumbing — consumers that miss it self-heal from the projection. The `episodes` DB projection's own `closed` column (what AC2's production `retrieve()` call site actually reads, via `read_closed_episode_ids`) is kept current by a targeted, idempotent `UPDATE` this same closure step issues (`app.episodes.closure._sync_projection_closed`) — never by the full ERE-02 `rebuild_episodes_projection()` replay, which no production caller schedules. All three writes (note, outbox, projection) are individually idempotent and ordered note → outbox → projection, so a crash between any two leaves the episode selectable by the next tick, which retries the remainder to completion instead of losing it.
