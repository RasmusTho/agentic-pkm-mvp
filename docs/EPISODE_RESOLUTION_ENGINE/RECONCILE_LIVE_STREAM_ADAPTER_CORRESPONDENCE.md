...
1. **Resolve the true state of the four streams (owner-facing decision, pre-filled recommendation).** `chat.sessions` is carved out from the default downgrade path because the voice-loop and conversational-journaling contracts already depend on it remaining live episode signal. Keep it `live` and land a follow-up adapter before ERE-12 closes. A product decision may downgrade it only if the same change reconciles every contract surface that asserts this live dependency — currently `docs/MIMER_VOICE_LOOP/README.md`, `docs/MIMER_VOICE_LOOP/CARRY_SESSION_FOLLOWUP_CONTEXT.md`, `docs/MIMER_VOICE_LOOP/PARENT_FEATURE_ISSUE.md`, `docs/CONVERSATIONAL_JOURNALING/README.md`, `docs/CONVERSATIONAL_JOURNALING/LEAD_REFLECTION_CONVERSATION.md`, and `docs/CONVERSATIONAL_JOURNALING/PARENT_FEATURE_ISSUE.md` — plus the ERE README inventory, `stream_registry.md`, and the ERE-01 stream contract. A repo-wide `chat.sessions` search must confirm that no stale live-stream claim remains. For each of `decision.receipts`, `kap.acquisitions`, and `heimdal.attention`, decide exactly one:
...
- [ ] AC5 (`chat.sessions` downstream contract): every voice-loop and conversational-journaling contract surface that asserts live `chat.sessions` episode signal remains consistent with stream liveness. The known set is `docs/MIMER_VOICE_LOOP/README.md`, `docs/MIMER_VOICE_LOOP/CARRY_SESSION_FOLLOWUP_CONTEXT.md`, `docs/MIMER_VOICE_LOOP/PARENT_FEATURE_ISSUE.md`, `docs/CONVERSATIONAL_JOURNALING/README.md`, `docs/CONVERSATIONAL_JOURNALING/LEAD_REFLECTION_CONVERSATION.md`, and `docs/CONVERSATIONAL_JOURNALING/PARENT_FEATURE_ISSUE.md`; a repo-wide search catches later additions. If the stream is downgraded, every matching surface is reconciled in the same change; otherwise the ERE-12 PR records that their existing live-stream claims remain valid. Verify: PR body owner-doc / source-doc writeback receipt + `rg -n "chat\.sessions" docs/MIMER_VOICE_LOOP docs/CONVERSATIONAL_JOURNALING`
...
- `docs/MIMER_VOICE_LOOP/CARRY_SESSION_FOLLOWUP_CONTEXT.md` and the other voice-loop / conversational-journaling contract surfaces named in AC5 (the downstream live-stream claims any downgrade must reconcile)
