...
7. **Human body preservation**: the markdown body is part of the re-cut surface even though Episode semantics remain frontmatter-first. Its exact raw suffix, including formatting-only newline edits, participates in the tracked baseline, becomes `segmentation: re-cut`, and every machine relabel carries a human-edited body through unchanged instead of regenerating the template over it. An untouched generated body is tracked separately and refreshes its derived labels/headings after a frontmatter-only cut edit. The scan-time note version is carried into the guarded write seam, so an editor save racing the relabel fails with a write conflict rather than being overwritten by a stale preservation decision.
