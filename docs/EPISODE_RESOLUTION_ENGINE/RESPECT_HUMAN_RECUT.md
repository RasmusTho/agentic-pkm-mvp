...
7. **Human body preservation**: the markdown body is part of the re-cut surface even though Episode semantics remain frontmatter-first. Its exact raw suffix, including CRLF and formatting-only newline edits, participates in the tracked baseline, becomes `segmentation: re-cut`, and every machine relabel carries a human-edited body through unchanged instead of regenerating the template over it. The suffix begins only after a standalone closing frontmatter delimiter line; `---` inside valid YAML scalar content is not a delimiter. An untouched generated body is tracked separately, re-baselined after canonical lifecycle rewrites, and refreshes its derived labels/headings after a later frontmatter-only cut edit. The raw-byte scan-time note version is carried into the guarded write seam, so a same-inode editor save or deletion racing the relabel fails with a write conflict rather than being overwritten or resurrected by a stale preservation decision.
8. **Invalid is not deleted**: a canonical Episode-note path that still exists but is temporarily unreadable, syntactically incomplete (including unterminated YAML frontmatter), or schema-invalid is skipped for that tick without aborting the scan, withdrawing bindings, or forgetting its baseline. A valid note whose frontmatter `episode_id` disagrees with its canonical filename is likewise retained under the path identity but never adopted under the mismatched id. Only the exact `episode_note_rel_path(episode_id)` is eligible for adoption, so nested copies cannot replace the canonical scan entry. Merge-deletion withdrawal runs only when the tracked canonical path is actually absent; once a mid-edit note becomes valid again, its preserved baseline still detects and reconciles the human re-cut.
