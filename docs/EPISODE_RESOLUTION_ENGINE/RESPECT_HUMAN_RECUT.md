...
7. **Human body preservation**: the markdown body is part of the re-cut surface even though Episode semantics remain frontmatter-first. Its exact raw suffix, including formatting-only newline edits, participates in the tracked baseline, becomes `segmentation: re-cut`, and every machine relabel carries a human-edited body through unchanged instead of regenerating the template over it. An untouched generated body is tracked separately, re-baselined after canonical lifecycle rewrites, and refreshes its derived labels/headings after a later frontmatter-only cut edit. The scan-time note version is carried into the guarded write seam, so an editor save or deletion racing the relabel fails with a write conflict rather than being overwritten or resurrected by a stale preservation decision.
