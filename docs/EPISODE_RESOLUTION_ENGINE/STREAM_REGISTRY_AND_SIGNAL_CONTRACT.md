...
4. Seeds the registry with the **complete inventory** (canonical enumeration lives in the capability [README](README.md), which this registry must match 1:1):
   - **live:** `heimdal.observations` (observation-log cursor; feeds time/protagonist/goal; consent-gated), `vault.activity` (`ingest.vault.changed`/`ingest.object.created`/`ingest.object.deleted` outbox topics + `extract_context_dimensions_for_note`; feeds time/goal/causation), `chat.sessions` (session log `.chats/`, `session_id`, per-turn timestamps; feeds time/goal), `decision.receipts` (WriteGuard-gated receipt log + `decisions` projection; feeds time/goal/causation), `kap.acquisitions` (`knowledge_acquisition.stage.completed`; feeds time/goal — content-origin, low situational weight), `heimdal.attention` (attention log; feeds goal/protagonist)
   - **planned:** `calendar` (ERE-09), `bifrost.native_capture` (space/protagonist/causation enrichment per `CAPTURE_TRANSPORT_FEASIBILITY.md` "Episode richness")
