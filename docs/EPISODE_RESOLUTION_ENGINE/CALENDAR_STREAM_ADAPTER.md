...
1. **Transport (decided here, within existing posture)**: read-only **CalDAV against iCloud** (app-specific password) with an ICS-file fallback for degraded/offline operation. Credentials and calendar selection live in the **private-bindings constituent** (operator-bound configuration — endpoints/credentials are exactly its charter), never in repo or vault. This conforms to the established egress posture (system is not local-only; Gemini-embedding precedent) — read-only external fetch, no data leaves the system. The `calendar-query` REPORT is bounded to a fixed window around "now" — `_CALDAV_TIME_RANGE_PAST` (7 days back) / `_CALDAV_TIME_RANGE_FUTURE` (60 days forward) in `app/episodes/calendar_stream.py` — both to cap the fetch and because it doubles as the required `<C:expand>` window: the REPORT requests server-side recurrence expansion so a recurring master VEVENT comes back as one distinct occurrence per instance (its own RECURRENCE-ID/DTSTART) instead of a single unexpanded master that would otherwise emit exactly once, ever. Every tick still re-reads that current item set, but the segmenter persists exact closed calendar signal identities from the segment's durable provenance; this also works when the later closing tick sees an empty or degraded CalDAV read. An event whose original segment closed therefore cannot contaminate a later segment, while an edit remains eligible through its changed `calendar_signal_id`.
2. **Adapter**: a poller (watcher-tick cadence class: sparse) normalizing calendar entries into the ERE-01 signal contract — `observed_at_start/end` from event times (TZID-qualified local times are resolved via `zoneinfo` to the correct UTC instant; a genuinely floating time with no `Z`/no resolvable TZID degrades the `time` dimension's confidence rather than being silently asserted as UTC), `dimensions_fed: {time: high, protagonist: medium, space: medium, goal: medium}`, attendees resolved *provisionally* against the shared entity register (three-state resolution, HEIM-6-honest: calendar attendee strings are never silently upgraded to canonical identities). Provenance ref / signal_id (`app.episodes.calendar_stream.calendar_signal_id`, review rounds 2+3, findings 1+2): a non-recurring event (or any expanded block that carries no RECURRENCE-ID) is `uid:etag` (unchanged since v1 — one CalDAV resource, one occurrence, so its own etag correctly is its change signal); one occurrence of a server-expanded recurring series is `uid:occurrence_key:content_token`, where IDENTITY (`occurrence_key`) is derived **solely** from this occurrence's own RECURRENCE-ID — canonical and **fully window-independent**, never gated on how many sibling occurrences happen to also fall inside this tick's expand/time-range window, and never on block count (finding 1; round 3 removed the last block-count-gated DTSTART fallback — an instance with no RECURRENCE-ID is treated as a single `uid:etag` event, a conservative under-count that stays stable and idempotent instead of flipping identity with the window). CHANGE detection (`content_token`) is a hash of this occurrence's own DTSTART/DTEND/SUMMARY/SEQUENCE/LAST-MODIFIED, encoded as **structured JSON** (not a delimiter-joined string, so a free-text SUMMARY can never shift a field boundary and forge an unchanged token — finding 2, round 3), and never the shared resource etag, since editing one occurrence must not re-emit its unmodified siblings as "new" evidence (finding 2).
...
- [ ] AC3 (enforcement): the segmenter consumes calendar signals through the registry-driven entrypoint with the pure segmentation core unchanged — the adapter registers via the ERE-01 registry and the existing entrypoint picks it up (asserted by running the ERE-04 fixture with calendar added and observing improved boundary confidence). The entrypoint may retain durable transport/idempotency boundaries such as the fixed-window calendar-consumption ledger. Verify: `tests/episodes/test_calendar_adapter.py::test_calendar_joins_fusion_via_registry_only`
