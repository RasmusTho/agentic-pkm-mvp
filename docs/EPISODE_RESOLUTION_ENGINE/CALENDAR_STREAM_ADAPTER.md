...
1. **Transport (decided here, within existing posture)**: read-only **CalDAV against iCloud** (app-specific password) with an ICS-file fallback for degraded/offline operation. Credentials and calendar selection live in the **private-bindings constituent** (operator-bound configuration — endpoints/credentials are exactly its charter), never in repo or vault. This conforms to the established egress posture (system is not local-only; Gemini-embedding precedent) — read-only external fetch, no data leaves the system.
2. **Adapter**: a poller (watcher-tick cadence class: sparse) normalizing calendar entries into the ERE-01 signal contract — `observed_at_start/end` from event times, `dimensions_fed: {time: high, protagonist: medium, space: medium, goal: medium}`, attendees resolved *provisionally* against the shared entity register (three-state resolution, HEIM-6-honest: calendar attendee strings are never silently upgraded to canonical identities), provenance ref = calendar item UID + etag.
