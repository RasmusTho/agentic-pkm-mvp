...
Without in-session context, every spoken follow-up must be a fully-restated question — punishing for a voice-first, dyslexic user, and unnatural for speech. Persisting through the chat-session path (rather than a bespoke store) is what makes voice conversations flow into the Episode Resolution Engine automatically; a bespoke transcript store would strand them outside the episode substrate and duplicate a live stream. The per-turn timestamp is the small correctness fix that lets that segmentation actually work.
...
- [ ] AC3: the transcript persists via `SessionLogWriter` to `vault/.chats/…` with `type: chat-session` + `session_id`, matching the ERE `chat.sessions` stream shape (so it is consumed as episode signal without new registration). Verify: `tests/voice/test_session_transcript.py::test_voice_transcript_written_as_chat_session`
...
- **What survives a restart:** the **persisted transcript** in `vault/.chats/` (it is a file; ERE consumes it whenever it next ticks).
...
- `docs/EPISODE_RESOLUTION_ENGINE/README.md` (input inventory — `chat.sessions` live) + `STREAM_REGISTRY_AND_SIGNAL_CONTRACT.md` (the stream contract voice turns feed)
