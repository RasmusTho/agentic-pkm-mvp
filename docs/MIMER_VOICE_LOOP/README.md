State: Specification directory — FILED (parent #3335; children #3336–#3339 filed 2026-07-07, all agent:blocked at filing per the uniform closed-loops filing policy). One of the five Yggdrasil closed-loop capabilities grounded in `docs/research/yggdrasil-closed-loops-ideation.md` §5. Subordinate to `docs/contracts/MIMER_CLIENT_CONTRACT.md` (client-agnostic transports), ADR-0056 (client contract + transport set), ADR-0043 (Heimdal = sensor constituent), ADR-0055 (multi-writer posture — only touched by the append-only transcript log), and `docs/architecture/SBS_OPERATING_MODEL.md` (classification + boundary-work rules).
