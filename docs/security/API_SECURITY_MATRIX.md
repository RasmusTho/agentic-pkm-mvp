...
| `app/api/routes/capture.py` | `POST /api/companion/capture` | Governed capture append to the vault inbox note; no due-date/task-state fields admitted. | Trusted-device server default; loopback opt-out; public unsupported. | governance-bearing append | WriteGuard check, schema `extra=forbid`, explicit empty-text rejection; no route auth/rate-limit. | deterministic-writer receipt in response plus `capture.inbox.appended` outbox event | vault inbox note, outbox event log | Level 2 |
