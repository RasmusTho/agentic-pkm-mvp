...
Generalize the existing SHA-256 optimistic guard into the shared rewritten-note filesystem write path. When current content differs from the caller's expected version, preserve the caller's proposed content as a sibling conflict artifact using the iCloud-compatible convention and return a legible staged-conflict outcome with VMW-01 provenance.

## Concretely

The production `FsVaultAdapter.write_note` path writes through temp-file-plus-`os.replace`. A stale rewritten write must never replace the current file and must never lose the proposed bytes. Append-only operations bypass this path.
...
- [ ] A stale rewritten write stages the losing content beside the current note and reports both the artifact and writer provenance. Verify: `tests/invariants/test_vault_multiwriter.py::test_stale_rewritten_write_stages_conflict_artifact_at_filesystem_seam`
