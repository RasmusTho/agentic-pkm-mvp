...
- privacy-safe query/question and unsupported-request observation;
...
4. Query execution is read-only and side-effect free: no directory creation, schema initialization, migration, receipt emission, or mutation.
...
- **I-MA11 — Read-side-effect freedom.** The query path uses SQLite read-only mode and cannot initialize, migrate, receipt, or mutate any surface.
