...
| Session / chat history | HKA, SIP | The canvas chat-session artifact (`type: chat-session`, `.chats/` vault-resident) is a durable HKA-owned artifact class, related 1:N to its parent vault note via the SIP-owned `chat_for`/`has_chats` relation (`docs/CONCEPTS/RELATION_TAXONOMY.md`); see `docs/CANVAS_CHAT_SURFACE/DEFINE_CHAT_ARTIFACT_DURABILITY.md`. | Session/chat history can be treated as ephemeral runtime exhaust instead of a governed HKA artifact with its own identity/canonicality/GC posture. |
