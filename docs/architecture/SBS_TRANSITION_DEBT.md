...
| D3 | Persistence/store construction outside PDM ownership | PDM vs all state owners | `to verify` — direct store/DSN/migration construction sites not yet inventoried. | Storage technology leaks into semantics; migrations scatter. | High | Open. |
| D4 | Retrieval result shape not formalized as ContextBundle | RCA vs HIX/CAO/MEM/HKA | `to verify` — retrieval output shape on live paths not yet conformance-checked. | Retrieval output treated as truth or prompt stuffing. | Medium | Open. |
...
| D3 | Require StorePort and no private DSN/store construction in new work. | Platform persistence resolves through `StorePort`; PDM owns DSNs, migrations, lifecycle. | PDM | #2358 | "No PDM bypass for platform persistence/store resolution" |
| D4 | Use ContextBundle for scoped candidate evidence with non-authority posture. | Retrieval emits `ContextBundle` with scope + provenance; RCA never writes HKA. | RCA | #2359 | "Retrieval becomes truth" failure mode; ContextBundle candidate-only |
| D5 | Use MemoryRecord with review state, provenance, confidence, staleness, correction, forgetting, and GOV promotion path. | Memory carries review/provenance; promotion to HKA only via GOV. | MEM | #2360 | "No memory promotion to HKA without GOV" |
...
- Persistence/store construction outside PDM ownership — D3 (`to verify`).
- Retrieval result shape not formalized as ContextBundle — D4 (`to verify`).
