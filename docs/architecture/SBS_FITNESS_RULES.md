...
| I-MA11 read-side-effect freedom | Blocking invariant target; manual review now | A query creates directories/store/schema, migrates, emits receipts, or otherwise mutates state. | Open CKM storage read-only and return a typed error for absent, old, or unsupported stores. |
