...
| No Builder learning or TCD signal bypasses governed Builder destinations. | Manual review now | Failed prompts, quota/context failures, rework, failed gates, high-TCD decisions, or release/UAT failures are stored only in chat context, hidden prompts, or runtime/user memory, or are promoted into Product HKA/MEM without Product authority. | Route to issue/PR receipts, BuilderOps `LearningSignal`/receipts, skills, transition debt, fitness rules, roadmap, or prompt/policy PRs; use Product owner path only for explicit Product changes. |
