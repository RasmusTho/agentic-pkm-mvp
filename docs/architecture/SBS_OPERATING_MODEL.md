...
| Cross-plane information authority | `docs/architecture/INFORMATION_AUTHORITY_MODEL.md` | Accepted target-state overlay for authority, provenance, projection, and explicit promotion across PKM and Builder; existing local owner docs remain authoritative for local detail and current truth. |
| Cross-plane artifact classification and lifecycle | `docs/architecture/ARTIFACT_CLASSIFICATION_AND_LIFECYCLE.md` | Accepted target-state overlay for orthogonal artifact standing and Capture → Explore → Synthesize → Propose → Promote → Implement → Verify → Supersede/Retire routing; local lifecycle contracts remain authoritative. |
| DevUI discovery/control-plane boundary | `docs/architecture/DEVUI_DISCOVERY_ARCHITECTURE.md` | Accepted target-state discovery and navigation boundary; DevUI remains a projection over source owners, not a docs portal, source registry, task store, or promotion authority. |
...
| Specialist subagent role adapters and agent settings | `.codex/agents/**` (Codex project-scoped TOML execution-role adapters, plus the legacy CI-invoked `docs-guardian.yaml` automation agent), `.codex/config.toml` agent settings | Builder System governance, routed by `AGENTS.md`, `.codex/skills/README.md`, and `docs/development/BUILDER_SUBAGENT_ROLES.md` | Codex-specific execution-role configuration; not canonical workflow contracts, not runtime/system-agent semantics | Durable repo-governed artifacts | Governance-lane PR or bounded issue-backed PR | PR validation, TOML parse/schema and role-map consistency tests, and delivery receipt; subagent handoff and failure feed Builder learning only, never runtime/user memory | Executes Builder workflows over Product/Runtime changes but does not define Product/Runtime behavior or workflow contracts; adapters route work into `.codex/skills/**` rather than carrying contract text | `pytest -q tests/governance/test_codex_agents_contract.py`; manual role-boundary review that each adapter loads the relevant skill instead of duplicating its contract |
| Issue intake and lifecycle controls | `.github/ISSUE_TEMPLATE/task.yml`, issue labels, optional Project Status rules, issue pickup claim scripts, issue maintenance workflow | Builder System governance with GitHub Project as optional lifecycle projection | Governance-bearing shared signal for executable work | Durable GitHub Issue/PR state plus durable repo templates; Project status is a rebuildable projection | Explicit `gh` mutations for live labels/state; GraphQL only for explicitly scoped Project repair; template/config changes via PR | Claim, block, ready, and closure comments or receipts; optional Project state verified only when mutated | SBS Impact blocks route Product/Runtime work and Builder work; GitHub state is not Product runtime truth | `gh issue view --json state,labels`; optional `projectItems` read after a projection mutation; template/config tests where applicable |
| PR governance and publication workflow | `.github/pull_request_template.md`, `.github/github-governance.yml`, PR hot path/escalation docs, `publish-pr`, conditional `pr-integration`, review-gate fallback policy | Builder System governance | Normative merge/review/publication contract for repo-governed changes | Durable repo templates/config plus durable GitHub PR state | PR template/config/docs changes via PR; PR state via explicit `gh` commands | PR body must carry lane, SBS Impact, validation, BuilderOps routing where required, owner-doc writeback resolution, and final delivery receipt | Governs how Product/Runtime changes become accepted repo truth; does not itself ship runtime behavior | Governance tests for PR-body rules; current-SHA CI for every path and independent local review only on the full path |
| CI, architecture fitness, validation, and verified-merge convergence rails | `.github/workflows/**`, `tests/architecture/**`, `tests/governance/**`, `importlinter.ini`, docs guard scripts, validation commands in workflow docs, verified-merge projection wait/write-once receipt helper and phase builder | OEF for fitness posture, Builder System governance for delivery use, CES practice for SBS rule lifecycle | Enforceable or manual-review governance depending on rule posture; convergence receipts are authority-bound delivery evidence, never Product/Runtime authority | Durable repo-governed code/config; CI and live GitHub projection reads are ephemeral evidence attached to PRs, while the one authenticated content-addressed convergence comment and phase receipts are durable delivery records | Implementation, docs-authoring, or governance PR depending on touched surface; rule posture changes follow §11 | CI/check output recorded in PR; full-path prepared phase additionally requires authenticated post-edit check identity, an empty-read quorum, a fresh final observation, and consumer readback of the one durable convergence comment; fitness-rule status updates only when real enforcement exists | Verifies Product SBS boundaries and Builder workflow contracts; OEF observes/blocks but does not mutate runtime policy, and projection reads never replace post-merge closure attribution | `pytest -q tests/architecture` and targeted governance/verified-merge tests for touched rules |
...
| Governed Builder design runs (delivered, dormant) | `app/builderops/design_run_contract.py`, `app/builderops/design_agent_adapters.py`, `app/builderops/design_run_governance.py`, the `builderops design-run` operator CLI, design-run artifacts/receipts, and the read-only CKM cockpit design-hub projection; capability contract `docs/CKM_DESIGN_AGENT_INTEGRATION/README.md` | Builder System / CKM, above the ADR-0064 model-access substrate | Produces Builder design material only; a design result is never accepted repo, Product/Runtime, or CKM truth, and the hub holds no ranking, selection, or fallback authority | Durable repo-governed code plus append-only BuilderOps design-run artifacts and causal receipts | Issue-backed implementation PR; making a run executable additionally requires a governed host-secret-contract change through the credential-contract path, not a docs or config edit | Admission, approval, transition, refusal, and result receipts in the BuilderOps store, plus the normal delivery receipt on the governing Issue/PR | Generates Builder design material about Product/Runtime surfaces without becoming a Product/Runtime subsystem, a runtime capability, or a Companion UI surface | `pytest -q tests/builderops/test_design_hub_acceptance.py::test_design_hub_production_matrix_is_fail_closed` and `tests/builderops/ckm/test_design_cockpit.py::test_design_hub_projection_preserves_direction_b_authority` |
| Recovery and failure workflows | Quota/context/subagent failure handling, branch/worktree preflight, git hygiene, dispatcher fallback, review-gate fallback, blocked/needs-human transitions | Builder System governance | Normative safety and coordination policy for builder execution | Durable workflow docs/scripts plus durable GitHub blocker receipts; transient terminal state is not authority | Workflow/script changes via governance PR; live recovery state via explicit labels and comments; optional Project repair is separate | Blocker or recovery receipt names the failed gate, fallback used, timestamp, and next action; BuilderOps learning only for upstream divergence | Prevents builder failures from corrupting Product/Runtime repo truth; recovery material must not become runtime/user memory | Preflight output, `gh issue view --json state,labels`, recovery comment, and relevant workflow test when a script changes |

**On the design-run row specifically.** Parent #4131's six child slices are closed with merged PRs,
so the contracts, adapter registry, governed lifecycle, operator CLI, and cockpit projection exist on
`main`. The capability nevertheless **ships dormant and fail-closed and no design run can execute**:
the host secret contract declares no `builderops-design-run` consumer on any channel, so all three
registered design agents report unavailable on `dev`, `test`, and `prod` (`codex` and `fable` as
`model_access_unavailable`, `claude-design-via-claude-code` as `interactive_subscription_only`), and
no credential is read on the availability path. `claude-design-via-claude-code` is never
headless-available by invariant (INV-CDH-5A), not by configuration. The `design_agent_profiles` rows
in `docs/settings/models/providers.yaml`, including the `prod` channel, are ADR-0064 census
...
2. **Classify it as blocked by evidence/system** and remove it from the autonomous-ready queue.
   Gate outage, retry exhaustion, tool incompatibility, or a safe fail-closed pause is not by itself
   an owner decision and must not create `agent:needs-human`.
3. **Record a blocker receipt** on the issue/PR: which gate, why it was unavailable, and the timestamp.
4. **Route through the autonomous escalation classifier** in
   `docs/development/AUTONOMOUS_REVIEW_REPAIR_GATE_CONTRACTS.md`. Retry, repair, or use an approved
   alternate reviewer path only when the governing contract permits it.
5. **Do not merge** until the required gate succeeds. A non-waivable CI/review/merge gate cannot be
   replaced by a human approval. Escalate to `agent:needs-human` only when the classifier names a
   separate Human Exception authority category; that decision may resolve scope or authority but
   may not fabricate a passed gate.

An unprotected `main` branch does **not** waive this gate: "autonomous merge" means running the
applicable tier's full gate chain, not bypassing it because branch protection is off.
