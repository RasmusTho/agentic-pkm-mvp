...
| Issue intake and lifecycle controls | `.github/ISSUE_TEMPLATE/task.yml`, issue labels, Builder Ops Vault tickets/claims, dispatcher transition fallback, issue pickup claim scripts, issue maintenance workflow | Builder System governance with Builder Ops Vault as active operational status; GitHub Project deprecated as hot-path dependency | Governance-bearing shared signal for executable work | Durable GitHub state plus vault Markdown/YAML operation files; Project status is optional/deprecated projection | Vault mutations through `builderops vault`; GitHub labels/comments through REST; template/config changes via PR | Claim, block, ready, and closure comments or receipts; vault validation verifies folder/YAML/claim invariants | SBS Impact blocks route Product/Runtime work and Builder work; GitHub/vault state is not Product runtime truth | `builderops vault validate`; REST issue/PR checks; template/config tests where applicable |
| PR governance and publication workflow | `.github/pull_request_template.md`, `.github/github-governance.yml`, PR hot path/escalation docs, `publish-pr`, `pr-integration`, review-gate fallback | Builder System governance | Normative merge/review/publication contract for repo-governed changes | Durable repo templates/config plus durable GitHub PR state | PR template/config/docs changes via PR; PR state via explicit REST-first `gh api` commands | PR body must carry lane, SBS Impact, validation, BuilderOps routing where required, owner-doc writeback resolution, and final delivery receipt | Governs how Product/Runtime changes become accepted repo truth; does not itself ship runtime behavior | Governance tests for PR-body rules; PR review/CI via REST-only check wait before merge |
...
| Roadmap, vault, and projection surfaces | Builder Ops Vault, optional GitHub Project/read-only board, issue dependency comments, `docs/ROADMAP.md`, `docs/architecture/SBS_ROADMAP.md`, BuilderOps roadmap/docs-freshness projections | Roadmap owner docs for stable strategic truth; Builder Ops Vault for active operational movement; GitHub Project deprecated for hot-path automation | Mixed: owner docs are strategic authority; vault state is operational authority; projections are derived | Owner docs durable; vault operational files external to product repo; projections durable but derived or high-churn | Stable roadmap changes via PR; vault operations through `builderops vault`; BuilderOps projections regenerated from source records, not hand-edited as authority | Movement receipts on issues/PRs or vault tickets; owner-doc writeback only when stable truth changes | Routes Product SBS initiatives and Builder System delivery without replacing Product owner docs or shipped status docs | `builderops vault validate`; roadmap diff review; projection source-record check when BuilderOps is involved |
