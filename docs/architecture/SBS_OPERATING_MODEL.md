...
| Cross-plane information authority | `docs/architecture/INFORMATION_AUTHORITY_MODEL.md` | Proposed target-state overlay for authority, provenance, projection, and explicit promotion across PKM and Builder; existing local owner docs and ADRs win until accepted. |
| Cross-plane artifact classification and lifecycle | `docs/architecture/ARTIFACT_CLASSIFICATION_AND_LIFECYCLE.md` | Proposed target-state overlay for orthogonal artifact standing and Capture → Explore → Synthesize → Propose → Promote → Implement → Verify → Supersede/Retire routing; local lifecycle contracts remain authoritative. |
| DevUI discovery/control-plane boundary | `docs/architecture/DEVUI_DISCOVERY_ARCHITECTURE.md` | Proposed target-state discovery and navigation boundary; DevUI remains a projection over source owners, not a docs portal, source registry, task store, or promotion authority. |
