...
> inter-system relationship — are **superseded by ADR-0043** (Heimdall naming + ecosystem Norse name
> register, #2886) and its owner-captured decision **D-NAME-WHOLE**: **Yggdrasil is the whole**
> (the acknowledged SoS); the constituents are **Munin** (knowledge/memory), **Hugin** (agent-runtime),
> and **Heimdall** (sensor). "Federation" is reserved for SFC and is **not** used for the SoS
> relationship. The owner ratified ADR-0043 as canonical (2026-07-04); RESEARCH-08 conforms to it via
> ADR-0044. Aligning this artifact's prose to that model is a flagged follow-up (**#2890**). Until
> then, **where this artifact and ADR-0043 disagree on naming/model, ADR-0043 wins.** The seam and
> interaction *substance* below (public/private invariant INV-EF1, tier classification, governed
> event/candidate intake) still stand — only the naming/whole-vs-constituent framing is superseded.
