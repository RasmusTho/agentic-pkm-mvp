...
| 5 | `file_state` row | `(vault_binding_id, path)` PK, `uuid` secondary (`c7f4b1a83d29_mvr05a0_file_state_binding_key.py`) | upsert per sync; explicit DELETE on delete/rename (`vault_sync.py:166-168,203-206,221`) | derived (sync bookkeeping) | full vault rescan | delete/rename only; stale rows persist if watcher misses an event |
...
| 15 | Memory: `agent_memories` (short-term) | `id` UUID (`d1e8a0c5f37b_mvr05a1_objects_agent_memories_adoption.py`) | append + timestamp decay (`memory_kv/store.py:164-191`) | derived (working memory) | lossy — decay is the design | timestamp decay only |
