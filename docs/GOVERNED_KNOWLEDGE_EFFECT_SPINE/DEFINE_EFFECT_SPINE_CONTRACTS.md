...
| EXE effect producers | `app/execution/execution_request.py` and real-tool adapters | Authorized external/tool effect and normalized execution result | Validate the bound token, execute once, return an effect/mutation receipt, then let GOV record accountability | EXE must not decide policy, infer permission, mint tokens, or write HKA/MEM state directly |
