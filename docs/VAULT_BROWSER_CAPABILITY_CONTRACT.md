...
Shipped (#1473 via #1554/#1555): the existing frontmatter-preferred/path-derived `zone` now carries this envelope as additive metadata — `zone_source` (`frontmatter.zone` | `vault_path_segment`), `zone_authority_role` (`durable_vault_metadata` | `runtime_projection` | `unavailable`), `zone_provenance`, and `zone_degradation` (`none` | `frontmatter_absent` | `frontmatter_invalid`). The Vault Browser UI surfaces this so a durable, frontmatter-authored zone is visibly distinguishable from a path-derived runtime projection, and a degraded zone exposes its provenance/degradation. The envelope describes the **current** source only; a genuinely new topology source (registry/graph/semantic neighborhood) remains future per #1488.
...
- read-only enumeration of Markdown notes in the active vault — the vault the
  ...
  - **No-vault front door — "Choose a vault" overlay (#2564, part of #2561).**
    ...
    (Role folds into the Ask-3b "Initialize here" confirm; typed-path chrome is
    retired everywhere — the loaded-note in-app switch reuses this same overlay,
    #2590). The overlay dispatches the existing `vault.select` action —
...
**#1488 (topology authority/runtime decision):** Runtime Vault Browser reads remain active-vault only. The current browser `zone` source order is frontmatter `zone` first, then first vault-relative path segment as deterministic fallback. No topology registry, graph projection, multi-vault selector, or semantic neighborhood source is authoritative for browser reads. Future topology-derived fields must carry source, authority role, provenance, and degradation behavior; they may not overwrite frontmatter `zone`, mutate vault metadata, or influence ordering/overlays/filters unless a bounded implementation issue surfaces the contributing topology signals and tests the degradation path. On the strength of this decision, #1473 was rewritten into a parent validation hub and split into bounded children #1554/#1555 (delivered — see the #1473 entry below); a genuinely new topology source (registry/graph/semantic) remains deferred.

**#1473 (topology-aware zone projection — self-describing zone):** Delivered via #1554 (PR #1558) and #1555 (PR #1561). The Vault Browser `zone` projection is now self-describing under the #1488 envelope without introducing a new topology source. The API emits, additive to the existing `zone` value, `zone_source` (`frontmatter.zone` | `vault_path_segment`), `zone_authority_role` (`durable_vault_metadata` | `runtime_projection` | `unavailable`), `zone_provenance`, and `zone_degradation` (`none` | `frontmatter_absent` | `frontmatter_invalid`); malformed frontmatter degrades to the path posture (`frontmatter_invalid`) and never fabricates a zone. The Companion UI artifact inspector surfaces these signals (`data-zone-authority-role`, a visible `frontmatter` vs `derived from path` affordance, and an inspectable `workspace-vault-browser-inspector-zone-provenance` detail for degraded zones), reading the server envelope verbatim with no client-side authority re-derivation. The existing `zone` value, zone filter contract (§4.3), and `zone` frontmatter schema are unchanged, and no zone-based ordering/overlay or semantic ranking was introduced. Realizes the §4.1 envelope for the current frontmatter/path source; new topology sources stay deferred per #1488.
...
**#1547 / #1474 (agent-memory posture projection):** The vault browser API now includes a separate per-note `agent_memory_posture` field when the agent-memory review-queue source is available. The field is populated by `app.agent_memory.posture_projection.agent_memory_posture_for_artifacts`, a read-only batch projection over explicit `MemoryCandidateReviewQueue` entries keyed only by candidate `source_refs` / `derived_from` artifact references such as `note_path:<path>` and `artifact_uuid:<uuid>`. It does not parse arbitrary companion note body text, scan ObjectStore metadata, read raw events, mutate the queue, promote/reject/revise candidates, or authorize browser actions. The payload remains separate from frontmatter posture (`review_state` / `trust`) and receipt posture (`receipts`) and carries `authority="non_authoritative"`, aggregate `state`, lifecycle `counts`, and read-only item rows. The Companion UI inspector renders this in `data-testid="workspace-vault-browser-inspector-agent-memory-posture"` with `data-agent-memory-authority="non_authoritative"` and no mutation controls.
...
**#1471 (UI-only multi-note selection substrate) — REMOVED by #2526 (PR #2529, 2026-06-25):** This substrate previously added a per-row selection checkbox (`data-testid="workspace-vault-browser-selection-toggle"`), row-level `data-selected` state, and a local selected-count summary (`data-testid="workspace-vault-browser-selection-summary"`), all classified `ui_only`. It is now removed: no action was ever wired to it (toggling only updated DOM/selected-count state and never navigated, called the server, created receipts, persisted state, or introduced bulk review/trust/edit actions), so it was dead UI carrying no capability and was flagged in a live operator review. The browser list no longer renders the selection checkbox, the `data-selected` row attribute, or the selected-count summary; note opening remains the separate read-only row link. A future bulk-action capability that genuinely needs multi-select must re-introduce the substrate together with the action it serves (not as a standalone `ui_only` affordance).
...
**#2590 (loaded-note vault switch reuses the Choose-a-vault overlay):** Part of #2561. The loaded-note vault surface (the "V" vault chip on a loaded note) previously rendered the old foreign-form drawer — typed "Open existing vault" / "Create/init vault" path fields, a machine-Role select, Initialize / Reload, plus the scoped-settings editor. That violated the same two rules #2564 fixed for first contact (no typed paths; one graphical language) and conflated vault *switching* with vault *settings*. This slice separates the two concerns by **reuse, not rebuild**: the "V" chip now opens the **same `_render_vault_selection_required_section` Choose-a-vault overlay** (delivered for the first-contact picker by #2564/#2565/#2572), hosted as a hidden reveal (`data-testid="vault-switch-host"`) over the loaded note — same titled overlay, same visible-list filter, same clickable rows, same Browse-for-a-folder filesystem mode over `GET /api/companion/vault/browse`, same operator ⓘ drawer, and the **same `_render_vault_picker_script` controller** (it dispatches the existing `vault.select` / `vault.initialize` authorities, incl. the #2518 409-confirm). The ONE new render-side piece is a small `_vault_switch_payload(fields)` data-shaper that builds the payload the overlay already consumes (a neutral `reason="switch"`, switch copy from the server-declared `runtime_vault_name`); it adds no browser, controller, or endpoint, and fabricates no path — switching is always a visual row/Browse click, never a typed path or a Role select. The scoped vault **settings** (the `enableVaultWatcher` / `allowWritesToVault` / `allowSharedSettingsEdits` / `allowLocalSettingsEdits` flags, handoff/assets folder fields, Save) relocate to a new **"vault" section of the Settings drawer** (a server-write surface, `data-authority="server-write"`); the existing `vault.settings.write` controller (`vault_settings_panel_script`) stays the single owner of that write and the #2518 confirm guard — only its mount point moved. Reload / "Open settings folder" / Role are reached behind the overlay's ⓘ / Operator drawer / the init confirm, not the front of the switch. Vault resolution/identity stay server-declared; no new authority. Multi-vault active switching remains future (#2566 / #2143). See `docs/SYSTEM_ENTRY_POINT/SETTINGS_DRAWER.md` for the relocated settings section.
