...
- Manual run:
  - `python -m app.cli settings validate`
  - `POLICY_ENFORCE=1 WATCHER_ENABLE=1 WATCHER_VAULT_PATH=/path/to/vault WATCHER_DEBOUNCE_MS=1500 WATCHER_RATE_LIMIT_PER_MIN=30 WATCHER_BACKOFF_SECONDS=10 INDEX_OUTBOX_PATH=tmp/index-outbox.live.jsonl python -m app.cli watcher run`
