...
| Forward-only migration is found for a channel | Stop that candidate before channel mutation and report the exact source-to-target migration classification. Never set `--ack-forward-only` or `DEPLOY_ACK_FORWARD_ONLY` from automation, and do not treat GitHub environment approval as the migration acknowledgement. | Keep that channel and candidate on the existing operator-governed deploy path. Resume only after the existing explicit acknowledgement for that exact channel/candidate and its normal receipt; classify independently for dev, test, and prod. Production retains its target-bound migration-token preflight. |
...
- A forward-only migration stops the automatic path before channel mutation and cannot proceed without the existing explicit, candidate- and channel-specific operator acknowledgement; automation never supplies a blanket acknowledgement.
