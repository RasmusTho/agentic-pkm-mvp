...
description: Implement provider discovery adapters, freshness-bounded snapshots, and safe latest-compatible selection from verifiable provider metadata.
task_id: MARR-04
github_issue: 5622
source_anchor: docs/adr/ADR-0066-shared-model-access-router-and-catalogs.md :: D4
parent_capability: MODEL_ACCESS_ROUTER
prerequisites: [MARR-01, MARR-02, MARR-03, MARR-08]
depends_on: [ESTABLISH_SHARED_ROUTE_AND_PROVENANCE_CONTRACTS.md, BUILD_ADAPTER_REGISTRY_AND_CODEX_CLI_TRANSPORT.md, FORMALIZE_OLLAMA_AND_PREFLIGHT_FALLBACK.md, ADD_TAILSCALE_CODEX_EXECUTOR_TRANSPORT.md]
...
Add a common descriptor and freshness-bounded CatalogSnapshot containing exact model ID, provider, allowed transports, declared capabilities, supported reasoning efforts, source release/created or explicit replacement metadata, deprecation/sunset status, limits/pricing where provided, source/fetched time, freshness, and deterministic hash. Product consumes the Codex snapshot from the authenticated executor catalog operation; the Codex login and discovery process remain on the designated macOS host. Product validates the returned descriptor set and content hash before route selection.

Implement separate read-only discovery adapters:

- Codex app-server model/list using the existing host-local login; retain account-scoped available model IDs and reasoning efforts in a host-local snapshot. MARR-08 exposes only its sanitized descriptor/hash over the authenticated Tailscale boundary. Its list order is not release order. Use only a provider-supplied release timestamp or explicit upgrade relation to select a newer target; otherwise leave Luna or another explicit policy target pinned.
...
- [ ] Codex account catalog discovery is read-only, returned only through the authorized executor catalog operation, and does not infer release order from list position.
...
- [ ] Catalog refresh makes no model invocation and no snapshot/receipt field contains a credential, prompt, raw Tailscale identity/capability claim, or endpoint secret.
