...
The executor receives an exact resolved target such as provider=openai, model=<catalog-resolved-id>, transport=codex_cli; preflight either returns ready or a typed preflight failure. The host-local profile is loaded by path and binds the exact CLI version, tool-surface inventory version, and model-catalog schema version; callers cannot inject a profile object. Before each inference the executor reads the bundled model catalog, validates every required field and declared value type, rejects unknown fields or an absent exact model, and supplies Codex a temporary catalog with every known model-derived tool surface neutralized. Fixture tests verify ChatGPT-only auth, malformed-catalog rejection, the hard file-size ceiling, strict JSON constants, the sanitized catalog, disabled configuration gates, isolated working directory, separate instruction/user fields, descendant cleanup after leader exit, and process-group cleanup on the outer bridge timeout. This is adapter-level evidence, not a substitute for the designated-host runtime acceptance before activation. The factory never picks the next catalog model itself.
...
- [x] Codex preflight checks CLI version, ChatGPT subscription login mode, and the bundled catalog for the exact model; it rejects API-key auth and reports typed missing-cli/session-expired/model-unavailable failures without exposing host secrets.
...
- [x] Schema handling uses --output-schema only when supported and otherwise strictly validates JSON locally, rejecting non-standard numeric constants.
...
- [x] A successfully exiting CLI cannot leave same-group descendants running after the final response returns.
  - Verify: `tests/model_access/test_codex_cli_adapter.py::test_exit_cleanup_kills_descendant_with_redirected_output`
