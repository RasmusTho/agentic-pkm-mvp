...
| 5 | **Voice-memo capture adapter** — watch the iCloud Shortcut folder, admit under grant, write raw record (encrypted at rest, `content_identity`, `capture_chain`, KERNEL-06 stamping), delete-after-confirmed-ingest | **delivered (#3025)** | The only component touching the source. `app.heimdal.capture_adapter` + `app.heimdal.raw_store` (migration `d5a8e2f1b6c3`); see `docs/EVENTS.md :: Heimdal raw-evidence store + voice-memo capture adapter`. |
