...
| 10 | **Mimer projector** — cursor consumer; projects events into bundle-carrying candidate artifacts (`requires_review: true`, noncanonical, capture scope, provenance chain) entering existing triage | **delivered (#3031)** | The read-model that proves the seam. `app.heimdal.candidate_projection.project_pending_candidates` — its own per-consumer cursor (`mimer.candidate_projector`, independent of A3's `mimer.content_quarantine_projector`), folds supersede/revision chains last-correction-wins, writes `heimdal_observation_candidate` notes through the governed `write_note_relative`/`WriteGuard` path (mirrors `app.knowledge_acquisition.candidate_writeback`'s posture). The client contract's [Sources zone](../contracts/MIMER_CLIENT_CONTRACT.md#sources-zone--sensoracquisition-landing-zone) is the settings-named landing zone for published/transcribed sensor content notes; its concrete filesystem/settings enactment remains follow-on work. |
