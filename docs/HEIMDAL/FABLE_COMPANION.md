State: Draft (Fable window design output, 2026-07-05). The living companion document for the Heimdal Fable-5 architecture window: the proposed design for all 8 OPEN problems in `CAPABILITY_CHARTER.md`, anchored in the voice-memo v1 vertical. Advisory until owner-accepted and enacted via CES/ADR; creates no runtime behavior, no schemas-as-shipped, and no GitHub work.
Doc role: Companion / living design thread (Fable window) — prose-mirror contracts + proposed invariants
Authority: Authoritative for the *proposed* Heimdal design within the FIXED constraints of `CAPABILITY_CHARTER.md`. Subordinate to `OWNER_DECISIONS.md` (reserved calls stay reserved; this doc surfaces them, it does not take them), `ECOSYSTEM_SOS_MODEL.md` (container), and the existing SoT contracts it reconciles against (`docs/EVENTS.md`, `docs/KNOWLEDGE_ACQUISITION/*`, `docs/architecture/*`). Claims no shipped reality. Every schema sketched here is a prose mirror; the machine-readable schema (minted at enactment under `schemas/`) will be canonical.
...
# Heimdal Fable window — capability design (companion draft)

## Read first

- `docs/HEIMDAL/CAPABILITY_CHARTER.md` — FIXED vs OPEN; this doc solves the OPEN set within the FIXED set.
- `docs/HEIMDAL/OWNER_DECISIONS.md` — reserved decisions; §9 below routes back to them.
- `docs/HEIMDAL/ECOSYSTEM_SOS_MODEL.md` — where Heimdal sits (sibling constituent; Layer-2 substrate inventory).
...
A **Heimdal observation event** is the published, minimized, attributed record that *something was observed in reality*: an interval of time, a set of actors and entities, observed content at a stated fidelity, a consent-state, and a provenance chain back to immutable raw evidence. It is the terminal artifact of Heimdal's responsibility (Charter FIXED #2) and the *only* thing that crosses the raw→published seam by default (FIXED #5).

Two event classes exist and must never be conflated `[extend]`:

1. **Published observation events** — the cross-constituent contract. Topic family `heimdal.observation.*`. These are what Mimer and future constituents consume. Everything in §1.3 is about these.
2. **Heimdal-internal pipeline events** — stage progress, capture receipts, retries. These stay inside Heimdal (mirroring how KAP stage events stay inside KAP) and ride whatever internal mechanics Heimdal wants; they are not part of the constituent seam and are not designed further here beyond conforming to the outbox idempotency discipline.
...
- **Threads / semantic correlation** (this memo continues yesterday's topic; these three memos are one project) are **downstream read-model concerns**, not Heimdal's. Heimdal correlates only what capture mechanics know (episode, device, place-session). Semantic threading is Mimer projection/retrieval work over the stream — putting it in Heimdal would smuggle interpretation into the sensor (Charter FIXED #2). Events may carry an optional `correlation_hints[]` (e.g. same place-session grant) but hints are advisory. `[conform — interpretation stays downstream]`
- **Corrections** are new events (`heimdal.observation.corrected`, §3.5) carrying `supersedes`; **revisions** (better stage re-runs over the same raw) are new events carrying `revision_of`. Projections must fold supersede/revision chains; the log never rewrites (HEIM-1).
...
3. **Thresholds are consumer policy, not Heimdal's.** Heimdal publishes low-confidence events rather than suppressing them (an unpublished observation is invisible uncertainty — worse). What a consumer does at `transcription < x` is that consumer's declared policy; the promotion gate (HEIM-8) is where low confidence must bite hardest.
...
Companies, projects, places, named systems ride the identical mention → resolve → register path with a different `kind`. This is deliberate (owner emphasis): the register is the join layer that lets a memo mentioning "the Heimdal design" link to the same project entity as a calendar event and a note. No separate "topic tagging" mechanism — one register, one resolution contract, all kinds. `[extend]`

### 3.5 Correction-as-new-event

Topic `heimdal.observation.corrected` (schema at enactment; prose mirror): `{ supersedes: observation_id, corrects: mention_id | attribution_id | time | consent, replacement: <same shape as the corrected fragment>, basis, actor }`. Rules:
...
### 4.2 Recommendation: Heimdal-owned append-only log + per-consumer cursors, DB-native, outbox-disciplined

`[extend — generalizes the outbox pattern; does not change outbox ownership]`

- **The Heimdal observation log** is the canonical stream: an append-only Postgres table in the same database (monorepo, no split-trigger fired), rows = published events in envelope form, insert-only (no `delivered_at`, no updates, ever — enforced by grant/trigger at enactment).
...
- **The existing outbox is untouched.** Mimer's internal machinery keeps its queue; the Mimer projector consumes the Heimdal log via cursor and drives whatever internal outbox work it needs. No cross-constituent writes into each other's tables: Heimdal writes only its log; Mimer reads the log and writes only its own state. The seam is the log contract + read access, directional by construction.

### 4.3 Alternatives and trade-offs

- **(a) Reuse the outbox table directly** (Heimdal inserts into `outbox`, worker dispatches `heimdal.*` topics). Cheapest to ship — and wrong: single-consumer `delivered_at` semantics can't serve sibling read-models; rows-as-queue is not rows-as-canonical-log; and it makes Heimdal's canonical stream a Mimer-internal implementation detail, silently demoting Heimdal to a Mimer subsystem (violates FIXED #1). Rejected on architecture, not effort.
- **(b) Stream-native broker now** (NATS JetStream / Redis Streams / Kafka). Real replay/consumer-group semantics off the shelf, honest v2 posture. Rejected for v1: new always-on infrastructure on a 4 GB Colima host for ~5 events/day; a second persistence technology to operate, back up, and secure inside the raw-data trust boundary; and the DB-native log meets every stated v1 requirement. Named as the *designated v2 escape hatch* — the publish/cursor contract is written so a broker can implement it without consumer changes; adopting one is a §4.2 transport swap plus an ADR, not a redesign. Split-trigger note: if the privacy threat model ever requires Heimdal on separately credentialed infrastructure (SoS model §4 trigger 2), the broker question reopens as part of that move.
...
> **Revised by owner decision §9-k (2026-07-05):** the owner has since confirmed **Heimdal = the ecosystem ingestion organ** (watch → fetch → transcribe → attribute for *all* external sources, not just voice). This demotes the §5.1 records-of-reality-vs-authored-content line from an *ownership boundary* to an event *typing*, and collapses §5.2's "separate backbones" toward one Heimdal acquisition backbone with Mimer downstream. §5 is retained for its reasoning; §9-k is the standing decision (a `reshape` → CES/ADR).

### 5.1 The boundary question, made concrete by the voice memo

Is a deliberately-recorded voice memo a Heimdal capture (observation) or a KAP source (authored artifact → candidate)? Both readings are defensible: the memo is discrete, batch, deliberately created, lands as a file — KAP-shaped mechanics; KAP even already specs an ASR stage (KA-02). But the memo also demands attribution, consent-state, episode time semantics, and the raw privacy seam — none of which KAP has or should grow.

**Recommendation: the voice memo is a Heimdal capture. The discriminator is the nature of the claim, not the mechanics of arrival** `[extend — proposed boundary rule]`:

> **KAP acquires *authored content* whose value is the content itself, detached from the moment (a video, a paper, a feed item). Heimdal captures *records of reality* where when/who/where is load-bearing (something happened, someone said something, at a time, in a context).**

A voice memo is a first-party record of lived reality: its provenance is "reality, via my sensor"; its timestamp, speaker, and mentioned entities are the point. Run it through KAP and the event semantics die at the door: KAP's `RawEvidence` has no consent-state, no attribution, no episode, no encrypted raw seam, and its terminal artifact (candidate) erases the fact-of-occurrence that makes a memo useful to a timeline or an agent's situational awareness. The cost of the Heimdal routing is building capture machinery KAP would have given for free — accepted, because that machinery is exactly what the Heimdal window exists to build.

Edge case honesty: a memo that is pure dictation ("draft: email to Anna about the invoice") is *authored content in an observational wrapper*. It still enters as a Heimdal capture (the sensor cannot classify intent at the seam, and must not — that is interpretation), and the Mimer projection turns it into a candidate note through the normal triage path. **The backbones meet at the candidate, not at acquisition** (§5.3): downstream of a published event, an "authored-artifact-shaped" observation flows into exactly the same candidate → triage → promotion path a KAP acquisition feeds. Nothing is lost; the event wrapper adds the timeline fact for free.

### 5.2 Backbone recommendation (D-BACKBONE): shared primitives, separate backbones

**Heimdal does not ride KAP's acquire→candidate→publish pipeline. Both backbones conform to a shared set of Layer-2 provenance/replay primitives, promoted from KAP's contracts.** `[extend — substrate promotion per SoS model §5; enactment is R-PROMOTE, owner-gated]`

The shared primitives (the "shared provenance standard" the owner fixed as guardrail, made concrete):

| Primitive | KAP form | Heimdal form |
| --- | --- | --- |
...
| Per-stage idempotent events | KAP stage events on the outbox | Heimdal-internal pipeline events (§1.1), same idempotency discipline |
...
Why separate backbones: the two constituents have different terminal artifacts (candidate vs published event), different sensitivity postures (KAP raw = re-fetchable public content; Heimdal raw = the most sensitive data in the ecosystem, encrypted + policy-gated), different lifecycles (KAP pull/scheduled; Heimdal capture-driven), and different governance surfaces (consent has no meaning for a YouTube caption). Forcing one backbone means either weakening Heimdal's seam to KAP's posture or dragging KAP's public-content pipeline behind Heimdal-grade controls — both bad trades.

What is concretely shared anyway: the **ASR engine** `[extend — owner-directed 2026-07-05; see §9-j]`. The system already has it — `app/media/transcribe.py` (faster-whisper): `run_asr()` is the reusable engine, while `download_audio` (egress) + `_record_outbox(kind=transcript)` are KAP's wrapper around it. The owner's rule is "no two whisper instances." **Resolution: the ASR engine is a shared Layer-2 substrate as a *library + model cache*, invoked in-process inside each constituent's own trust boundary — never a shared ASR *service* that raw audio is shipped to** (a shared service would puncture Heimdal's seam, FIXED #5). This preserves the seam exactly as well as a Heimdal-owned engine: Heimdal invokes the shared library on raw audio *inside its seam* (raw never leaves), Mimer/KAP invoke the same library on their own (public) content. Concrete refactor: extract `run_asr` + model cache to `app/media/asr_engine.py` (shared substrate); KAP keeps its wrapper; Heimdal adds an in-seam wrapper that emits an observation event instead of a `kind=transcript` record; additively expose per-segment `avg_logprob`/`no_speech_prob` for the transcription confidence axis (§2). Mechanism sharing without pipeline merging — and without a second instance.

Alternatives:

- **(a) Generalize KAP into a universal acquisition platform; Heimdal sensors become a source-plugin class.** One backbone, one contract set, plugins all the way down. Rejected: smears the raw privacy seam across a contract whose raw layer is neither encrypted nor consent-aware; makes attribution/consent optional plugin metadata instead of structural requirements; and inverts the SoS decision (Heimdal becomes a KAP tenant, not a sibling). This is the seductive wrong answer.
...
An append-only ledger of consent grants and revocations, held inside Heimdal, mutated only by explicit operator action, and itself evented (`heimdal.consent.granted` / `heimdal.consent.revoked` on the observation log — consent changes are observations about the operator's declared will).
...
Trust boundaries crossed in v1, in order: device (Watch/iPhone) → **Apple iCloud** → mac-mini host filesystem → Heimdal raw store (encrypted at rest) → [ASR/attribution stages, local] → observation log (DB) → Mimer projector → vault/companion surfaces.

**Stated honestly:** the v1 capture chain transits Apple's cloud *before* Heimdal's seam begins. Raw audio in iCloud is inside Apple's trust domain (consistent with the existing vault-on-iCloud posture, but audio is more sensitive than notes). The Heimdal raw seam begins at ingestion; hardening the pre-seam hop (direct device→host transfer, e.g. Tailscale-local shortcut upload) is a named v2 item, and accepting the v1 iCloud transit is an owner acknowledgment (§9-h). `capture_chain[]` (§1.3) makes this legible per event forever.
...
The entity register inherits Heimdal-grade protection despite being Layer-2 substrate: reads governed (it answers "who is in this life"), mutations receipted, no bulk export operation exists in its contract. `[extend]`
...
- **Affected boundaries:** Heimdal log (constituent-internal); PDM/SIP on the Mimer projection side.
...
- **Future test path:** `tests/invariants/test_heimdal_stream.py::test_append_only_truth`.
...
- **Enforcement level:** `schema_enforced` at enactment (required provenance family) + `runtime_test` — **delivered (#3031, Epic #3019 slice A11)** at the candidate-projection seam. `app.heimdal.candidate_projection.project_pending_candidates` carries `content_identity` + `raw_ref` + `capture_chain` from the observation's provenance family onto the written candidate's frontmatter unchanged, and stamps `derived_from` (the source `observation_id`) so a downstream reader always resolves a candidate back to the exact observation it was projected from.
- **Future test path:** `tests/invariants/test_heimdal_stream.py::test_provenance_survives`.
...
- **Affected boundaries:** Heimdal capture adapters + consent ledger; HIX (grant surface).
...
- **Future test path:** `tests/invariants/test_heimdal_consent.py::test_consent_gated_capture`.
...
- **Future test path:** `tests/invariants/test_heimdal_seam.py::test_seam_minimization`.
...
- **Future test path:** `tests/invariants/test_heimdal_seam.py::test_policy_gated_raw_access`.
...
- **Affected boundaries:** Heimdal attribution/resolution stages; SIP (register); HIX (rendering unresolved as unresolved).
...
- **Future test path:** `tests/invariants/test_heimdal_attribution.py::test_attribution_honesty`.
...
- **Purpose:** A Heimdal event is candidate evidence; its Mimer projection carries `requires_review: true` / noncanonical standing and cannot become canonical human knowledge without a governed authority transition (token + receipt).
- **Protected principle:** Charter FIXED #3; `authority_transition_required_for_durable_mutation`, `promote_requires_governance` `[conform — extends the existing xfail skeletons to the Heimdal source path]`.
...
- **Future test path:** `tests/invariants/test_heimdal_projection.py::test_event_not_authority`.
...
- **Affected boundaries:** CAO, GOV, EXE on the Mimer side; Heimdal contract (events carry no imperative channel).
- **Enforcement level:** `runtime_test` — **delivered (#3040, Epic #3019 slice A3)** at the projector seam. The content-quarantine projector (`app.heimdal.projector`) is the sole sanctioned path from an observation into agent-visible context: it fence-neutralizes observed content (`app.heimdal.quarantine`) and emits only non-authoritative `requires_review` candidates with `evidence_role=observed_evidence`. No projector or quarantine entrypoint executes/mutates/grants on the basis of observed content; the neutralization sits at the one seam so no downstream consumer can reopen the injection surface. (The governed *promotion* of a reviewed candidate into canonical knowledge still rides the existing GOV xfail skeleton per HEIM-8/§11#11.)
- **Contract wording:** *Observed content is data, never instruction.* Observed content enters agent context only as fenced, untrusted evidence-candidates (`evidence_role=observed_evidence`, `requires_review: true`, non-authoritative); instruction-shaped text (imperatives, forged system/role headers, fenced-code breakouts, homoglyph/zero-width tricks) is neutralized so it cannot be interpreted as a command, and no auto-executing prompt path may consume raw observed content.
- **Test path:** `tests/heimdal/test_content_quarantine.py::test_observed_content_never_in_prompt_path`, `::test_fence_neutralizes_injected_directive` (+ adversarial-variant coverage) — exercised through the real production projector call site. (The earlier-planned `tests/invariants/test_heimdal_projection.py::test_observed_content_is_not_instruction` remains the future *invariant-registry* home once the full Heimdal projection module lands.)
...
- **Affected boundaries:** Heimdal capture adapters + publish path; DRI (timeline projections).
- **Enforcement level:** `schema_enforced` at enactment (all three fields required + `clock_basis` enum) + `future_runtime`.
- **Future test path:** `tests/invariants/test_heimdal_stream.py::test_bitemporal_honesty`.
...
- **Future test path:** `tests/invariants/test_heimdal_attribution.py::test_attribution_resolves_in_register`.
...
- **Affected boundaries:** Heimdal adapters/stages; EBF-analog; CES (posture review at adapter addition).
...
- **Future test path:** `tests/invariants/test_heimdal_seam.py::test_declared_egress`.
...
- **Affected boundaries:** Heimdal consent ledger + raw store; Mimer projections (DRI/HIX).
- **Enforcement level:** `future_runtime`.
- **Future test path:** `tests/invariants/test_heimdal_consent.py::test_revocation_propagates`.
...
- **Affected boundaries:** Heimdal pipeline; shared Layer-2 replay primitives (§5.2).
- **Enforcement level:** `future_runtime`.
- **Future test path:** `tests/invariants/test_heimdal_stream.py::test_replay_is_stage_local`.
...
### a. Heimdal-vs-KAP backbone (ratify D-BACKBONE)

- **Background:** Owner left the backbone open (D-BACKBONE) with the shared provenance standard fixed. §5 designs the answer.
- **The decision:** Ratify "separate backbones on shared promoted provenance/replay primitives" as the Heimdal–KAP relationship.
- **Options:** (1) One generalized KAP backbone, sensors as plugins — one contract set, but smears the privacy seam and demotes Heimdal to a KAP tenant. (2) Fully separate including provenance vocabulary — autonomy, but violates the fixed shared-provenance guardrail and forks join keys. (3) Separate backbones, shared primitives (`content_identity`, immutable raw, stage-local replay, single-touch source rule, KERNEL-06 stamping), shared ASR stage implementation — recommended.
...
- **Background:** The concrete boundary case the window had to make concrete. §5.1 recommends Heimdal-capture with the "record of reality vs authored content" discriminator, and routes dictation-shaped memos to candidates via Mimer projection.
- **The decision:** Ratify the discriminator rule as the standing Heimdal/KAP admission test for future inputs.
...
### j. ASR engine ownership — Heimdal-owned vs shared substrate (owner-directed 2026-07-05)

> **Superseded by §9-k (owner-confirmed 2026-07-05):** with Heimdal owning *all* ingestion and transcription, there is no competing Mimer transcription, so the "two instances" problem dissolves and ASR is simply **Heimdal-owned**. §9-j is retained for the reasoning; §9-k is the standing decision.
...
- **The decision:** Where the ASR engine lives — Heimdal-owned, or shared Layer-2 substrate.
- **Options:** (1) **Heimdal owns whisper**, Mimer takes over after transcription — clean for the seam, but forces KAP (which transcribes *public* content) to either duplicate the engine or depend on Heimdal (a public pipeline coupling to the most-private constituent). (2) **whisper = shared substrate.** Sub-variants matter: (2a) shared **library + model cache**, invoked in-process inside each constituent's boundary; (2b) shared **service** audio is sent to — **rejected**: shipping Heimdal raw audio to a shared service punctures the privacy seam (FIXED #5).
- **Recommendation:** **Option 2a.** It kills duplication *and* satisfies Option 1's intent: the engine is shared, but Heimdal still owns the *invocation on raw audio* inside its seam (raw never leaves), and Mimer takes over at the published event. Refactor: extract `run_asr` + model cache → `app/media/asr_engine.py`; KAP keeps its wrapper; Heimdal adds an in-seam, event-emitting wrapper; expose per-segment confidence additively. Validated: the v1 prototype was re-pointed at the real faster-whisper engine (one instance), whisper.cpp demoted to fallback.

### k. Heimdal as the ecosystem ingestion organ (owner-CONFIRMED 2026-07-05) `[reshape → CES/ADR]`
...
- **Background:** The §5.1 discriminator drew the Heimdal/KAP line at *records-of-reality vs authored-content*, leaving KAP (in Mimer) owning YouTube/web watch → download → transcribe. The owner's reframe: *monitoring, downloading, and transcribing are sensing/capture activities — they belong to Heimdal, not Mimer,* for **all** external sources, not just voice.
- **The decision (confirmed):** **Heimdal is the ecosystem's ingestion/sensing organ.** It owns the front of the chain for every external source — **watch → fetch → transcribe → attribute → published event/candidate**. **Mimer** owns cognition from the handoff: **extract meaning → integrate → promote to knowledge**. Mimer never watches, fetches, or transcribes.
- **Handoff point:** Heimdal resolves *who/what was observed* (attribution, register refs, provenance, confidence); Mimer decides *what it means* (claims, summaries, promotion). Consistent with the interpretation boundary already fixed in §2.1 (interpretation is downstream).
- **What this supersedes / revises:**
  - **§9-j → moot:** ASR is **Heimdal-owned** (Heimdal owns all transcription; no competing Mimer instance). The `run_asr`/model-cache reuse is now an internal Heimdal implementation detail, not a cross-constituent shared substrate.
  - **§5.1 discriminator → demoted:** records-of-reality vs authored-content is no longer an *ownership* boundary; it becomes an event **typing** (a voice memo and a YouTube transcript both flow through Heimdal, carrying different `modality`/`sensitivity`/scope). §5.2's "separate backbones" collapses toward *one Heimdal acquisition backbone*, with Mimer downstream.
  - **KAP (spec #2786):** its acquisition front-end (source plugins, download, ASR) moves from Mimer to Heimdal; KAP's residual cognition/candidate-refinement stays in Mimer. Touches RESEARCH-08 and the constituent definitions in ADR-0043/0044 — a genuine `reshape`, to be enacted via an ADR.
- **OPEN sub-question (b) — watch-as-selection (owner UX inputs captured 2026-07-05; principle now resolved, ownership + detailed UX in a follow-on thread):** does Heimdal own *watch-as-selection* (what is worth monitoring / pulling in), or only fetch+transcribe once something is designated? **Principle resolved (2026-07-05, see input 3): "Markdown holds the record, the UI is the lens"** — the control surface is a markdown settings note in the vault; UI is a lens. The remaining ownership question + the full UI/UX are moving to a fresh Claude Design brief on Heimdal's UI/UX dimension. Captured owner UX inputs:
  ...
  - **Emerging boundary read (advisory, not final):** because selection here is genuinely *cognitive* (inferring interests, relevance-filtering under quality gates), it leans **Mimer / the agent layer**, with Heimdal doing fetch+transcribe on designated targets. A clean loop falls out: **Heimdal senses behaviour → Mimer infers interests → designates sources/items → Heimdal fetches+transcribes → Mimer makes meaning.** To be confirmed after the Claude Design pass (input 3) and the staged attention-calibration work (input 5).

### Decision run — owner rulings (2026-07-05)

Batched build-gating rulings. Three confirmations + one reshape.

1. **Entity/identity register → Mimer-owned, markdown-built `[reshape → CES/ADR]`.** *Reverses D-IDENTITY / charter FIXED #6* (which fixed the register as a shared Layer-2 substrate owned by no constituent). Owner ruling: **the register belongs to Mimer (the system-of-record); Heimdal integrates with it, not owns it.** Source of record is **`.md` files in the vault** (markdown-first, companion-note style); a **graph database is a derived index** built *from* those notes to improve relationship functionality — never the canonical store. This is a genuine reshape of a FIXED constraint, so it routes through CES/ADR at enactment (same ADR as §9-k).
   - **v1 mechanic (interpretation — owner to confirm):** with the register on Mimer's side, **Heimdal emits entity _mentions_** (surface forms + provisional local refs) in its events; **canonical _resolution_ (which Anna, which Northvolt) is Mimer's** — identity is knowledge, and knowledge is Mimer. This refines the §9-k handoff (resolution moves from Heimdal to Mimer's side of the seam) and **de-risks v1**: Heimdal neither builds nor blocks on a register. Supersedes the companion's earlier "Entity Register v0 before the first event" as Heimdal's biggest trade-off — that trade-off now belongs to Mimer, on its own schedule.
...
3. **YouTube seam → Heimdal owns download + transcribe + attribute(mentions); Mimer owns extract-meaning → candidate.** Confirms §9-k for the concrete YouTube pipeline. **Watch-as-selection stays deferred** (UX-gated, §9-k(b) + input 3 above) — the *fetch+transcribe* front moves to Heimdal now; *what is worth watching* waits for the Claude Design pass.
...
| F1 | HIGH | The seam minimizes the wrong asset: v1 `content` is the full inline transcript, so the life-content/entity-graph/timing all cross the seam; HEIM-4 guards the audio codec, not the secret; CrossScopeFlow has no volume/aggregate dimension (one grant is a firehose); revision events can leak previously withheld spans via diffing. | Owner-accepted residual OR hardening: declare published `content` itself Heimdal-grade; add aggregate/expiry budgets to capture-scope flows; revisions must re-apply the strictest current degradation. Surfaced as owner decision §9-i. |
...
| 2a | **Content quarantine (projector)** — observed content framed untrusted, fence-neutralized, excluded from any auto-executing prompt path (red-team F2) | **delivered (#3040)** | Closed the F2 blocker. Enforces HEIM-9 at the projector seam (`app.heimdal.projector` + `app.heimdal.quarantine`); observed content enters only as fenced `requires_review` evidence-candidates. |
| 3 | **Event contract schemas** — `heimdal.observation.published.v1` (+ consent-granted/revoked, corrected topics) | build-now (published topic) / contract-stub (corrected, revoked runtime) | This doc is the prose mirror; schemas become canonical at enactment. |
...
| 5 | **Voice-memo capture adapter** — watch the iCloud Shortcut folder, admit under grant, write raw record (encrypted at rest, `content_identity`, `capture_chain`, KERNEL-06 stamping), delete-after-confirmed-ingest | **delivered (#3025)** | The only component touching the source. `app.heimdal.capture_adapter` + `app.heimdal.raw_store` (migration `d5a8e2f1b6c3`); see `docs/EVENTS.md :: Heimdal raw-evidence store + voice-memo capture adapter`. Runtime driver (`app.heimdal.capture_runtime`, `app heimdal capture-watch` CLI, #3094) and a supervised `heimdal-capture-watch` Compose service (#3098) complete the vertical from adapter code to a running process. Prod go-live checklist (one-time, #3098): the operator sets `HEIMDAL_CAPTURE_WATCH_DIR` (the watched folder's host path — differs per device), `HEIMDAL_RAW_STORE_KEY` (a 64-char hex encryption key, generate once with `python3 -c "import secrets; print(secrets.token_hex(32))"` — never rotate without a migration plan, since rotating loses access to previously-encrypted raw records), and optionally `HEIMDAL_CAPTURE_INTERVAL_SECONDS` (poll interval, default 30s) in the host's runtime env file, mirroring the existing `HEIMDAL_RAW_READ_ALLOWLIST` provisioning note in `docs/EVENTS.md`. |
...
| 9 | **Publish** — assemble payload (§1.3), validate, insert with revision-aware idempotency key | **delivered (#3030)** | `app.heimdal.publish.assemble_observation_payload` / `publish_full_observation`; validates against the A4 schema (`app.events.topic_schema_registry.validate_topic_payload`) before insert, stamps `provenance.content_hash` in the same durable write (KERNEL-06), and folds `stage_versions`/`revision_of`/`supersedes` into a revision-aware idempotency key (A2 `derive_idempotency_key` discipline) so a re-processed revision never collides with or duplicates the prior observation. |
| 10 | **Mimer projector** — cursor consumer; projects events into bundle-carrying candidate artifacts (`requires_review: true`, noncanonical, capture scope, provenance chain) entering existing triage | **delivered (#3031, #3555)** | The read-model that proves the seam. `app.heimdal.candidate_projection.project_pending_candidates` — its own per-consumer cursor (`mimer.candidate_projector`, independent of A3's `mimer.content_quarantine_projector`), folds supersede/revision chains last-correction-wins, and requires a selected existing vault before processing. It advances that cursor only after every candidate in the batch is durable (`written` or a non-symlink regular file whose complete replay-safe candidate contract matches, including provenance, correction lineage, noncanonical posture, and the exact neutralized evidence section); a blocked write or an occupied/tampered path leaves the batch unread for safe retry. Candidate notes use the governed `write_note_relative`/`WriteGuard` path (mirrors `app.knowledge_acquisition.candidate_writeback`'s posture). The client contract's [Sources zone](../contracts/MIMER_CLIENT_CONTRACT.md#sources-zone--sensoracquisition-landing-zone) is the settings-named landing zone for published/transcribed sensor content notes; its concrete filesystem/settings enactment remains follow-on work. |
...
| Heimdal-owned log + cursors; outbox pattern generalized, outbox table untouched (§4) | `extend` — no outbox ownership change | transport swap, if ever → ADR |
| Voice memo = Heimdal capture; record-of-reality vs authored-content discriminator (§5.1) | `extend` — proposed boundary rule | owner ratification (§9-d) |
...
- `docs/HEIMDAL/CAPABILITY_CHARTER.md` (A3), `OWNER_DECISIONS.md` (A4), `ECOSYSTEM_SOS_MODEL.md` (A1), `FABLE_WINDOW.md` (A5)
