...
- [ ] **MVR-06B:** Same-channel tombstone reactivation advances the binding's content epoch and
  invalidates every object/file/vector/index/retrieval/episode/standing-question/cache and other
  binding-keyed derived projection introduced by MVR-05 before a production rescan and durable rebuild
  receipt may restore healthy reads or effects. Failure leaves the binding reactivating/degraded and
  sealed; historical receipts retain the retired epoch and cannot alias rebuilt rows.
  - Verify: `tests/integration/test_multi_vault_background_lifecycle.py::test_tombstone_reactivation_rebuilds_all_derived_state_before_healthy`
