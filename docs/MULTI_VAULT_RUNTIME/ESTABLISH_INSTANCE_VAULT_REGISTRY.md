...
- [ ] **MVR-01B:** Until 01C installs the authenticated selected-root-only rollback gateway and
  roll-forward lineage, every previous-image startup remains blocked; no scalar-representable export
  permits the old picker, initializer, broad host mounts, or legacy mutation paths to start early.
  - Verify: `tests/integration/test_vault_registry_rollback.py::test_previous_image_startup_is_sealed_until_01c_gateway`
