...
- Add a typed `ContextSelectionStore` keyed by an opaque server-minted `context_selection_id`,
  bound to the existing Companion request-auth context (#2223: loopback-local or configured API
  key for non-loopback state changes), plus an explicit request override carrier. Arbitrary client
  correlation IDs and paths are never session identity, authentication, or authority. The
  production resolver snapshots one context per request without mutating process-global state and
  GOV re-authorizes every binding on every resolution.
- Add production Companion endpoints to create, replace, inspect, and clear a TTL-bound selection.
  Creation returns the server-minted ID; later mutation/read requires that ID plus the same
...
- [ ] Session selection uses a server-minted ID bound to the production request-auth context;
  arbitrary client correlation IDs cannot read or mutate another selection.
  - Verify: `tests/api/test_active_context_resolution.py::test_selection_id_is_server_minted_and_auth_bound`
...
- [ ] Cache/retrieval context cannot collide across two sessions with the same binding and
  generation but different principal, scope, or dimension/filter.
...
Create one child under #2143 after MVR-01/02. Use Sol/xhigh: request concurrency, cache isolation,
and authority enforcement have high blast radius. Reuse #2356's type; do not create a rival seam.
