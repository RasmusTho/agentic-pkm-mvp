...
- [ ] **MVR-05C:** When the source is MVR-05B/#3163's durable compatibility binding, transfer invokes
  ...
  authorized replacement or durable `no_lifecycle` outcome; it never retargets compatibility state
  implicitly. Fault recovery exposes neither a tombstoned compatibility binding nor a destination
  lifecycle that was not explicitly selected.
