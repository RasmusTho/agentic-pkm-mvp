...
- Preserve and repair #3163 during the MVR-05→MVR-06 transition: before 05B can activate the scoped
  ...
  interim MVR-05A scalar worker is an explicit participant in that same revision: it closes A-bound
  claim acquisition, lets already-authorized A effects settle through acknowledgement/receipt,
  persists its drained revision, and acknowledges prepare before selection/default/compatibility may
  commit B. A crash or expired claim recovers against the durable A revision and cannot be silently
  redispatched or acknowledged under B; post-commit recovery rolls forward before B claims open.
...
- At every production filesystem, settings, projection, cache-fill, and retrieval read call site,
  ...
  locator and resolve its nearest enclosing initialized vault boundary before I/O. For an initialized
  binding, that boundary must be the exact root of the declared target. A provisional `uninitialized`
  binding instead permits read-only access inside its canonical registered root when no initialized
  boundary encloses the requested locator; it still holds the same ownership/revision/auth/effect
  fences and can never write. If an initialized nested boundary is encountered beneath that
  provisional root, the request fails closed unless that child is separately selected and authorized.
  A parent binding cannot read through an initialized
...
| MVR-05A5 | [#4579](https://github.com/RasmusTho/agentic-pkm-mvp/issues/4579) | replay projection group and the binding-scoped rebuild paths | 05A4 | delivered |
...
episode, and decision rebuilds to one binding, and fails startup migration before mutation when a
legacy row cannot be attributed uniquely. The no-vault/single-vault floor continues to use the
explicit `legacy-compatibility-binding` sentinel; active-context routing remains owned by MVR-05B.
...
- Owner-doc impact: each owning child updates in its PR: 05A `docs/DB_SCHEMA.md`, the durable envelope
  ...
  `docs/ARCHITECTURE.md`, `docs/SETTINGS.md`, and
  `companion-ui/docs/UI_RUNTIME_BOUNDARIES.md`; 05C
...
- [ ] **MVR-05B:** A provisional uninitialized binding preserves the production read-before-initialize
  journey within its registered root while remaining write-sealed; an initialized nested child is a
  hard boundary and cannot be read through the provisional parent.
  - Verify: `tests/integration/test_multi_vault_nested_effect_boundary.py::test_provisional_uninitialized_parent_reads_stop_at_initialized_child`
...
- [ ] **MVR-05C:** When the source is MVR-05B/#3163's durable compatibility binding, transfer invokes
  its protected `settings_rebind.v1` journal hook before source retirement and records an explicit
  authorized source replacement or, when none exists, preserves an enabled watcher as durable
  idle/no-vault. `no_lifecycle` is retained only when configuration already declared the watcher
  disabled or omitted; transfer cannot manufacture that posture to skip later acknowledgements. It
  never retargets compatibility state implicitly. Fault recovery exposes neither a tombstoned
  compatibility binding nor a destination lifecycle that was not explicitly selected.
  - Verify: `tests/integration/test_multi_vault_channel_transfer_lifecycle.py::test_transfer_repairs_compatibility_binding_before_source_retirement`
  - Verify: `tests/integration/test_multi_vault_channel_transfer_lifecycle.py::test_transfer_preserves_enabled_watcher_as_idle_without_replacement`
...
- [ ] **MVR-05B:** Picker replacement and the #3163 compatibility watcher rebind are recoverable as
  ...
  - Verify: `tests/integration/test_multi_vault_picker_context.py::test_picker_rebind_drains_scalar_worker_before_binding_commit`
...
- [ ] **MVR-05B:** The Companion runtime-boundary owner describes the shipped authenticated scoped-read
  route, explicit selection-header forwarding, and fail-closed no-downgrade behavior through the
  single-origin proxy in the same PR.
  - Verify: doc writeback at `companion-ui/docs/UI_RUNTIME_BOUNDARIES.md :: Single-origin API proxy`
...
- Multi-binding watcher/worker lifecycle supervision (MVR-06), except MVR-05C's bounded
  source-channel transfer inventory/drain/stop/restart fence and `settings_rebind.v1` repair; the new
  visual switcher/overlay behavior owned by #2566, registry/default/dimension storage, or removing
...
- Verify the 05B PR diff contains its mapped `docs/ARCHITECTURE.md`, `docs/SETTINGS.md`, and
  `companion-ui/docs/UI_RUNTIME_BOUNDARIES.md` writebacks.
