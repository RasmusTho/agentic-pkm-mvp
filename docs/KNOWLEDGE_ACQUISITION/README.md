State: Specification directory (target-state framing; contracts filed via PR #2786). **Phase 2 issue set filed 2026-07-02**: parent feature issue #2795, children #2796–#2801. KA-01 (#2796) delivered via PR #2928; KA-02 (#2797) delivered via PR #2931 (captionless ASR fallback); KA-03 (#2798) delivered (deterministic normalize stage); KA-04 (#2799) delivered (open extraction registry + `summary` extractor); KA-05 (#2800) delivered (candidate assembly + governed `youtube_source_note` writeback); KA-06 (#2801) delivered (stage events on the DB outbox with deterministic idempotency keys, item-scoped dead-letters, and the `acquire-replay` no-source-egress replay command) — all six children delivered; parent #2795 closure pending its acceptance receipts. See `PARENT_FEATURE_ISSUE.md`. Fabric class decision resolved via #2794 (Acquisition source, class 11). **#3106 delivered** (acquisition entrypoint, `app/knowledge_acquisition/acquire.py` + `python -m app.cli acquire-youtube <url>`): the missing production front door — resolves the `youtube_url` plugin, calls `fetch` (real egress), persists the raw record, then runs `normalize → extractors → assemble_candidate → write_candidate_note`, emitting one KA-06 stage event per transition; idempotent on `content_identity`; fails loud without a configured `DATABASE_URL`/`DB_DSN`. **Onboarding bootstrap added**: `app/knowledge_acquisition/youtube_onboarding.py` and `python -m app.cli youtube-onboard <takeout-root> --output <registry.json>` parse and deduplicate Google Takeout subscriptions and playlist exports into source metadata only; no YouTube egress or vault writes occur during bootstrap.
