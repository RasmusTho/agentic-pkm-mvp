State: Partially implemented capability contract. The required `fetch(item_ref)` operation and its `content_identity`/dedup semantics are implemented for the `youtube_url` plugin (KA-01, #2796, `app/knowledge_acquisition/`); `discover` is implemented only for the one configured YouTube Inbox playlist by YSS-05 (#3920, `app/knowledge_acquisition/playlist_discovery.py`). `backfill`, subscription discovery, multi-playlist product sync, and other source instances remain not implemented.
