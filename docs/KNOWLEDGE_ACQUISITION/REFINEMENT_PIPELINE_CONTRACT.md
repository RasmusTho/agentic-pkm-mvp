State: Specification (docs-authoring; target-state framing). The `raw` level and the deterministic `normalize` stage (rolling-cue dedup, detected language, `acquisition_method` propagation, quality note) are implemented (KA-01 `#2796`, KA-03 `#2798`, `app/knowledge_acquisition/`); `extracted`, `candidate`, stage events/idempotency, and replay remain not implemented.
