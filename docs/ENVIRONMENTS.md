...
- **`VAULT_ROOT` runtime binding** — the env/runtime binding that points the running process at a content vault at boot (resolved by `resolve_vault_root` / `resolve_optional_vault_root` in `app/config/paths.py`).
  - *Invariants*: this binding is the boot-time *pointer*, not the data. An **unset/unbound** `VAULT_ROOT` resolves to an explicit no-vault state (`None`) — no silent fallback to a CWD-relative `./vault`. A **set-but-missing** `VAULT_ROOT` (path configured but absent) still fails loud with `VaultRootMisconfiguredError`; missing data is an error, not a no-vault state. A **bound, existing** root resolves to that path, environment-scoped when requested. Per-environment overrides (`VAULT_ROOT_DEV`, `VAULT_ROOT_TEST`) and a CLI override take precedence over the base binding.
