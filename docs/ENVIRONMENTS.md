...
- **Vault-settings** — the per-vault settings/config surface that lives *inside* a content vault at the single canonical root `<vault>/settings/` (see `app/settings/locations.py` and `docs/SETTINGS.md`). Retired locations are one-release compatibility inputs only: reads warn loudly, canonical files shadow matching legacy files, and values are never merged. `docs/SETTINGS.md` owns the mechanism and resolution rules; this document owns the four vault senses and their environment invariants.
