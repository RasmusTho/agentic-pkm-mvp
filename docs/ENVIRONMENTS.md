...
- The `test` compose override sets `PKM_ENVIRONMENT: test` (first-class as of Issue #848 / PR #940), gives the one-shot migration service the in-container `app_test` DSN, and binds runtime services to the existing mock provider. The universal test overlay remains vault-neutral for the no-vault idle/picker posture. Only an explicit test-vault startup adds `docker-compose.legacy-vault.yml`; that overlay both mounts the selected host vault at `/app/vault` and sets container-facing `VAULT_ROOT` / `VAULT_ROOT_TEST` to the same path. Test-channel isolation comes from that matching scoped DB, explicit scoped vault (`vault-test/` or operator-selected `VAULT_ROOT_TEST`), scoped artifact paths (`tmp-test/`), and port separation.
