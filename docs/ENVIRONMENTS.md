...
- The `test` compose override sets `PKM_ENVIRONMENT: test` (first-class as of Issue #848 / PR #940); it gives the one-shot migration service the in-container `app_test` DSN, binds runtime services to the existing mock provider, and passes the mounted test vault as `/app/vault` through `VAULT_ROOT` and `VAULT_ROOT_TEST`. Test-channel isolation comes from that matching scoped DB, scoped vault (`vault-test/` or operator-selected `VAULT_ROOT_TEST`), scoped artifact paths (`tmp-test/`), and port separation.
