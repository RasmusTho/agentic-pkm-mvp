...
Notes:
- `make start-test-system` and `make test-bootstrap` remain the canonical **local** `test` verification workflow (resettable UAT). `start-test-system` now leaves `VAULT_ROOT` empty unless the caller explicitly supplies `TEST_VAULT_ROOT` or `VAULT_ROOT_TEST`, so a fresh checkout boots the no-vault idle posture (#2005 — the in-app picker, no exit) instead of forcing the repo-local `vault-test` scratch vault. An explicit missing path still fails loud. `make test-bootstrap` and `make test-up` keep the repo-local `vault-test` fallback because they seed/start the test vault. The companion-UI launcher itself (`make test-ui`, via `companion_ui_startup.sh`) idles without a vault regardless. This is distinct from `make test-start-full VAULT_ROOT=<path>`, the canonical **promotion-verification** test startup, which deliberately requires an explicit vault — see the startup-command table in `docs/runbooks/RUNBOOK_STARTUP_FULL_SYSTEM.md`.
