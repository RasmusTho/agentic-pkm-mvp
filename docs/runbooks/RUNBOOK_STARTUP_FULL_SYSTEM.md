...
## Remote deployment boundary (verified 2026-08-22)

This runbook's `make` and Compose commands operate the local fallback runtime. They do not deploy or
verify a remote deployment target. The dedicated model-service target is separate from Product Runtime.
Do not use legacy model-service stacks, or a local Compose health check, as evidence for promotion to a
profile-defined runtime target. Promotion requires the exact-target deployment handoff and a durable
test verification receipt. See the selected profile, currently
[`docs/deployment/profiles/TARS_PROXMOX.md`](../deployment/profiles/TARS_PROXMOX.md), for placement.
...
| `make test-start-full VAULT_ROOT=<path>` | test | `docker-compose.yaml:docker-compose.test.yml` | `pkm-test` | `test` | Yes (by design) | **Canonical promotion-verification test startup.** All four bindings explicit; the explicit `VAULT_ROOT` is a deliberate guard so staged promotion never verifies against the wrong/absent vault (see `promote-to-test` skill). For *local* resettable test iteration use `make start-test-system` instead (no-vault-idle-capable). |
...
`make dev-start-full` and `make prod-start-full` also run the BuilderOps/dispatcher startup
bootstrap before Compose services start. That bootstrap is operational coordination only: it can
initialize local dispatcher/BuilderOps SQLite state and optionally pull the GitHub issue queue when
