...
2. Want to watch the inbox? rerun `START_WATCHERS=1 START_WORKER=0 scripts/start_full_system.sh`, which now also rotates `WATCHER_TICK_LOG_PATH` (default `/app/tmp/watcher_tick-<timestamp>.jsonl` and mirrored in `tmp/latest_watcher_tick_log`) and prints the path plus a `docker compose exec watcher` tail command so you can inspect `scanned_files`, `hashed_files`, and `bytes_read` for each tick.
