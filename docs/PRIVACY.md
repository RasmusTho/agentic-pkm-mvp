State: SoT v4.10 Reality-MVP (current, local-first).
# Privacy & PII
...
## Local execution
- Ingestion, transcription, and ASK run entirely locally by default (`LLM_PROVIDER=mock` or Ollama on the same machine).
- The only outbound calls are to Ollama/OpenAI/DeepSeek when explicitly enabled via env. No implicit cloud storage in Reality-MVP.
...
- Reality-MVP assumes local execution; no cloud record exists unless remote LLM providers are enabled. If a cloud deployment is ever introduced, execute a data processing agreement and scrub logs of PII before upload.
<!-- SECTION:PRIVACY:END -->
