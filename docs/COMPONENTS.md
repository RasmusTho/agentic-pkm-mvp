...
- **ReasoningFacade** — Shared reasoning/tool entrypoint for LangGraph agents. Maturity: Experimental. Code present (`app/reasoning/facade.py`, panel/canvas/chat paths) but opt-in and roadmap-gated off; not part of the locked v5.5 baseline.
- **BaseLangGraphAgent** — Common agent scaffolding for LangGraph inner loops. Maturity: Experimental. Code present but opt-in and roadmap-gated off; not part of the locked v5.5 baseline.
