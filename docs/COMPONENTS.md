...
- `docs/SYSTEM_OF_SYSTEMS_ARCHITECTURE.md` — kernel/extension-fabric spine and the eight-subsystem map.
...
- **ReasoningFacade** — Tool-facing reasoning entrypoint for the panel/pilot LangGraph paths (`reason` / `chat` / `structured` / `tool_use`). Maturity: Experimental. Code present (`app/components/reasoning/facade.py`, imported by the panel/pilot agents) but opt-in and roadmap-gated off; not part of the locked v5.5 baseline. (The separate `app/reasoning/facade.py` is the canvas/chat reasoning facade — claims/evidence/rank/answer/plan — not this tool-use facade.)
- **LangGraph agent scaffolding** — Common state/graph scaffolding for LangGraph inner loops: `AgentStateBase` and `build_agent_graph` in `app/components/reasoning/graph_builder.py`. Maturity: Experimental. Code present but opt-in and roadmap-gated off; not part of the locked v5.5 baseline.
