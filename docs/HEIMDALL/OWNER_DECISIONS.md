State: Draft (advisory groundwork, 2026-07-04). The owner-reserved decision list for Heimdall: what Fable may NOT decide, plus the owner decisions captured in the 2026-07-04 session. Advisory until enacted through CES/ADR; creates no runtime behavior and no GitHub work.
Doc role: Owner-decision register (Draft)
Authority: Authoritative for which Heimdall decisions are reserved to the owner and for the recorded state of each captured decision. Subordinate to the ADRs/contracts that later enact any of these. Claims no shipped reality.
...
| **D-NAME-SENSOR** | Sensor name vs. `Heimdall`/observability collision | **Sensor keeps `Heimdall`;** observability alias reverts to boundary code `OEF` (one glossary edit, no shipped code) | ADR-0043 |
| **D-ADR41-TIMING** | Bundle Heimdall naming with ADR-0041 enactment? | **No — separate.** Heimdall naming is ADR-0043; ADR-0041/0042 enactment (#2855/#2856) stays independent | ADR-0043 |
...
| **D-BACKBONE** | Heimdall vs. KAP backbone | **Left OPEN for Fable.** Fixed guardrail: a shared provenance standard. Stream-vs-batch backbone is Fable's design | Charter OPEN #5 |
...
- **`Munin` reassignment** (`reshape`): `Munin`'s former glossary role ("raw-media / raw-memory
  module") is absorbed by Heimdall's raw observation layer; `Munin` now denotes the whole
