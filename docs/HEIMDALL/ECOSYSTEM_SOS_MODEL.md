State: Draft (advisory groundwork, 2026-07-04). Proposes an acknowledged System-of-Systems model for the personal agentic ecosystem, with Heimdall as a sibling constituent. This is a `reshape` relative to the current single-system framing (ADR-0041) and is advisory until enacted through CES/ADR. It creates no runtime behavior and no GitHub work.
...
# Heimdall A1 — Ecosystem System-of-Systems model

## Purpose

Establish the architectural container that a new capability — **Heimdall** (continuous observation of
reality → attributed, timestamped events with confidence and provenance) — plugs into. This doc fixes
*where Heimdall sits* so that the capability charter (`CAPABILITY_CHARTER.md`) can hand Fable a bounded
...
- **Heimdall** — the sensor / event-capture constituent (observation → attributed event stream).

Heimdall is a **sibling constituent, not a subsystem of Munin**. `[reshape → CES/ADR]` The reasons
are load-bearing and were fixed before this doc:

- **Dependency direction.** Every other constituent *consumes* Heimdall's event stream. A thing that
  everyone reads from, and that reads from no one, is a peer source, not a submodule of one reader.
- **Public/private seam.** Heimdall carries the most sensitive private data in the ecosystem (raw
...
- **Ends at a published event.** Heimdall's responsibility ends at a *published, attributed event*,
...
ecosystem itself to an acknowledged SoS with Yggdrasil-as-the-whole-and-Munin/Hugin/Heimdall-as-
...
Heimdall owns an **append-only fact stream**: the canonical, immutable record of "what was observed,
...
until promoted. Heimdall extends that model *upstream* to the point of observation. `[extend]`
...
| **3. Constituents** | Munin, Hugin, Heimdall — the systems that do the work | Each constituent owns its own internals | The constituent's own backlog |
...
1. **Independent release cadence** — Heimdall must ship/roll back on a clock that fights Munin's.
...
Heimdall makes explicit a set of mechanisms that are today implicitly *Munin-internal* (or unbuilt)
...
|---|---|---|---|
| **Event bus** | No cross-constituent bus; the DB outbox (`docs/EVENTS.md`) is Munin-internal | A shared publish/subscribe seam Heimdall publishes to and others subscribe to. **Open design (Fable):** whether this generalizes the existing outbox or is a new stream-native bus | `extend` (generalize outbox) or `reshape` if it changes outbox ownership |
| **Identity / entity register** | Implicit; entities resolved per-surface | **Shared canonical register** (owner decision, §6) so "Rasmus"/"Anna" mean the same entity across Heimdall and Munin | `extend` |
...
| **Provenance / replay primitives** | KAP defines source-plugin + refinement + provenance/replay (`docs/KNOWLEDGE_ACQUISITION/`) | Candidate Layer-2 standard both KAP and Heimdall conform to. **Open design (Fable):** shared backbone vs. Heimdall-native (owner left this to Fable, §6) | `extend` |
...
- **Naming** — Yggdrasil = whole; Munin = knowledge/memory constituent; Hugin = agent-runtime;
  Heimdall = sensor (observability alias → `OEF`). Recorded in ADR-0043. `[reshape → ADR-0043]`
...
- **Heimdall vs. KAP backbone** — **left open for Fable**; the fixed guardrail is a shared provenance
...
|---|---|---|
| Ecosystem is an acknowledged SoS; Yggdrasil = whole; constituents = Munin/Hugin/Heimdall | `reshape` | Own ADR + CES (not minted here) |
| Heimdall is a sibling constituent, not a Munin subsystem | `reshape` | Same ADR |
...
| Heimdall-vs-KAP backbone | `open` | Fable design within guardrail |

## References

- ADR-0043 — Heimdall naming + Norse name register.
