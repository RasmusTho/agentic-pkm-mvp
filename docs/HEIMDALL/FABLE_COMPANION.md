...
What is concretely shared anyway: the **ASR stage implementation**. KA-02's local-ASR fallback and Heimdall's transcription stage are the same mechanism (local Whisper, segment output, confidence emission); build it once as a shared stage component both pipelines invoke with their own posture wrappers. Mechanism sharing without pipeline merging.
