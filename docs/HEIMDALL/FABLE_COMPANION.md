...
- **OPEN sub-question (b) — watch-as-selection (owner UX inputs captured 2026-07-05; not yet decided):** does Heimdall own *watch-as-selection* (what is worth monitoring / pulling in), or only fetch+transcribe once something is designated? Captured owner UX inputs:
  ...
  3. **Control surface:** **routed to a Claude Design exploration** (owner has no answer yet — deliberately a design question). Brief: `WATCH_SELECTION_DESIGN_BRIEF.md`, whose central axis is the owner's *markdown-first (everything doable in Obsidian as `.md`, UI a complement) vs always-our-UI* principle question.
