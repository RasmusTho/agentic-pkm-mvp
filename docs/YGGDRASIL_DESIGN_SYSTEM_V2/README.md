...
| `companion-ui/companion-app/yggdrasil-tokens.css` | Tokens only: the same tokens, themes, density, opt-in rules, and reduced motion, without v1 element defaults. For surfaces that own their base styles (the Companion workspace pages, served at `/static/yggdrasil-tokens.css`). |
