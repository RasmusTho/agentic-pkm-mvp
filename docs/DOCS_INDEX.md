State: SoT v5.5 Reality-MVP baseline locked (watcher safety, panel action provenance, and concurrency guardrails); v5.6 delivery line closed with post-v5.6 follow-ups tracked separately for LangGraph/Reasoning expansion, Orchestrator V2 hardening, A2A/MCP lifecycle cleanup, and local verification defects while referencing `docs/STATUS.md#baseline-definition`.
Doc role: Core SoT
...
| docs/STATUS.md | Operational snapshot | Aligned (SoT v5.5 baseline locked + v5.6 delivered) | 2026-04-13 | Current snapshot of runtime reality, verification status, and post-v5.6 follow-up posture; now records issue/code truth that v5.6 is closed, Orchestrator V2 is flagged with checkpoint plumbing but no supported checkpoint/resume execution path, and runtime-health contract drift remains follow-up work. |
...
| docs/plans/V60_CAPABILITY_AND_AGENT_EVOLUTION.md | v6.0 capability and agent evolution plan | Plan | 2026-04-12 | Working plan for capability-based composition, Panel/Chat separation, staged Deep Agent rollout, bounded template-agent coexistence, governed mutation paths, and explicit v5.6 blocker/invariant handling; now narrows read-only Chat wording to the Deep Agent introduction slice per the recorded interaction-surface decision and remains compatible with the commitment-family spec. |
