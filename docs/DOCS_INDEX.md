...
| docs/STATUS.md | Operational snapshot | Aligned (SoT v5.5 baseline locked + v5.6 delivered) | 2026-04-30 | Current snapshot of runtime reality, verification status, and post-v5.6 follow-up posture; now records issue/code truth that v5.6 is closed, Orchestrator V2 is flagged with checkpoint/resume hardening, retry/backoff observability, per-tool timeout, and optional plan-timeout budgets shipped through #444/#445/#446/#540, local runtime/docs validation drift was fixed by #441 / PR #439, timeout discriminator cleanup shipped via #456 / PR #458, sync-latency measurement-mode hardening and retrieval salience/staleness diagnostics shipped via #570/#571/#572, minimal read-only orientation shipped via #576 / PR #579, bounded canvas-session scaffolding shipped through #598/#599/#600/#601 while owner-doc promotion remains open on #597, A2A lifecycle issue #359 is closed, v6-driven domain/zone current-state bugs #435/#436/#437 are closed, event/outbox consumer contracts are current, retrieval capability metadata plumbing exists, and delivery feedback loop governance is present. |
...
| docs/ROADMAP.md | Strategic roadmap | Aligned (SoT v5.5 baseline locked + v5.6 delivered) | 2026-04-30 | Roadmap treats v5.6 as a closed delivery line, keeps v5.5 as the locked baseline, separates post-v5.6 follow-ups from release blockers, distinguishes shipped Orchestrator V2 pilot behavior, checkpoint/resume hardening, retry/backoff observability, timeout/SLA, plan-timeout budget, timeout-discriminator cleanup, read-only Chat cognition scaffold, the shipped flagged remote MCP multiplex seam with deterministic local fallback, bounded canvas-session scaffolding pending owner-doc promotion on #597, retrieval capability metadata plumbing, and closed A2A parent lifecycle from remaining follow-ups, and keeps stale AMG wording replaced with the current three-surface persistence model, forward-looking canvas-Chat framing, and full Issue-contract guardrails. |
...
| docs/HUMAN-FLOWS.md | Human flows | Aligned (forward line v5.x + v6 planning) | 2026-04-30 | User-facing behavior contract for the current system; explicitly covers life spheres/contexts, role identities, overlap-first semantics with bounded cross-scope permissions, heterogeneous device roles, archive-brain use, artifact continuity beyond the current runtime, canonical human loops used to read runtime support paths, read-only Chat cognition as shipped scaffold, minimal orientation and commitment-runtime slices as bounded support seams rather than full cognitive rollout, bounded canvas-session scaffolding that is not yet promoted into the supported baseline, and the v6.0 interaction, finding/reorienting, persistence-surface, and commitment capability-spec reading anchors. |
...
| docs/INTERACTION_SURFACES_AND_AUTHORITY/README.md | v6.0 capability spec — Panel, Chat, Automation surfaces and authority boundaries | Aligned (v6.0 capability spec) | 2026-04-22 | Names the three interaction surfaces and their authority boundaries; resolves the Chat read-only-vs-canvas contradiction in favor of canvas-shaped Chat with governed mutation rights (decision recorded 2026-04-11 in `RECONCILE_CHAT_MUTATION_AUTHORITY.md`), defines Panel as primary command surface without making it the exclusive authoritative intent source, and adds the hybrid Chat integration schema. Current runtime has only a read-only Chat cognition scaffold; canvas co-editing and Chat mutation remain future work. Read before any Panel/Chat/Automation evolution. |
...
| docs/RELEASE_CHANNELS/README.md | v6.0 capability spec — release channels (stable prod alongside active dev) | Aligned (v6.0 capability spec, docs-only) | 2026-04-23 | Defines channel identity, per-channel logical Postgres DBs (`pkm_prod`/`pkm_dev`/`pkm_test`), promotion plan contract, migration reversibility classification, concurrency rule, and rollback posture. Extends the environment model in `docs/ENVIRONMENTS.md` with channel-level isolation so a stable build can run against the real vault while dev continues on the same machine. Read before any promotion/rollback or per-channel DB work. |
