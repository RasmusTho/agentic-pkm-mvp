...
| docs/STATUS.md | Operational snapshot | Aligned (SoT v5.5 baseline locked + v5.6 delivered) | 2026-04-29 | Current snapshot of runtime reality, verification status, and post-v5.6 follow-up posture; now records issue/code truth that v5.6 is closed, Orchestrator V2 is flagged with checkpoint/resume hardening, retry/backoff observability, per-tool timeout, and optional plan-timeout budgets shipped through #444/#445/#446/#540, local runtime/docs validation drift was fixed by #441 / PR #439, timeout discriminator cleanup shipped via #456 / PR #458, sync-latency measurement-mode hardening and retrieval salience/staleness diagnostics shipped via #570/#571/#572, minimal read-only orientation shipped via #576 / PR #579, A2A lifecycle issue #359 is closed, v6-driven domain/zone current-state bugs #435/#436/#437 are closed, read-only Chat cognition scaffold exists while canvas-Chat co-editing remains future work, event/outbox consumer contracts are current, retrieval capability metadata plumbing exists, and delivery feedback loop governance is present. |
...
| docs/ROADMAP.md | Strategic roadmap | Aligned (SoT v5.5 baseline locked + v5.6 delivered) | 2026-04-29 | Roadmap treats v5.6 as a closed delivery line, keeps v5.5 as the locked baseline, separates post-v5.6 follow-ups from release blockers, distinguishes shipped Orchestrator V2 pilot behavior, checkpoint/resume hardening, retry/backoff observability, timeout/SLA, plan-timeout budget, timeout-discriminator cleanup, read-only Chat cognition scaffold, the shipped flagged remote MCP multiplex seam with deterministic local fallback, retrieval capability metadata plumbing, closed v6 Priority 1-4 parent features, and open Priority 5 commitment-runtime follow-up #688, while keeping stale AMG wording replaced with the current three-surface persistence model, canvas-Chat future-surface framing, and full Issue-contract guardrails. |
