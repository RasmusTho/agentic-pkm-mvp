...
| docs/ARCHITECTURE.md | Architecture (SoT v5.x forward line on v5.5 base) | Aligned (forward line v5.x) | 2026-04-08 | Active runtime architecture source of truth; focused on the current runtime/data model, explicit about forward-line versus historical material, and now makes interaction primary on the user-facing side while keeping foundational capabilities and bounded agents first-class. |
...
| docs/STATUS.md | Operational snapshot | Aligned (SoT v5.5 baseline locked + forward line v5.6 planned) | 2026-04-08 | Current snapshot of runtime reality, verification status, and forward-line Now/Next/Later; includes the present-tense agent-evolution posture while keeping autonomous execution explicitly out of current truth. |
...
| docs/ROADMAP.md | Strategic roadmap | Aligned (SoT v5.5 baseline locked + forward line v5.6 planned) | 2026-04-08 | Forward line leans on the v5.5 baseline while v5.6 LangGraph/Reasoning stages are planned; capability-based architecture, bounded agent evolution, and governed mutation paths are framed as phased sequencing, with v5.6 treated primarily as an invariant layer rather than a rigid linear prerequisite chain. |
...
| docs/plans/V60_CAPABILITY_AND_AGENT_EVOLUTION.md | v6.0 capability and agent evolution plan | Plan | 2026-03-27 | Working plan for capability-based composition, Panel/Chat separation, staged Deep Agent rollout, bounded template-agent coexistence, governed mutation paths, and explicit v5.6 blocker/invariant handling; expands the roadmap without turning top-level docs into backlog surfaces. |
...
| docs/EVENTS.md | Outbox/event contracts | Aligned (forward line v5.x) | 2026-03-27 | Canonical Outbox envelope + selected event meanings; compatibility anchored in EVENT_COMPATIBILITY_CONTRACT, with clearer reminders that event flow is current runtime coordination rather than the full target-state architecture. |
...
| docs/HUMAN-FLOWS.md | Human flows | Aligned (forward line v5.x) | 2026-04-08 | User-facing behavior contract for the current system; now explicitly covers life spheres/contexts, role identities, overlap-first semantics with bounded cross-scope permissions, heterogeneous device roles, archive-brain use, artifact continuity beyond the current runtime, canonical human loops used to read runtime support paths, and the operator-facing enablement questions the current runtime must answer without reading code. |
...
| docs/OPERATIONS.md | Operations playbook | Aligned (forward line v5.x) | 2026-04-08 | Top-level operator entrypoint focused on current runtime checks, watcher operations, and escalation to HEALTH, OBSERVABILITY, INFRASTRUCTURE, and runbooks; now reflects that deterministic `make verify-runtime` is shipped while the broader local bootstrap contract continues through explicit follow-up slices. |
| docs/LOCAL_TEST_BOOTSTRAP/README.md | Local test bootstrap spec index | Reference | 2026-04-07 | Index for the local test bootstrap specification set and its task-document workflow. |
...
| docs/ORCHESTRATOR_A2A_ROUTING/README.md | Orchestrator A2A routing spec index | Reference | 2026-04-07 | Index for the orchestrator-managed A2A routing specification set and its feature-validation workflow. |
...
| docs/ORCHESTRATOR_A2A_ROUTING/EMIT_AGENT_CALL_RESPONSES_AND_FAILURES.md | A2A routing spec: emit routing receipts | Reference | 2026-04-07 | A2A-ROUTING-02 task spec for complete request/response/error receipt emission on routed agent calls. |
...
| docs/plans/LOCAL_TEST_ENVIRONMENT_BOOTSTRAP.md | Local test environment bootstrap | Plan | 2026-04-02 | Docs-first stabilization plan for the canonical local `test` bootstrap path, its golden path, acceptance criteria, relationship to existing blocker issues, and the recommended parent-feature / child-slice execution mapping. |
