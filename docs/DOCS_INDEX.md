...
| docs/CONCEPTS/COMPANION_NOTE_CONTRACT.md | Companion note contract (system identity surface per vault note) | Aligned (forward line v5.x) | 2026-03-27 | Bounded field set, path, ownership rules, attachment manifest semantics, and what the companion must NOT contain. Replaces VaultMirror. |
| docs/CONCEPTS/ARTIFACT_MODEL_AND_LIFECYCLES.md | Artefact model and lifecycles (three surfaces + UUID healing) | Aligned (forward line v5.x) | 2026-03-27 | Defines vault note / companion note / runtime DB surfaces, UUID healing priority order (7 steps), and authority matrix for all 6 healing scenarios. |
...
| docs/ARCHITECTURE.md | Architecture (SoT v5.x forward line on v5.5 base) | Aligned (forward line v5.x) | 2026-03-14 | Active runtime architecture source of truth; focused on the current runtime/data model and explicit about forward-line versus historical material. |
...
| docs/plans/COMPANION_NOTE_AND_NOTE_CONTEXT.md | Companion note + Note Context implementation plan (8 parts) | Planned / in progress (Parts 1–3 done) | 2026-03-27 | Replaces VaultMirror with flat companion notes; introduces NoteContext assembler for agents. Parts 1–3 shipped; Parts 4–8 pending. |
...
| docs/HUMAN-FLOWS.md | Human flows | Aligned (forward line v5.x) | 2026-03-14 | User-facing behavior contract for the current system; keeps human surfaces and acceptance expectations separate from operational commands. |
...
| .codex/AGENTS.md | Coding agent guidance | Aligned (forward line v5.x, with known debt) | 2026-03-14 | Dev-layer system prompt and repo-specific docs workflow; requires owner-first reading and anti-dilution updates for spec docs. |
