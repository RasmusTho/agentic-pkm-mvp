...
| docs/DEV_WORKFLOW.md | Developer workflow | Aligned (forward line v5.x) | 2026-04-02 | Compatibility pointer to `docs/development/DEV_WORKFLOW.md`; now also mirrors the lightweight planning chain from docs and capability intent down to slices, PRs, verification, and acceptance. |
| docs/development/DEV_WORKFLOW.md | Development workflow | Aligned (forward line v5.x) | 2026-04-02 | Development reference for builder-agent working loop, the lightweight docs -> capability -> slice -> PR planning chain, and validation/acceptance expectations; not an auto-loaded instruction file. |
...
| docs/plans/LOCAL_TEST_ENVIRONMENT_BOOTSTRAP.md | Local test environment bootstrap | Plan | 2026-04-02 | Docs-first stabilization plan for the canonical local `test` bootstrap path, its golden path, acceptance criteria, and relationship to existing blocker issues. |
