...
| docs/STATUS.md | Operational snapshot | Aligned (SoT v5.5 baseline locked + v5.6 delivered) | 2026-04-17 | Current snapshot of runtime reality, verification status, and post-v5.6 follow-up posture; now records issue/code truth that v5.6 is closed, Orchestrator V2 is flagged with checkpoint/resume hardening and retry/backoff observability shipped through #444/#445, local runtime/docs validation drift was fixed by #441 / PR #439, the bounded timeout/SLA contract shipped via #446 / PR #448, timeout discriminator cleanup shipped via #456 / PR #458, A2A lifecycle issue #359 is closed, and v6-driven domain/zone current-state bugs #435/#436/#437 are closed. |
...
| docs/ROADMAP.md | Strategic roadmap | Aligned (SoT v5.5 baseline locked + v5.6 delivered) | 2026-04-17 | Roadmap treats v5.6 as a closed delivery line, keeps v5.5 as the locked baseline, separates post-v5.6 follow-ups from release blockers, distinguishes shipped Orchestrator V2 pilot behavior, checkpoint/resume hardening, retry/backoff observability, timeout/SLA, and timeout-discriminator cleanup from remaining follow-ups, and keeps stale AMG wording replaced with the current three-surface persistence model. |
...
| docs/ORCHESTRATOR_A2A_ROUTING/README.md | Orchestrator A2A routing spec index | Reference | 2026-04-09 | Index for the orchestrator-managed A2A routing specification set and its feature-validation workflow; tasks 1-3 are delivered in GitHub/code, while parent issue lifecycle/project closure appears stale and should be reconciled separately. |
