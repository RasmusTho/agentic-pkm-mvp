...
| docs/runbooks/UAT_PANEL_WATCHER.md | UAT guide (panel + watcher) | Aligned (forward line v5.x) | 2026-03-27 | Registry watcher UAT flow with AI fence policy and UUID healing; explicitly scoped as operational validation of the current runtime path, not the full target-state design. |
...
| docs/runbooks/RUNBOOK_STARTUP_FULL_SYSTEM.md | Runbook: full-system startup | Aligned (forward line v5.x) | 2026-03-27 | Canonical v5.x startup; includes strict Obsidian gate (`STARTUP_ENFORCE_OBSIDIAN`), health boundary checks, and startup-status telemetry fields, while explicitly framing startup wiring as current operational reality rather than the final architecture model. |
...
| docs/plans/COMPANION_NOTE_AND_AGENT_CONTEXT_PLAN.md | Companion note and agent context plan | Plan | 2026-03-28 | Implementation-ready plan for companion note service, note context, and agent-context enrichment. |
