...
| docs/ARCHITECTURE.md | Architecture (SoT v5.x line on v5.5 base) | Aligned (forward line v5.x) | 2026-04-22 | Active runtime architecture source of truth; carries an executive summary and current-vs-planned status map that treats v5.6 as delivered while separating active v5.5 runtime, flagged pilots/scaffolding, delivered V2 checkpoint/retry/per-tool and plan-timeout follow-ups, retrieval capability metadata plumbing, read-only Chat cognition scaffold, closed A2A lifecycle cleanup, remaining post-v5.6 follow-ups, v6 target-state direction, current event envelope/outbox consumer contracts, and the current full GitHub Issue contract without reviving historical AMG/SetDB or module-map claims. |
...
| docs/STATUS.md | Operational snapshot | Aligned (SoT v5.5 baseline locked + v5.6 delivered) | 2026-04-22 | Current snapshot of runtime reality, verification status, and post-v5.6 follow-up posture; now records issue/code truth that v5.6 is closed, Orchestrator V2 is flagged with checkpoint/resume hardening, retry/backoff observability, per-tool timeout, and optional plan-timeout budgets shipped through #444/#445/#446/#540, local runtime/docs validation drift was fixed by #441 / PR #439, timeout discriminator cleanup shipped via #456 / PR #458, A2A lifecycle issue #359 is closed, v6-driven domain/zone current-state bugs #435/#436/#437 are closed, read-only Chat cognition scaffold exists while canvas-Chat co-editing remains future work, event/outbox consumer contracts are current, retrieval capability metadata plumbing exists, and delivery feedback loop governance is present. |
...
| docs/HUMAN-FLOWS.md | Human flows | Aligned (forward line v5.x + v6 planning) | 2026-04-22 | User-facing behavior contract for the current system; explicitly covers life spheres/contexts, role identities, overlap-first semantics with bounded cross-scope permissions, heterogeneous device roles, archive-brain use, artifact continuity beyond the current runtime, canonical human loops used to read runtime support paths, read-only Chat cognition as shipped scaffold only, canvas-Chat as a future direct co-editing surface, and the v6.0 interaction, finding/reorienting, persistence-surface, and commitment capability-spec reading anchors. |
...
| docs/OPERATIONS.md | Operations playbook | Aligned (forward line v5.x) | 2026-04-22 | Top-level operator entrypoint focused on current runtime checks, watcher operations, settings validation, event/queue troubleshooting, and escalation to HEALTH, OBSERVABILITY, INFRASTRUCTURE, and runbooks; still reflects deterministic `make verify-runtime`, registry watcher audit rows, and bounded worker retries for transient missing or unstable notes after current-repo verification. |
