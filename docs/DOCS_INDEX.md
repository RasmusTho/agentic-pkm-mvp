...
| docs/STATUS.md | Operational snapshot | Aligned (forward line v5.x + v5.5/v5.6 planned) | 2025-03-11 | Forward line tracked through v5.5; SoT baseline/forward-line fields plus intent counters (promote.intent.created totals/24h) now surfaced in status; v4.10 baseline remains locked foundation. |
...
| docs/HUMAN-FLOWS.md | Human flows | Aligned (forward line v5.x) | 2025-03-11 | Panel/watcher flow notes keep Runtime V1 mapping; wiring precedence (env > vault System/Config > repo default), watcher daemon, UAT checklist with status intent counters; LangGraph/LLM decider planned for v5.5+. |
...
| docs/PANEL_AGENT.md | PanelAgent / NoteInteractionAgent | Aligned (forward line v5.x + planned v5.5 LangGraph) | 2025-03-11 | Runtime V1 fan-out documented; catalog-driven decider and planner pipeline noted; wiring config precedence (env > vault System/Config > repo default) and validation/fallback described; UAT guide at UAT_PANEL_WATCHER.md. |
...
| docs/OBSERVABILITY.md | Observability | Aligned (forward line v5.x) | 2025-03-11 | Status snapshot exposes SoT baseline/forward lines, active features, and promote.intent.created counters (total + 24h) from outbox; JSON log/span schema unchanged. |
