...
| docs/RELEASE_CHANNELS/README.md | v6.0 capability spec — release channels (stable prod alongside active dev) | Aligned (v6.0 capability spec, docs + operator skills authored) | 2026-05-01 | Defines channel identity, per-channel logical Postgres DBs (`app`/`app_dev`/`app_test`), promotion plan contract, migration reversibility classification, concurrency rule, and rollback posture. Extends the environment model in `docs/ENVIRONMENTS.md` with channel-level isolation so a stable build can run against the real vault while dev continues on the same machine; promotion skills are authored, while feature-level acceptance remains pending. Read before any promotion/rollback or per-channel DB work. |
