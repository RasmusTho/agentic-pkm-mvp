...
| docs/ARCHITECTURE.md | Architecture (SoT v5.x forward line on v5.5 base) | Aligned (forward line v5.x) | 2026-04-09 | Active runtime architecture source of truth; focused on the current runtime/data model, explicit about forward-line versus historical material, and now makes interaction primary on the user-facing side while keeping foundational capabilities and bounded agents first-class. |
...
| docs/STATUS.md | Operational snapshot | Aligned (SoT v5.5 baseline locked + forward line v5.6 planned) | 2026-04-09 | Current snapshot of runtime reality, verification status, and forward-line Now/Next/Later; includes the present-tense agent-evolution posture while keeping autonomous execution explicitly out of current truth. |
...
| docs/plans/LOCAL_TEST_ENVIRONMENT_BOOTSTRAP.md | Local test environment bootstrap | Plan | 2026-04-09 | Docs-first stabilization plan for the canonical local `test` bootstrap path, its golden path, acceptance criteria, relationship to existing blocker issues, and the recommended parent-feature / child-slice execution mapping; now includes the delivered sync-latency harness in its verification posture. |
