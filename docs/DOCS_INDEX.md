...
| docs/ARCHITECTURE.md | Architecture (SoT v5.x line on v5.5 base) | Aligned (forward line v5.x) | 2026-04-23 | Active runtime architecture source of truth; carries an executive summary and current-vs-planned status map that treats v5.6 as delivered while separating active v5.5 runtime, flagged pilots/scaffolding, delivered V2 checkpoint/retry/per-tool and plan-timeout follow-ups, the shipped flagged remote MCP multiplex seam with deterministic local fallback, retrieval capability metadata plumbing, read-only Chat cognition scaffold, closed A2A lifecycle cleanup, remaining post-v5.6 follow-ups, v6 target-state direction, current event envelope/outbox consumer contracts, and the current full GitHub Issue contract without reviving historical AMG/SetDB or module-map claims. |
...
| docs/OPERATIONS.md | Operations playbook | Aligned (forward line v5.x) | 2026-04-23 | Top-level operator entrypoint focused on current runtime checks, watcher operations, settings validation, event/queue troubleshooting, and escalation to HEALTH, OBSERVABILITY, INFRASTRUCTURE, and runbooks; keeps v5.6 closed in the temporal framing while still reflecting deterministic `make verify-runtime`, registry watcher audit rows, sync-latency harness measurement-mode behavior, and bounded worker retries for transient missing or unstable notes after current-repo verification. |
