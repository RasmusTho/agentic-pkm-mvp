...
| docs/ARCHITECTURE.md | Architecture (SoT v5.x line on v5.5 base) | Aligned (forward line v5.x) | 2026-04-14 | Active runtime architecture source of truth; now carries an executive summary and current-vs-planned status map that treats v5.6 as delivered while separating active v5.5 runtime, flagged pilots/scaffolding, post-v5.6 follow-ups, and v6 target-state direction without reviving historical AMG/SetDB or module-map claims. |
...
| docs/STATUS.md | Operational snapshot | Aligned (SoT v5.5 baseline locked + v5.6 delivered) | 2026-04-14 | Current snapshot of runtime reality, verification status, and post-v5.6 follow-up posture; now records issue/code truth that v5.6 is closed, Orchestrator V2 is flagged with checkpoint plumbing but no supported checkpoint/resume execution path, and local runtime/docs validation drift was fixed by #441 / PR #439. |
