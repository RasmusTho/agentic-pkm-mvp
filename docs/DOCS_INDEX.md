...
| docs/ARCHITECTURE.md | Architecture (SoT v5.x line on v5.5 base) | Aligned (forward line v5.x) | 2026-04-15 | Active runtime architecture source of truth; now carries an executive summary and current-vs-planned status map that treats v5.6 as delivered while separating active v5.5 runtime, flagged pilots/scaffolding, post-v5.6 follow-ups, the delivered timeout/SLA contract, and v6 target-state direction without reviving historical AMG/SetDB or module-map claims. |
...
| docs/STATUS.md | Operational snapshot | Aligned (SoT v5.5 baseline locked + v5.6 delivered) | 2026-04-15 | Current snapshot of runtime reality, verification status, and post-v5.6 follow-up posture; now records issue/code truth that v5.6 is closed, Orchestrator V2 is flagged with checkpoint plumbing but no supported checkpoint/resume execution path, local runtime/docs validation drift was fixed by #441 / PR #439, and the bounded timeout/SLA contract shipped via #446 / PR #448. |
...
| docs/ROADMAP.md | Strategic roadmap | Aligned (SoT v5.5 baseline locked + v5.6 delivered) | 2026-04-15 | Roadmap now treats v5.6 as a closed delivery line, keeps v5.5 as the locked baseline, separates post-v5.6 follow-ups from release blockers, distinguishes shipped Orchestrator V2 pilot behavior and the delivered timeout/SLA contract from remaining checkpoint/resume hardening, and replaces stale AMG wording with the current three-surface persistence model. |
...
| docs/HUMAN-FLOWS.md | Human flows | Aligned (forward line v5.x) | 2026-04-15 | User-facing behavior contract for the current system; now explicitly covers life spheres/contexts, role identities, overlap-first semantics with bounded cross-scope permissions, heterogeneous device roles, archive-brain use, artifact continuity beyond the current runtime, canonical human loops used to read runtime support paths, and the v6.0 capability-spec reading anchors through PR #431. |
