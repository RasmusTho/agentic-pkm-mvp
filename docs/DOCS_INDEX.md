...
| docs/ARCHITECTURE.md | Architecture (SoT v5.x forward line on v5.5 base) | Aligned (forward line v5.x) | 2026-04-07 | Active runtime architecture source of truth; focused on the current runtime/data model, explicit about forward-line versus historical material, and still aligned to the shipped v5.5/v5.6 runtime seams after the latest bootstrap/UAT writeback. |
...
| docs/STATUS.md | Operational snapshot | Aligned (SoT v5.5 baseline locked + forward line v5.6 planned) | 2026-04-07 | Current snapshot of runtime reality, verification status, and forward-line Now/Next/Later; now reflects shipped bootstrap startup/UAT receipts while keeping deterministic runtime verification explicitly open. |
...
| docs/ROADMAP.md | Strategic roadmap | Aligned (SoT v5.5 baseline locked + forward line v5.6 planned) | 2026-04-07 | Forward line leans on the v5.5 baseline while v5.6 LangGraph/Reasoning stages remain selectively staged; shipped panel decider work no longer reads as pending, and remaining rollout/acceptance items stay forward-looking. |
| docs/plans/LOCAL_TEST_ENVIRONMENT_BOOTSTRAP.md | Local test bootstrap plan | Plan | 2026-04-07 | Canonical bootstrap chain plan; steps 1-3 and 5 are shipped, while deterministic runtime verification remains the active open slice. |
...
| docs/HUMAN-FLOWS.md | Human flows | Aligned (forward line v5.x) | 2026-04-07 | User-facing behavior contract for the current system; still anchors the broader human loops while the latest audit kept automation/operator language aligned with current runtime support surfaces. |
...
| docs/OPERATIONS.md | Operations playbook | Aligned (forward line v5.x) | 2026-04-07 | Top-level operator entrypoint focused on current runtime checks, watcher operations, and escalation to HEALTH, OBSERVABILITY, INFRASTRUCTURE, and runbooks; now explicitly keeps the local bootstrap path framed as a supported stabilization lane while `#334` remains open. |
