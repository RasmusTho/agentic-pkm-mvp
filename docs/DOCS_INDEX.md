...
| docs/ARCHITECTURE.md | Architecture (SoT v5.x line on v5.5 base) | Aligned (forward line v5.x) | 2026-04-19 | Active runtime architecture source of truth; now carries an executive summary and current-vs-planned status map that treats v5.6 as delivered while separating active v5.5 runtime, flagged pilots/scaffolding, delivered V2 checkpoint/retry/timeout follow-ups, closed A2A lifecycle cleanup, remaining post-v5.6 follow-ups, the delivered timeout/SLA contract, and v6 target-state direction without reviving historical AMG/SetDB or module-map claims. |
...
| docs/STATUS.md | Operational snapshot | Aligned (SoT v5.5 baseline locked + v5.6 delivered) | 2026-04-19 | Current snapshot of runtime reality, verification status, and post-v5.6 follow-up posture; now records issue/code truth that v5.6 is closed, Orchestrator V2 is flagged with checkpoint/resume hardening and retry/backoff observability shipped through #444/#445, local runtime/docs validation drift was fixed by #441 / PR #439, the bounded timeout/SLA contract shipped via #446 / PR #448, timeout discriminator cleanup shipped via #456 / PR #458, A2A lifecycle issue #359 is closed, and v6-driven domain/zone current-state bugs #435/#436/#437 are closed. |
...
| docs/ROADMAP.md | Strategic roadmap | Aligned (SoT v5.5 baseline locked + v5.6 delivered) | 2026-04-19 | Roadmap treats v5.6 as a closed delivery line, keeps v5.5 as the locked baseline, separates post-v5.6 follow-ups from release blockers, distinguishes shipped Orchestrator V2 pilot behavior, checkpoint/resume hardening, retry/backoff observability, timeout/SLA, timeout-discriminator cleanup, and closed A2A parent lifecycle from remaining follow-ups, and keeps stale AMG wording replaced with the current three-surface persistence model. |
...
| docs/plans/OWNER_DOC_FEEDBACK_LOOP.md | Owner-doc feedback loop plan | Plan | 2026-04-18 | One `post-merge-owner-doc` agent, invoked from `verification-validation-feedback` at merge time, reads the diff and either opens a docs PR, opens a bounded follow-up issue, or leaves an explicit "no owner-doc change implied" receipt. No gates, labels, tokens, or new compliance machinery. |
