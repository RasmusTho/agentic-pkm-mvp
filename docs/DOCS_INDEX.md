...
| docs/contracts/TIMEOUT_AND_SLA_CONTRACT.md | Timeout and SLA contract | Aligned (forward line v5.x) | 2026-04-15 | Canonical current-state reference for executor-level per-tool timeout handling, observable timeout errors, orchestration timeout constraints, and A2A timeout non-claims delivered by Issue #446 / PR #448. |
...
| docs/ARCHITECTURE.md | Architecture (SoT v5.x line on v5.5 base) | Aligned (forward line v5.x) | 2026-04-21 | Active runtime architecture source of truth; now carries an executive summary and current-vs-planned status map that treats v5.6 as delivered while separating active v5.5 runtime, flagged pilots/scaffolding, delivered V2 checkpoint/retry/timeout follow-ups, closed A2A lifecycle cleanup, remaining post-v5.6 follow-ups, the delivered timeout/SLA contract, v6 target-state direction, current event envelope/outbox consumer contracts, and the current full GitHub Issue contract without reviving historical AMG/SetDB or module-map claims. |
...
| docs/STATUS.md | Operational snapshot | Aligned (SoT v5.5 baseline locked + v5.6 delivered) | 2026-04-21 | Current snapshot of runtime reality, verification status, and post-v5.6 follow-up posture; now records issue/code truth that v5.6 is closed, Orchestrator V2 is flagged with checkpoint/resume hardening and retry/backoff observability shipped through #444/#445, local runtime/docs validation drift was fixed by #441 / PR #439, the bounded timeout/SLA contract shipped via #446 / PR #448, timeout discriminator cleanup shipped via #456 / PR #458, A2A lifecycle issue #359 is closed, v6-driven domain/zone current-state bugs #435/#436/#437 are closed, canvas-Chat co-editing is a docs-only target surface, event/outbox consumer contracts are current, and delivery feedback loop governance is present. |
...
| docs/ROADMAP.md | Strategic roadmap | Aligned (SoT v5.5 baseline locked + v5.6 delivered) | 2026-04-21 | Roadmap treats v5.6 as a closed delivery line, keeps v5.5 as the locked baseline, separates post-v5.6 follow-ups from release blockers, distinguishes shipped Orchestrator V2 pilot behavior, checkpoint/resume hardening, retry/backoff observability, timeout/SLA, timeout-discriminator cleanup, and closed A2A parent lifecycle from remaining follow-ups, and keeps stale AMG wording replaced with the current three-surface persistence model, canvas-Chat future-surface framing, and full Issue-contract guardrails. |
...
| docs/HUMAN-FLOWS.md | Human flows | Aligned (forward line v5.x + v6 docs-only planning) | 2026-04-21 | User-facing behavior contract for the current system; explicitly covers life spheres/contexts, role identities, overlap-first semantics with bounded cross-scope permissions, heterogeneous device roles, archive-brain use, artifact continuity beyond the current runtime, canonical human loops used to read runtime support paths, canvas-Chat as a future direct co-editing surface, and the v6.0 interaction, finding/reorienting, persistence-surface, and commitment capability-spec reading anchors. |
...
| docs/INTERACTION_SURFACES_AND_AUTHORITY/README.md | v6.0 capability spec — Panel, Chat, Automation surfaces and authority boundaries | Aligned (v6.0 capability spec, docs-only) | 2026-04-21 | Names the three interaction surfaces and their authority boundaries; resolves the Chat read-only-vs-canvas contradiction in favor of canvas-shaped Chat with governed mutation rights (decision recorded 2026-04-11 in `RECONCILE_CHAT_MUTATION_AUTHORITY.md`), defines Panel as primary command surface without making it the exclusive authoritative intent source, and adds the hybrid Chat integration schema while keeping runtime Chat implementation future work. Read before any Panel/Chat/Automation evolution. |
...
| docs/OBSERVABILITY.md | Observability | Aligned (forward line v5.x) | 2026-04-21 | Runtime observability contract for heartbeats, counters, spans, and status interpretation; now explicitly keeps runtime monitoring separate from higher-level architecture ownership, aligns `watcher_runs` semantics with the registry watcher audit stream, and documents bounded retry/poison-message observability without claiming a dedicated DLQ. |
| docs/OPERATIONS.md | Operations playbook | Aligned (forward line v5.x) | 2026-04-21 | Top-level operator entrypoint focused on current runtime checks, watcher operations, event/queue troubleshooting, and escalation to HEALTH, OBSERVABILITY, INFRASTRUCTURE, and runbooks; still reflects deterministic `make verify-runtime`, registry watcher audit rows, and bounded worker retries for transient missing or unstable notes after current-repo verification. |
...
| docs/RETRIEVAL.md | Retrieval | Aligned (forward line v5.x) | 2026-04-14 | Hybrid retrieval + optional rerank hooks (`RERANK_ENABLE`/`RERANK_PROVIDER`); now includes a v6 reading boundary that keeps current retrieval behavior separate from staged target migration toward capability extraction and retrieval/orientation/resurfacing separation. |
