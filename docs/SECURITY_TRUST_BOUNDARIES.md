...
| Environment / exposure boundary | `prod`, `dev`, `test` scoped vaults/stores; trusted-device server bind default | LAN, Tailscale, public internet, wrong env/channel | Environment changes data touched, not product semantics | `docs/ENVIRONMENTS.md`, `companion-ui/docs/LOCAL_ACCESS_MODEL.md` | Public exposure is unsupported; LAN/Tailscale exposure must stay proportionate to trusted-device personal use. |
