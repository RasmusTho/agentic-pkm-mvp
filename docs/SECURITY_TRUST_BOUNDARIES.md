...
Authority: Security trust-boundary inventory for Mimer. Consumes existing architecture and authority docs; does not replace their detailed ownership.
...
| Direct filesystem agent boundary | Human-declared agent workspace roots, OS/tool permissions, human delegation decisions | External agent processes with direct Markdown access (Claude Code, Codex, Fable, future MCP/A2A/cloud agents) | Writes are observed, not mediated: not APPLY, no WriteGuard, no Mimer receipt; output is not human-canonical until promoted. Mimer's control is post-hoc observation/classification/promotion governance, plus mediated egress for context it serves itself (`docs/AGENT-FLOWS.md` §4/§7/§12) | `docs/AGENT-FLOWS.md`, `docs/CONCEPTS/TRUST_SEMANTICS_CONTRACT.md` | Do not claim runtime control where only filesystem delegation exists. Mediated-egress rules do not protect files already exposed by OS-level access. Root-scoped grants, RBAC, and per-agent/per-tool authorization are future posture (`docs/security/AGENT_TOOL_EXECUTION_SECURITY_ADDENDUM.md`), not shipped behavior. Review root declarations that include sensitive or cross-scope spheres. |
...
| Environment / exposure boundary | `prod`, `dev`, `test` scoped vaults/stores; loopback UI bind default | LAN, Tailscale, public internet, wrong env/channel | Environment changes data touched, not product semantics | `docs/ENVIRONMENTS.md`, `companion-ui/docs/LOCAL_ACCESS_MODEL.md` | Public exposure is unsupported; LAN/Tailscale exposure must be explicit and proportionate to trusted-device personal use. |
| Source-repo content exposure | Operator-invariant product artifacts; secret-free, registered builder/ops exceptions | The tracked public repository and its readers | The public tree must obey the INV-EF1 public/private seam; personal bindings resolve on the private side | `docs/adr/ADR-0046-inv-ef1-public-private-seam.md`, `docs/architecture/ecosystem-federation.md` § Public/private invariant | Review changed tracked content for secrets and unregistered personal-environment material; this is distinct from network reachability. |
