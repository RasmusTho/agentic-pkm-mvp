...
## Addendum 2026-07-11 — definitional iteration (owner-directed; Fable-5 + external GPT-5 review)

After ADR-0051/0054/0058 landed, the owner directed a definitional deep-dive on what an episode *is* (length, nesting, episode-vs-project), ran it through an independent external GPT-5 review, and ruled on the convergence. This addendum records the outcome so future work does not re-derive it. Where it touches decided ADRs, the enactments are: the ADR-0058 inline amendments dated 2026-07-11 (this change), and the ADR-0051 grain-example amendment tracked as #3436 — **pending**: until #3436 lands, ADR-0051 §3 still shows the old `⊂ a project` example and this addendum is the corrective record, with #3436 as the fix vehicle. Everything else here is engine-design direction for the ERE/capture epics — advisory until a slice enacts it.

**Definition (sharpened).** An episode is the *smallest coherent lived situation that can close independently*. Segmentation heuristic: mint a separate episode only when the situation has (a) a recognizable identity of its own and (b) a closure that *can* carry independent significance for material currency. "Can" is load-bearing: an episode's existence must never depend on how the system later wants to rank its material — identity is defined by the experience; decay merely *consumes* closure (the two-jobs separation; guards against ranking retro-defining episodes).

**Episode ≠ project (the demarcation test).** Duration is not the test; **continuity of lived presence vs. required re-entry** is. Machine-checkable form — (1) reasonably contiguous presence in time? (2) would sleep/several intervening situations normally mean the situation was *left*? (3) is there a single moment participants could call "this occasion is over"? (4) does continuing require separate re-entries? 1–3 yes + 4 no → episode; 4 yes → project/process/series (a *structure* episodes bind to via the goal dimension, ADR-0051 §1). A multi-day trip can still be a container episode (one continuous deviant life-situation); "build the app" is a structure. Example fix: `a call ⊂ a workday, bound to a project` — pending enactment via #3436.

**Micro / situation / container** are scale heuristics, not ontological types. Engine defaults: propose *situation* episodes; create containers only when experienced or used as one coherent situation (a calendar day is not automatically a container); never auto-mint micro-episodes — micro-moments stay raw observations until positive evidence promotes them (human asks after them, multiple artifacts attach, a decision lives there, or they need independent closure).

**Gaps and resumption.** A gap splits the episode when the human must *re-enter* the situation rather than resume a subordinate interruption. Presumptions: short interruption + same goal/context → same episode; sleep/day boundary → new episode by default; week+ gap → always new. Continuation links (`resumes`/`continues`/`part-of`) carry identity across splits. No stored "paused" state — open/closed suffices in the vault; "possibly ongoing" is an engine hypothesis only. Re-open is reserved for *wrong* closures; genuine later continuation gets a new linked episode (enacted in ADR-0058 §4).

**Nesting.** The data model is unbounded; the *engine* auto-proposes at most three levels; deeper trees only via human re-cut or import. Parent closure yields derived `closed-by-ancestor` on children, never a rewrite of their own state (enacted in ADR-0058 §1). Re-cut operates locally: split/merge the selected episode, keep children's internal relations, propose (never silently apply) reassignment of ambiguous children.

**Spheres.** One episode model across work/private/RPG; per-sphere *parameters* (boundary thresholds, expected length, gap sensitivity, decay strength) may vary and should be learned from situation type + human corrections. Deliberate bend vs. the external review: sphere/scope is **not** a weak optional hint in this system — scope crossings remain governance-gated (ADR-0027/0028; ADR-0054 §5). Parameter variation yes; soft classification no.

**Bindings.** Three internal states — no candidate, candidate binding(s), accepted binding — where candidates mature into the current binding under the opt-out posture, with uncertainty+origin retained in machine metadata for later re-cut. No synthetic "unknown episode" note (it would become a landfill that looks like a lived situation). Retroactive material defaults to `unbound`; reconstruct episodes only on positive evidence (date + calendar + place + multiple coherent artifacts). Open bindings must be *credible* to hold material hot (enacted in ADR-0058 §3).

**Decay scope.** Closure-dampening applies to episode-near classes only (raw captures, transcripts, working notes, ephemeral plans); accepted decisions, evergreen knowledge, and project status are exempt — their liveness follows their own properties (enacted in ADR-0058 §2; concrete class list decided in the ADR-0055/T2 note-class table, #3131).

**Decision order for the capture/ERE epics** (from the external review, owner-endorsed): 1 separate episode identity from material persistence; 2 fix the episode–project test; 3 fix auto-segmentation/closure rules; 4 fix multi-binding decay behavior; 5 add nesting/containers; 6 tune per-sphere parameters only after real usage data.
