...
- The same governed seam now covers watcher-detected `settings/local.md` deltas for runtime-gating keys, so direct value edits to `enableVaultWatcher` / `enableAutoIndexing` (where the key remains present in frontmatter) emit `SettingsWriteReceipt`s instead of silently waiting for the next start (#2512). Deleting a gating key entirely reverts it to its default without emitting a receipt: the delta path (`app/watcher/settings_delta.py`) builds `current_values` only from keys still present and diffs those, so key removals are not yet receipted.
