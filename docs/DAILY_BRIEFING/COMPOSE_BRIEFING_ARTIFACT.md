...
6. **Determinism and conflict-aware regeneration**: composing from the same inputs on the same day produces the same content. A regeneration that finds an existing briefing reads its exact target bytes and passes their hash as the expected version to the canonical guarded rewrite path. The complete proposal remains privately staged until that path atomically replaces the target; if the target changed after observation, the rewrite fails closed and preserves the concurrent edit. The composer is the note's only writer: because the note declares `agent_maintained: true` and `read_only: true`, the shared uuid-heal seam (`app/services/note_uuid.py::ensure_note_uuid`) leaves it unchanged instead of reserializing it during watcher/worker ingest (#5656).
