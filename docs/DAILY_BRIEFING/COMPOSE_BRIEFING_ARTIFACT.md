...
6. **Determinism and conflict-aware regeneration**: composing from the same inputs on the same day produces the same content. A regeneration that finds an existing briefing reads its exact target bytes and passes their hash as the expected version to the canonical guarded rewrite path. The complete proposal remains privately staged until that path atomically replaces the target; if the target changed after observation, the rewrite fails closed and preserves the concurrent edit. The composer is the note's only writer: because the note declares `agent_maintained: true`, `read_only: true`, and `authority_role: derived`, the shared uuid-heal seam (`app/services/note_uuid.py::ensure_note_uuid`) never reserializes it during watcher/worker ingest and returns a stable path-derived identity instead of persisting one (#5656).
