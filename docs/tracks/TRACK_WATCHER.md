...
- DB outbox is the authoritative queue; `index-outbox.jsonl` is telemetry only. The panel watcher enqueues intents/events to the DB outbox so the worker processes them.
