...
Today's same-note substrate is progressively enforced. A legacy rewritten writer that omits `expected_version` still takes the versionless migration path and can resolve concurrent edits as silent last-write-wins. A filesystem caller that supplies the raw-byte SHA-256 version it read uses the descriptor-anchored atomic exchange path. If that version is already stale at the first seam comparison, VMW-02 stages the exact proposal as a sibling Markdown conflict artifact without changing the canonical note and returns `outcome="conflict_staged"`, `conflict_artifact`, `writer_identity`, and `written_at`. Missing targets and races after the first comparison retain the fail-closed exchange/rollback behavior and internal displaced-inode preservation. Append-only operations do not enter the stale path; independently, `append_note_relative` asserts `WriteGuard` before port resolution or mutation (INV-VW2 / #3129). VMW-03 quarantines both iCloud and runtime-staged artifacts at the shared production iterator before ordinary ingestion; VMW-04 owns the final INV-VW3 registry promotion.
...
- **F6 — Full multi-writer consistency model: remaining runtime enactment.** The decision itself is made — `docs/adr/ADR-0055-vault-multiwriter-consistency-model.md` (Accepted 2026-07-07) resolves #3114 and gates B2 (#3024). #3131 supplies the published note-classification table; VMW-01 supplies the shared request/receipt/provenance/classifier substrate; VMW-02 stages initially stale rewritten proposals; VMW-03 quarantines shared artifacts before ordinary ingestion; and #3129 independently closes INV-VW2 by guarding `append_note_relative` at the seam. What remains is migrating the remaining versionless rewritten writers (#3570) and VMW-04's registry/parent reconciliation. This contract's §6 discipline complements the progressive runtime enforcement; it is not a substitute for those remaining steps.
...
| Admitting external app agents to the live writer set | **Extend** | Owner ruling 2026-07-07, enacted via ADR-0056; extends the writer set ADR-0055 governs (supersedes ADR-0053) to a new writer class ahead of complete enactment; VMW-01 through VMW-03 and INV-VW2 are now delivered |
...
- `docs/adr/ADR-0055-vault-multiwriter-consistency-model.md` — the decided multi-writer mechanism (supersedes ADR-0053, resolves #3114); remaining versionless-writer migration and VMW-04 reconciliation are tracked there.
