...
The remaining same-note write substrate is unchanged from what ADR-0053 described: concurrent writes to the same vault note resolve as **silent last-write-wins** until the stale-write slice lands. The general write primitive is a blind in-place overwrite with no stale check (`app/knowledge/adapters.py:29-40`); compare-and-swap exists only in the panel-watcher family (`OptimisticWriteGuard`, `app/components/concurrency.py:118-131`); and `append_note_relative` does not itself assert the WriteGuard (audit INV-VW2). Independently, iCloud and runtime-staged conflicted-copy Markdown artifacts are now quarantined at the shared production iterator before ordinary ingestion (VMW-03 / #3452); VMW-04 owns the final INV-VW3 registry promotion.

Therefore: the rules below are **client discipline that shrinks the collision window and makes collisions detectable and attributable during the enactment gap — they do not eliminate LWW today** and they implement no INV-VW1/VW2/VW3 enforcement themselves. They are **binding on external app agents** (the writer class this contract admits) and **recommended for Bifrost during B1** (per the ADR-0056 §3 amendment of 2026-07-11: ADR-0055 superseded ADR-0053 in full, so nothing is "carried forward" — Bifrost's B1 free pass is an **explicit owner extension for the enactment gap**, ending with a forced re-decision (a) before the mechanical-hygiene auto-apply flip (ADR-0048 / G2-3) turns on and (b) immediately on the first observed same-note data-loss incident). When ADR-0055's mechanism ships, it becomes binding for all writers over the rewritten-note classes, including this one — at that point this section's discipline becomes a client-side complement to real enforcement rather than the only mitigation.
...
- **F6 — Full multi-writer consistency model: remaining runtime enactment.** The decision itself is made — `docs/adr/ADR-0055-vault-multiwriter-consistency-model.md` (Accepted 2026-07-07) resolves #3114 and gates B2 (#3024). #3131 supplies the published note-classification table; VMW-01 and VMW-03 now supply the shared conflict-artifact grammar and production ingest quarantine. What remains is closing INV-VW2 (`append_note_relative` WriteGuard coverage), the INV-VW1 stale-check/conflict-staging path, writer provenance at the substrate, and VMW-04's registry reconciliation. This contract's §6 discipline remains the interim client-side complement to those unfinished mechanisms, not a substitute for them.
