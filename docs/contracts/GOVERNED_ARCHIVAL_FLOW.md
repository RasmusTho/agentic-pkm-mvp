...
Before reservation or copy can have an effect, `OperationBinding` durably binds one idempotency key to the exact artifact identity, owner generation, policy profile, distinct source representation, and distinct target representation. The owner-native source readback must still be active and readable before reservation or copy. A same-key call may resume only that immutable tuple and re-reads terminal owner-journal state before interpreting source retirement, so a stale caller converges on a concurrent completion. An incompatible same-key or competing source binding returns typed conflict before source retirement; atomic exclusion and durable operation readback remain owner-adapter responsibilities rather than a kernel lock or registry.

Every reservation, verification, receipt, activated representation, retired representation, and loaded operation is checked against the complete binding before the next authority-changing effect. Initial and resumed uncertainty is reconciled through owner-native operation and representation readback and projects typed `transition_pending` or `unavailable` rather than blind replay or false success; when that validated readback already proves completion, the caller receives the canonical completed receipt. Completion stores and returns one canonical `retired` receipt, so first success, completion-after-effect reconciliation, and retry expose identical terminal evidence.

Restore authorization names the exact artifact, generation, policy, and representation and must carry active stage and liveness for the current call before any prior restore receipt can be reconciled. Terminal restore then requires an exact owner-native `restored` receipt and readback with active liveness; refused or indeterminate evidence never projects success. Cleanup may project `erased` only from an owner-native `CleanupProof` that matches the artifact, generation, policy, and complete set of policy-required representations. Retry reads that durable proof before consulting live enumeration, because completed cleanup may legitimately remove the representations it proves handled; any newly live same-generation representation contradicts the proof and remains `erasure_pending`, while missing or unavailable readback remains typed pending or `unavailable`.
