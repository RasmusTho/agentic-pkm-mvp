...
- `read_note(locator)`
- `write_note(locator, content, expected_version=None)` — `expected_version` opts a rewritten note into optimistic concurrency over raw on-disk bytes. The filesystem adapter performs an atomic path exchange, preserves the existing permission mode, and verifies the displaced original, rolling the exchange back when bytes or mode changed at the linearization point. Every successful exchange retains the displaced inode as a standard conflicted-copy artifact under the nearest parent-boundary `_conflicts/<original-parent>/` tree so a late save through a pre-existing descriptor cannot disappear; rollback and rollback-failure preservation use the same non-canonical, same-filesystem tree. Cleanup failures are logged without masking the write receipt or primary `KnowledgeWriteConflict`. Changed or missing targets fail rather than being overwritten, recreated, or silently discarded; unsupported atomic-exchange platforms fail closed with `KnowledgeCapabilityError`.
