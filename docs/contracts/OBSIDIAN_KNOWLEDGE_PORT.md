...
- `read_note(locator)`
- `write_note(locator, content, expected_version=None)` — `expected_version` opts a rewritten note into optimistic concurrency over raw on-disk bytes. The filesystem adapter performs an atomic path exchange, preserves the existing permission mode, and verifies the displaced original, rolling the exchange back when bytes or mode changed at the linearization point. Every successful exchange retains the displaced inode as a non-indexed `*.md.conflict` artifact in a containment-checked `_conflicts` directory so a late save through a pre-existing descriptor cannot disappear without reintroducing stale note content into search/projection readers. Normal adapters keep this directory inside `vault_root`; root-anchored legacy adapters keep it beside the actual target. Rollback and rollback-failure preservation use the same same-filesystem tree. Cleanup failures are logged without masking the write receipt or primary `KnowledgeWriteConflict`. Changed or missing targets fail rather than being overwritten, recreated, or silently discarded; unsupported atomic-exchange platforms fail closed with `KnowledgeCapabilityError`.
