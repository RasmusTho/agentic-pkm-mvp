...
- Environment posture: `dev` and `prod` are now explicit first-class environments in the SoT. The governing contract and runtime implementation live in `docs/ENVIRONMENTS.md`. Current baseline/runtime docs remain production-facing unless they explicitly say lab/dev-only.
- Runtime environment selection: explicit environment model for `dev` and `prod` is now canonical in the config layer (`app.config.environment`). The default environment is `prod` (production-like). Operators can select `PKM_ENVIRONMENT=dev` for development/diagnostic mode. Existing `PKM_SETTINGS_PROFILE` (operator/lab) behavior is preserved and maps into the environment model: `lab` → `dev`, `operator` → `prod`. This provides one documented control surface for environment-specific behavior across the runtime. See `docs/ENVIRONMENTS.md` for full definition.
