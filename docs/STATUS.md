...
- Nightly verification now includes the deterministic Quality Wave acceptance harness (`tests/quality_wave/test_uat_harness.py`) plus a first bounded Postgres contracts lane (`tests/int/test_pg_backend.py`, `tests/api/test_status_store_pg.py`, `tests/indexer/test_outbox_roundtrip_pg.py`) inside `integration-nightly`. The PG lane uses a `postgres:16` service with `DATABASE_URL=postgresql://app:app@localhost:5432/app`, `STORE_BACKEND=pg`, and `alembic upgrade head` as the schema/bootstrap contract for the first green recurring PG wave.
### Later
