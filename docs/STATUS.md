...
- The System Entry Point feature (#1782) is partially shipped and still open as a validation hub. Shipped child surfaces now include server-declared entry state, latency-ladder re-entry treatments, unified topbar/overlay host, the Panel command palette, governed capture append plus capture modal, memory review-queue endpoints plus drawer, and the read-only receipts history modal. Remaining parent acceptance, including system-map/guidance/settings/state-gallery closure and any owner-doc promotion needed by SEP-11, is still tracked on #1782 and its child issues.
