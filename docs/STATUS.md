...
| Orchestrator V2 pilot | Shipped/flagged: parallel execution + dependency-aware scheduling with `ORCHESTRATOR_VERSION=v2`; V1 is default. Coverage: flag contract, plan-graph scheduling, event/trace compat, compensation/rollback, retry metadata handling, and bounded per-tool timeout/SLA contract coverage from Issue #446 / PR #448. Per-tool timeout via `tool_timeout_seconds` setting is supported on both V1 and V2 (see `docs/contracts/TIMEOUT_AND_SLA_CONTRACT.md` for the complete contract). `CheckpointStore` plumbing exists, but checkpoint/resume is not the supported baseline execution path. Delivery receipts include Issue #250 (pilot), Issue #251 (compensation), Issue #252 (checkpoint/retry handling), and Issue #446 (timeout/SLA contract). | Future slices: checkpoint/resume hardening (#444), retry/backoff observability (#445), possible plan-level timeout budgets, possible repo-wide A2A/runtime delivery SLA, and V2 adoption after stability signals. |
