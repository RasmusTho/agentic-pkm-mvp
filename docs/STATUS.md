...
| Orchestrator V2 pilot | Shipped/flagged: parallel execution + dependency-aware scheduling with `ORCHESTRATOR_VERSION=v2`; V1 is default. Coverage: flag contract, plan-graph scheduling, event/trace compat, compensation/rollback, and retry metadata handling. Per-tool timeout via `tool_timeout_seconds` setting is supported on both V1 and V2 (see `docs/contracts/TIMEOUT_AND_SLA_CONTRACT.md` for the complete contract). `CheckpointStore` plumbing exists, but checkpoint/resume is not the supported baseline execution path. Delivery receipts include Issue #250 (pilot), Issue #251 (compensation), and Issue #252 (checkpoint/retry handling). | Future slices: checkpoint/resume hardening (#444), retry/backoff observability (#445), plan-level timeout budgets, repo-wide A2A/runtime delivery SLA (#446), and V2 adoption after stability signals. |
