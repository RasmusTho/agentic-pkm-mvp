...
- Environment posture: `dev` and `prod` are now explicit first-class environments in the SoT. The governing contract lives in `docs/ENVIRONMENTS.md`. Current baseline/runtime docs remain production-facing unless they explicitly say lab/dev-only. Implementation: runtime now exposes explicit environment selection via `PKM_ENVIRONMENT` (explicit override) or `PKM_SETTINGS_PROFILE` mapping (`lab` → `dev`, `operator` → `prod`, default is `prod`). Environment is resolved at startup and stored in `InstanceSettings.environment` within the `SettingsBundle`. See `docs/ENVIRONMENTS.md` for full specification (Issue #263).
...
- Settings tiering (watcher controls): runtime defaults to `PKM_SETTINGS_PROFILE=operator`; dev/lab-only watcher tuning env vars are applied only when `PKM_SETTINGS_PROFILE=lab`. This is current shipped control behavior; see `docs/ENVIRONMENTS.md` for the environment contract it serves.
