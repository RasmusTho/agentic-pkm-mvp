...
- **Next**
  - **Companion Note + Note Context** — replaces VaultMirror (`System/Metadata/VaultMirror`) with a flat, UUID-keyed companion file at `vault/_system/companions/<uuid>.md` and introduces a `NoteContext` runtime abstraction that gives agents rich, structured context instead of raw 800-char snippets. This is infrastructure work that unblocks agent quality improvements (Panel Agent, Classifier, ASK, Promotion). Full plan with bounded field set, healing authority matrix, implementation sequence, and exit criteria: `docs/plans/COMPANION_NOTE_AND_NOTE_CONTEXT.md`. **8 parts in sequence**: (1) contract docs (`COMPANION_NOTE_CONTRACT`, `ARTIFACT_MODEL_AND_LIFECYCLES`); (2) `app/services/companion_note.py` — read/write/scan_attachments; (3) healing scenarios — 6 recovery paths tested; (4) migrate `app/ingest/vault_alpha.py` to write companions; (5) delete `app/services/note_log.py` + VaultMirror references; (6) `app/services/note_context.py` — multi-surface context assembler; (7) wire Panel Agent to Note Context (replaces `graph.py:240` 800-char snippet); (8) doc sync. **Key decisions**: companion `content_hash` scalar replaces `ingest_fingerprint` dict; `review_state`/`maturity` stay in vault note frontmatter only; ignore scope is `_system/companions/**` (not `_system/**`); event contracts unchanged. **Blocker for**: Panel Agent quality (Part 7), agent context improvements in v5.6+.
...
- Companion Note + Note Context plan (8-part): `docs/plans/COMPANION_NOTE_AND_NOTE_CONTEXT.md`
