...
- **Now**
  ...
  - **SBS operationalization initiative** — target-state architecture adoption and transition control. Purpose: make the 2030 target SBS actionable through ADRs, contracts, mapping, registers, fitness rules, issue tracking, and PR impact classification without claiming shipped runtime behavior. Status: target-state docs/contracts resident; implementation enforcement remains in progress. Tracking issue: #2337. Delivery issue set: #2355 with child delivery slices #2356-#2363. Phase plan: Phase 0 decision anchoring, Phase 1 current-to-target mapping, Phase 2 critical contracts, Phase 3 first enforcement rails, Phase 4 containment adapters, Phase 5 selective physical split. Source docs: `docs/SYSTEM_BREAKDOWN_STRUCTURE.md`, `docs/architecture/SBS_OPERATIONALIZATION_PLAN.md`, `docs/architecture/SBS_CURRENT_TO_TARGET_MAPPING.md`, `docs/architecture/SBS_BOUNDARY_REGISTER.md`, `docs/architecture/SBS_TRANSITION_DEBT.md`, and `docs/architecture/SBS_FITNESS_RULES.md`. Non-goals: no immediate fourteen-module split, no no-vault rewrite, no Obsidian removal, no full distributed consensus, and no target-state-as-shipped claim. First slices: ActiveContextSet, GovernedWriteProtocol, ArtifactContract, StorePort, ContextBundle, MemoryRecord, ExecutionRequest, and ReplicationEnvelope.
