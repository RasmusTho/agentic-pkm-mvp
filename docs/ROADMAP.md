...
- **Next**
  - **Environment contract follow-through** — implement the docs-first environment contract from `docs/ENVIRONMENTS.md` in bounded slices: explicit runtime environment selection, environment-scoped vault/store separation, and environment-aware operator diagnostics without changing the shared architecture contracts.
