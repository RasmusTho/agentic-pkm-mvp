...
- **Version token:** use a version column, content hash, or `mtime` snapshot captured at read time.
  Vault text producers that pass a content hash as `expected_version` MUST obtain the decoded
  UTF-8 text and the SHA-256 of the exact raw bytes from the same filesystem read via
  `read_note_text_with_version`. Re-encoding `Path.read_text()` is not equivalent because universal
  newline handling can turn CRLF into LF and falsely classify an unchanged note as stale. A
  hash-only producer may hash `read_bytes()` directly.
...
- **Acknowledgement fence:** panel note updates prepare their Markdown/events without persisting
  ...
  non-empty executed IDs and dispatch eligible events. A stale or staged-conflict result performs
  none of those acknowledgement effects, leaves the prior snapshot unchanged, and is counted by the
  note watcher as skipped/deferred rather than processed.
