...
| #5531 [First approved Issue-delivery operation](#first-approved-issue-delivery-operation) | Finite M2 operation contract defined under FCA-08; FCA-ID-A admission and FCA-ID-B destination/effect adapter are delivered by #5550/#5551 | Independent readback, destination activation, and actual owner evidence remain separate. |
...
authenticated preview/Hold/Start approval and durable receipt; FCA-ID-B now delivers the
authenticated destination reservation/attempt/entry adapter around the existing launcher.
Independent source readback is not implemented or activated by this document. It delivers exactly one bounded, open, strictly valid `agent:ready` Issue in
...
<!-- anchor: FCA-ID-B -->
### FCA-ID-B destination/effect adapter (delivered)

Stable source anchor: **FCA-ID-B**. The production destination adapter is delivered in the
existing `cli.py::dispatch_sessions` → `epic_dispatch.py::dispatch_issue_sessions` →
`CodexIssueSessionLauncher.launch` path. It consumes one committed FCA-ID-A approval through the
authenticated BuilderOps service and persists destination-owned `BuilderOpsReceipt` records for
the exact operation/approval/manifest/destination/run binding, one reservation, one pre-launch
attempt, and the separately observed Codex entry/session. Same-key replay and outbox-style retries
read those records before any launch; competing keys, changed manifests, approval reuse, stale or
revoked authority, and unavailable lookup fail closed. An entry or response lost after a possible
launch remains `launch_unknown`/reconciliation-needed and never allocates a replacement attempt or
session. The selected launcher's stop support remains `unsupported`; no kill, cancel, cleanup,
rollback, lock deletion, or synthetic terminal evidence is added.
...
implements the authenticated Issue approval/readback seam below; FCA-ID-B now supplies destination
reservation/attempt/entry execution, while independent source readback remains the later FCA-ID-C
slice. No second service,
...
| Durable reservation and attempt | FCA-ID-B's destination adapter surrounding `CodexIssueSessionLauncher` writes through the same authenticated service and PostgreSQL transaction/receipt/outbox owner in `control_plane/store.py`. Existing `BuilderOpsReceipt` records carry destination-owned reservation/attempt payloads; local run-state is not authority. The adapter atomically guards full repository/destination/workflow/type/key and approval bindings, rejecting competing or changed bindings. An unresolved operation for the same Issue/destination prevents another admitted launch. |
| Attempt and actual entry | FCA-ID-B persists a unique attempt before calling the existing launcher and durably observes that same attempt's actual process entry/session independently. Existing outbox intent/claim/reconcile remains the delivery mechanism where needed; a retry performs exact lookup before dispatch and cannot create a second attempt. Buffered subprocess/session output alone is not crash-safe entry evidence. |
| Continuing effects | FCA-ID-B exposes fresh authenticated effect gates for repository/worktree, claim, publication, review/merge, and closure/reconciliation. The service rechecks permission/revocation/epoch, expiry, source/profile and destination identity before each destination receipt write. Local files and skills retain their present ownership; no new workflow engine drives them. Independent GitHub/CI/source readback remains FCA-ID-C; an unavailable gate keeps this operation unavailable. |
...
as the destination/effect adapter and FCA-ID-C remains the independent readback obligation. Each Issue binds its exact production surface
...
| 2 — **FCA-ID-B**, delivered by #5551 | Authenticated destination adapter at `cli.py::dispatch_sessions` → `epic_dispatch.py::dispatch_issue_sessions` → `CodexIssueSessionLauncher.launch`, backed by the same service transaction/receipt/outbox owner; bind unique reservation/attempt/entry and fresh authority into the permitted effect gates. No raw CLI or prompt-only admission bypass. | `tests/builderops/test_issue_delivery_operation.py::test_production_dispatch_reservation_and_crash_matrix` enters the real adapter and covers competing keys/approvals, crashes before/after attempt and lost entry response. `::test_delivery_effect_boundaries_recheck_authority` reaches the adapter's effect gates and refuses changed/revoked authority before the next effect without a second launch. `::test_selected_launcher_reports_stop_unsupported` covers truthful stop. Independent source readback and M2 live/candidate/owner evidence remain pending FCA-ID-C/parent validation. |
