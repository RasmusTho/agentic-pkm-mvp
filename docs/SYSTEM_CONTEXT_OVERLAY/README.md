...
| [CLASSIFY_DEPLOYED_INFRASTRUCTURE](CLASSIFY_DEPLOYED_INFRASTRUCTURE.md) | SBI-2 | SBI-1 | §2, §14 | every `docker-compose.yaml` service + every host process in `docs/deployment/DEPLOYMENT_AND_ENVIRONMENTS.md :: Environment matrix :: Current reality (verified 2026-06-29)` has exactly one classification row; Ollama's two bindings both classified |
