...
<!-- PA2-FREEFORM -->
**Freeform catalog-driven proposal path (shipped).** When a panel has an instruction but no checkbox actions, the LLM decider (`PANEL_AGENT_DECIDER=llm`) consults the full active catalog and proposes canonical action IDs from instruction text + catalog metadata (`llm_hint`, `labels`, `description`) alone, without requiring any checkbox-label match. Proposals are restricted to the active catalog; out-of-catalog IDs are dropped. For this no-checkbox path, the runtime writes proposed actions back as suggested unchecked checkboxes AND executes them immediately in the same runtime pass (via override_checked=True in _apply_actions). Note: the original intent was to withhold same-pass execution and require explicit human confirmation, but the shipped runtime does execute them immediately; #979 is tasked with hardening this boundary by preventing same-pass execution for governance-bearing-execution capabilities. Fallback to rule mode on LLM error or empty catalog. Tracked by: #241.
...
- PA2-FREEFORM is the catalog-driven proposal path. Proposals returned by the LLM decider for an instruction with no checkboxes are `proposal`-class outputs in intent-space (they originate as LLM-suggested actions without explicit human checkbox); they are written back as suggested unchecked checkboxes AND executed in the same runtime pass via override_checked=True. This is the current runtime behavior; #979 is tasked to harden the boundary by requiring explicit human confirmation (a second approval envelope hop) before same-pass execution for governance-bearing-execution capabilities.
...
- Proposal-only cognitive capability classes (orientation, clarification, retrieval, synthesis) are not yet represented in the action catalog. Adding them is `docs/settings/panel-actions.md` work and is tracked separately; the freeform proposal path gains proposal-class targets to bind to once those entries land. The current `actions: []` emission for a freeform instruction with no proposal-class catalog entries is the correct response under the existing catalog.
