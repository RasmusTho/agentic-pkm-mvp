...
**Concurrency safety.** The offload lets two `evaluate()` calls (the `/readyz` container healthcheck plus companion-UI's `/status` polling) run in parallel, where inline they were serialized on the single event loop. `evaluate()` therefore serializes on a per-contract `RLock` — restoring the pre-offload one-at-a-time semantics without blocking the loop — so concurrent calls cannot interleave the out-of-lock prev-state read that gates incident-log writes and double-write the incident audit log. `HealthStateMachine.update()`/`reset()` additionally hold an internal `Lock` around their counter/state/`transition_history` read-modify-write, as defense-in-depth for any direct caller. Both guards are pinned by deterministic lock-usage tests (`tests/health/test_readiness_dependency_aware.py::test_evaluate_serializes_on_the_contract_lock`, `tests/observability/test_health_state_machine.py::test_update_runs_under_the_instance_lock`).
