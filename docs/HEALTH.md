...
- When a durable vault is selected on the Postgres Product path, `product_readiness` additionally verifies that every retained Markdown source has exactly one matching `store_objects` projection with the RSC-02 replay tuple (source identity, source-content generation, and recipe version) and matching canonical text. An empty or corrupt Product projection therefore remains unready until a source-backed rebuild converges; a no-vault boot and an explicit empty workspace retain their existing semantics.
