...
- When a durable vault is selected on the Postgres Product path, `product_readiness` additionally verifies that every vault-alpha-eligible retained Markdown source has exactly one matching `store_objects` projection with the RSC-02 replay tuple (source identity, source-content generation, and recipe version) and matching canonical text. An empty or corrupt Product projection therefore keeps `/readyz` at 503 until a source-backed rebuild converges; only the named `product.source_backed_rebuild` admission bypasses the write guard for that reconstruction prerequisite, while ordinary writes remain guarded and a no-vault boot or explicit empty workspace retain their existing semantics.
...
heartbeat freshness (plus watcher observation state), so it can still read healthy during a
post-startup DB drop (see its row). `HealthContract.evaluate()` also exposes the same distinction as
`watcher_observation.state` (`healthy-idle` | `catch-up` | `degraded`) with `pending` and `healthy`
booleans; this does not turn watcher catch-up into an outbox/readiness failure. The
