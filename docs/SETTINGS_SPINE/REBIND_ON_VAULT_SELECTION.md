...
Close audit finding F4 (SET-7) and the named gap #3119: `WATCHER_VAULT_PATH` is read once at
watcher boot and never rebinds when a vault is selected through the Companion UI, so a capture can
succeed while remaining invisible to ingest (`docs/ENVIRONMENTS.md:94-108`). More generally,
vault-scoped settings consumers must follow the selection, not the env snapshot.
...
- [ ] SET-7 registered in the invariant registry with enforcement `runtime_test`; #3119 closes
      with this task's receipt.
  - Verify: doc writeback at `docs/testing/invariant-tests.md :: vault_selection_rebinds_consumers`
    + closure receipt on #3119
...
- Multi-active-vault (epic #2143) — exactly one active vault, as today.
...
- `docs/ENVIRONMENTS.md :: ingest binding` · Issue #3119 (extends and closes)

## Related GitHub Issues

One implementation issue, closing #3119. TCD hint: opus / high — concurrency-sensitive rebind on
the ingest hot path; silent-capture-loss blast radius.
