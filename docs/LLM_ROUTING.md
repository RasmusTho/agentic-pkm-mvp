...
- Importing the top-level `app` package is configuration-neutral. The canonical Product fabric
  enforces a present `LLM_PROVIDER` when `LLM_PROVIDER_ENFORCE=1` as `LLMRouter` is constructed;
  legacy Product LLM and embedding access seams retain the equivalent
  `app.config.llm.get_provider()` → `ensure_provider()` check. Provider execution therefore fails
  loud without making unrelated neutral-contract or Builder imports resolve Product configuration.
  An explicit provider override cannot substitute for an absent enforced provider or conflict with
  the enforced provider identity.
