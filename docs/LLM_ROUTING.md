...
- Importing the top-level `app` package is configuration-neutral. Product provider enforcement occurs
  when a Product LLM or embedding access seam calls `app.config.llm.get_provider()` (which delegates
  to `ensure_provider()`), so `LLM_PROVIDER_ENFORCE=1` still fails loud before provider execution
  without making unrelated neutral-contract or Builder imports resolve Product configuration.
