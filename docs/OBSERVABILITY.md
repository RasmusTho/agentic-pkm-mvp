...
- Registry watcher observation state (#5211): top-level `status` is `healthy-idle` after a deterministic scan generation drains, `catch-up` while a durable traversal cursor has unfinished work, and `degraded` only for an actual error, pause, blind scope, or repeated no-progress. Each watcher entry carries `observation_status`, `scan_in_progress`, `scan_generation`, and an optional `continuation_reason` (`file_budget` | `byte_budget` | `elapsed_budget`). Reaching a configured resource budget is normal catch-up and does not increment bad ticks or create `WATCHER_STOP` while cursor progress continues.
- Registry watcher scope visibility (#2988): each watcher entry also carries `scope_status` (`ok` | `zero_match` | `missing_prefix`). Zero-match is decided only when a full scan generation drains, so a partial batch containing no scoped content cannot create a false blind-scope alarm. A genuinely empty scope or missing static scope-prefix degrades the top-level heartbeat, with a warning logged at most once per `WATCHER_SUMMARY_INTERVAL`.
- The registry checkpoint JSON stores only bounded traversal/counter metadata. Per-file observations live in the adjacent `watcher_state_<name>.observations.sqlite3` sidecar and commit before the atomic cursor checkpoint; restart replay is therefore safe without serializing the full observed vault every tick. The legacy snapshot watcher keeps its existing JSON state path.
