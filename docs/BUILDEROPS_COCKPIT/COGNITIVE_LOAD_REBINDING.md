...
| Decision modes | Decision mode | Tri-state gate banding: the typed gate status decides *whether* a thread may demand owner attention at all (working / done / flawed / forgotten / needs-you). Ambiguous or unmapped technical status fails closed to unclassified/flawed and is presented as blocked by evidence/system, never as needs-you. The needs-you band requires an explicit canonical Human Exception authority category (normally a valid `agent:needs-human` classification). The risk-meter ordering (`docs/BUILDEROPS_COCKPIT/DESIGN_DECISIONS.md :: EXT-7`) orders threads only *within* one band and never moves a thread across bands. |
