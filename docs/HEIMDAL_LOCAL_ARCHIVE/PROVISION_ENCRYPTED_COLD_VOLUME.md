...
1. Choose an existing external volume root, a separate empty mountpoint, a stable archive identifier,
   ...
   values out of Git. Write the closed metadata document with the exact channel (`dev`, `test`, or
   `prod`), `generation` zero, state
   ...
   environment file. The declaration must be the canonical path spelling and may not traverse a
   symlink. Do not choose or invent an APFS UUID: the provisioner binds the one generated by the
   image tool. A legacy metadata document without `channel` refuses; no startup or replay path
   fabricates that authority.
