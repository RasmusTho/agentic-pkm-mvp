...
1. Choose an existing external volume root, a separate empty mountpoint, a stable archive identifier,
   ...
   values out of Git. Create the closed, canonical generation-zero metadata document with the
   module's `initialize` action; hand-authored or reformatted JSON refuses before any host command:

   ```bash
   python3 -m app.ops.heimdal_cold_volume initialize \
     --channel <dev-or-test> --metadata <absolute-metadata-path> \
     --archive-id <stable-id> --bundle-path <absolute-sparsebundle-path> \
     --mountpoint <absolute-empty-mountpoint> --parent-volume-uuid <external-volume-uuid> \
     --capacity-bytes <aligned-byte-capacity> --owner-uid <uid> --owner-gid <gid>
   ```

   The initializer writes `planned-unbound`, null `bundle_inode`, `image_metadata_sha256`,
   `filesystem_capacity_bytes`, and `volume_uuid`, mode value `448` (`0700`), and file mode `0600`.
   Set its absolute path as
   the single `HEIMDAL_ARCHIVE_METADATA_FILE` declaration in the ignored matching channel-local
   environment file. The declaration must be the exact on-disk path spelling (including case and
   Unicode normalization) and may not traverse a symlink. Do not choose or invent an APFS UUID: the
   provisioner binds the one generated by the image tool. A legacy metadata document without
   `channel` refuses; no startup or replay path fabricates that authority.
...
3. Run provisioning through HSP, which owns and removes the temporary credential surface:
   ...
   python3 -m app.ops.host_secret_bootstrap \
     ...
     --channel <same-dev-or-test> --config-root "$PWD"
   ```
   ...
   returned.
4. For a later mount, use the same HSP wrapper and identical channel/config-root arguments with the
   `mount` action. HSP binds the selected channel and consumer into the child handoff; a mismatch
   refuses before credential use. To detach, first stop the
