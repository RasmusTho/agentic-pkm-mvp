...
- `issue-to-code` owns vault-first claim (`Ready` -> `In Progress`, active claim file, then remove
  strictly validated `agent:ready`) to prevent double-pick. Workspace preflight runs before the
  vault claim; the verified dispatcher/claimant-receipt path remains the transition fallback when no
  vault ticket exists.
...
- prefer unclaimed vault tickets with folder/YAML `status=Ready` linked to Issues carrying a
  strictly validated `agent:ready` label; Project Status is not a pickup gate
...
- Strictly validated `agent:ready` qualifies an Issue externally; vault-backed pickup additionally requires an unclaimed `Ready` ticket.
