...
use evidence-based convergence across the whole PR: the ledger retains every round, TCD selects
capability, every substantive repair receives a fresh independent review, and the loop stops only on
documented non-progress, technical impasse, scope expansion, or authority conflict rather than a
fixed attempt count.
