...
- The top-level `app/__init__.py` package initializer is shared import-chain configuration and therefore selects the broad deterministic suite rather than a single subsystem.
...
- The neutral `app/llm_contract/**` leaf is owned by the `model_access` selector. A kernel change runs its direct contract suite, the BuilderOps adapter/runner compatibility suites, and the exact neutral-kernel/import-boundary architecture guards.
- Every `llm_eval` match includes the bounded Product provider-enforcement inventory so a new or changed execution seam cannot silently bypass the shared enforced-provider check while narrower LLM tests remain green.
