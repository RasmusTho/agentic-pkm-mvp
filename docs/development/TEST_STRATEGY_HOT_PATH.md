...
- PR unit CI uses `scripts/select_pr_tests.py` to map changed files to subsystem-scoped pytest targets. Shared CI/test configuration, migrations, dependencies, and shared fixtures run the deterministic broad suite; E2E coverage is deferred to post-merge and nightly validation. This document is `scripts/select_pr_tests.py`'s intended `docs/development/` contract for the `scripts/docs_guard_logic.py :: GOVERNANCE_TEMPORAL_ENFORCEMENT` temporal-owner-doc exemption: update it, not `docs/STATUS.md`/`docs/ROADMAP.md`/etc., when the selection script's behavior changes. The exemption's current check accepts any `docs/development/` touch rather than enforcing this specific pairing; tightening it to an explicit per-script mapping is tracked as a follow-up, not done here.
