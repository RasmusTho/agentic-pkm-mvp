...
- The `archival_contracts` subsystem owns `app/archival/`, its focused contract tests, and the governed archival contract surface. Its selector mapping is intentionally narrow: GAF-01's provider-free vocabulary must run `tests/archival` and the exact architecture contract test without claiming production-adapter coverage.
