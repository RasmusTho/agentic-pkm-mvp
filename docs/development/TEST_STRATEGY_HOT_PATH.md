...
- The neutral `llm_contract/**` leaf (top-level package, outside `app/` — issue #4290 change-control correction) is owned by the `model_access` selector. A kernel change runs its direct contract suite, the BuilderOps adapter/runner compatibility suites, and the exact neutral-kernel/import-boundary architecture guards.
