...
| verification dispatch consumer | implemented (repo contract; host enablement receipt required) | `app/dispatcher/verification_dispatch.py`, `app/dispatcher/verification_consumer.py`, `app/dispatcher/verification_agent_loop.py` | Revalidate artifact requests, claim one PR/head chain, run `verification_closer`, atomically replay/dedupe final review events, and retain restart-safe requested/current/verified-head plus 2+2/review/terminal state | `verification_dispatch_request.v1`, live PR/check truth, ChatGPT/keyring auth preflight | Minimal context pack, Codex thread id, attempt ledger, lease-fenced head rebind, backoff or terminal receipt | Dispatcher-local operational state and host-local `codex exec`; all GitHub mutation remains delegated to `verification-and-closure` | [issue #3603](https://github.com/RasmusTho/agentic-pkm-mvp/issues/3603), [docs/AGENT_ISSUE_DISPATCHER.md] |
...
| verification dispatch consumer | host-local bounded poll/single-run | verification coordinator launcher | request artifact, live PR/check state, dispatcher run ledger, ChatGPT/keyring preflight | authenticated `gh` reads, dispatcher SQLite, isolated worktree, `codex exec` | GitHub Actions model execution, API-key fallback, secret logging, alternate merge path | restart-safe coordinator/review/repair/terminal receipts | stale head, duplicate launch, auth/rate-limit drift | one-chain default; host enablement and pilot receipt before broad rollout |
