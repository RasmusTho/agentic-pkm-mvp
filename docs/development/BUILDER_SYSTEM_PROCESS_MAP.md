...
| verification dispatch producer | implemented (artifact-only) | `.github/workflows/verification-dispatch-request.yml`, `scripts/build_verification_dispatch_request.py` | Emit one versioned, idempotent request after successful `CI` for the current PR head | Completed `CI` workflow run plus live PR snapshot carrying exactly one explicit `Governing-Issue`, exact closing identities, then that issue's live snapshot; non-governing references remain supporting evidence | `verification_dispatch_request.v2` JSON/Markdown artifact | GitHub Actions artifact upload only; ambiguous or mismatched governing/closing authority emits no request; no agent, merge, issue, label, comment, or dispatcher mutation | [issue #3602](https://github.com/RasmusTho/agentic-pkm-mvp/issues/3602), [`.github/workflows/verification-dispatch-request.yml`], [scripts/build_verification_dispatch_request.py] |
...
| post-merge docs/spec classifier | partially_implemented | `post-merge-owner-doc` skill, watchdog workflow | Decide owner-doc update/follow-up/no-change | Merged PR diff plus authenticated closing/governing targets | Docs PR, follow-up issue, or PR-specific receipt on every closed child and distinct open governing parent; issue-free receipt on PR | Agent/GitHub Action nudge | [`.codex/skills/post-merge-owner-doc/SKILL.md`], [`.github/workflows/post-merge-owner-doc-watchdog.yml`] |
...
| merge gate | Verification complete | Agent | PR + issue + CI + v2 closer context | verification-and-closure | verification-and-closure | `await_pr_checks.sh`, verified merge preparer, REST/GraphQL | exact-head merge/block plus authority receipt | GitHub | CI/review/exact-child ACs/governing issue-set/body neutralization | eligible? race? | repair or closure reconciliation | no merge before mutation; no delivery receipt after unresolved race | non-waivable gate | [`.codex/skills/verification-and-closure/SKILL.md`], [`app/dispatcher/verified_merge.py`] |
...
watchdog trusts a unique same-head authority receipt during the temporary neutralized-body window and
does not let a generic or different-PR receipt satisfy the check [`.codex/skills/post-merge-owner-doc/SKILL.md`], [`.github/workflows/post-merge-owner-doc-watchdog.yml`].
...
| workflow_run completed/success for `CI` | `verification-dispatch-request` | Current-head `verification_dispatch_request.v2` producer with exact closing authority | contents read, pull-requests read, issues read | artifact-only | Mac mini consumption or verification/closure action remains outside GitHub Actions |
