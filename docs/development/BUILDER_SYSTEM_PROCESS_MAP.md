...
3. Contract layer: GitHub issues, PR templates, shared skill contracts, labels, Builder Ops Vault tickets/claims, `Verify:` markers, and SBS impact blocks define executable work [`.codex/skills/_shared/ISSUE_CONTRACT.md`:12-72], [`.github/ISSUE_TEMPLATE/task.yml`:73-109], [docs/development/BUILDER_OPS_VAULT_QUEUE.md].
4. Dispatch/routing layer: Builder Ops Vault queue/claims, verified dispatcher transition fallback, strictly validated labels, skill routing, model/reasoning policy, and worktree isolation select work and prevent collisions; Project status is projection evidence only [docs/development/BUILDER_OPS_VAULT_QUEUE.md], [docs/AGENT_ISSUE_DISPATCHER.md:132-180], [AGENTS.md:171-182].
...
6. Verification/evidence layer: local validation, CI, REST-only check waiting, local review gate, delivery receipts, vault validation, and owner-doc receipts prove work [`.codex/skills/verification-and-closure/SKILL.md`:46-77], [`.codex/skills/_shared/CI_WAIT_CONTRACT.md`:22-82], [docs/development/BUILDER_OPS_VAULT_QUEUE.md].
...
| issue queue | partially_implemented | Builder Ops Vault Markdown/YAML tickets, transient claim files, dispatcher transition fallback, GitHub labels | Expose ready work | Vault `Ready` tickets without active claims; fallback strictly validated GitHub `agent:ready` issues | Queue entries/ticket paths | Vault files via `builderops vault`; dispatcher store during transition | [docs/development/BUILDER_OPS_VAULT_QUEUE.md], [docs/AGENT_ISSUE_DISPATCHER.md:132-180] |
| dispatcher | implemented (transition fallback) | `app/dispatcher/**`, `docs/AGENT_ISSUE_DISPATCHER.md`, Makefile targets | Existing queue, claim, lease, heartbeat, completion bridge | GitHub `agent:ready` issues | Local tasks/leases/events | Local dispatcher DB only | [docs/AGENT_ISSUE_DISPATCHER.md:21-36], [Makefile:356-361], [app/dispatcher/cli.py:31-32] |
...
| labels affect readiness | deterministic | strictly validated `agent:ready` is the external pickup qualifier; `agent:blocked` and `needs-human` are non-active | [`.codex/skills/_shared/LABEL_TAXONOMY.md`] |
| Vault status affects routing | deterministic local file contract | Vault folder + YAML `status=Ready` qualifies pickup when no active claim exists; GitHub Project is deprecated hot-path projection | [docs/development/BUILDER_OPS_VAULT_QUEUE.md] |
| work is claimed | deterministic | Vault claim then GitHub label removal; verified dispatcher/GitHub-label fallback when no vault ticket exists | [`.codex/skills/issue-to-code/SKILL.md`:133-215] |
...
| Is ticket agent-ready? | strict Issue validation + label + vault status + claim file | yes | yes | no | body/labels plus folder/YAML status and `.builderops/claims` | queue eligible | malformed issue or vault validation error | [`.codex/skills/_shared/ISSUE_CONTRACT.md`], [docs/development/BUILDER_OPS_VAULT_QUEUE.md] |
