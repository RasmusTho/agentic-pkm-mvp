...
| BuilderOps Model Inquiry | Question -> immutable artifacts/model turns -> configured-Sol `single_target_acceptance` with `independence: false` -> synthesis -> deterministic `issue_ready` / `needs_input` / `not_ready` result -> file-first `PromotionIntent` | Required by the specialized inquiry promotion contract before any remote call. The intent binds the accepted run-terminal, readiness, and synthesis hashes; legacy consensus remains readable but is not an active routing mode. | Only the explicit `builderops inquiry promote <id> --create-issue` path may reconcile or create the Issue through REST and append the Issue receipt. |
