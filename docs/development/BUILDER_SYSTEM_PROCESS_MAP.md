...
| validation runner | partially_implemented | Makefile, scripts, CI, `DEV_WORKFLOW` | Run local and CI checks | Changed files | Logs/status | Local/CI | [docs/development/DEV_WORKFLOW.md:60-83], [`.github/workflows/ci-smoke.yaml`:17-104] |
...
| workflow_run completed/success for `CI Smoke` | `verification-dispatch-request` | Current-head `verification_dispatch_request.v2` producer with exact closing authority | contents read, pull-requests read, issues read | artifact-only | Mac mini consumption or verification/closure action remains outside GitHub Actions |
