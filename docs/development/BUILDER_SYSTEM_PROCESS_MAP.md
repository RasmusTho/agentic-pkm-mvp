...
| verification dispatch producer | implemented (artifact-only) | `.github/workflows/verification-dispatch-request.yml`, `scripts/build_verification_dispatch_request.py` | Emit one versioned, idempotent request after successful `CI` for the current PR head | Completed `CI` workflow run plus live PR snapshot carrying exactly one explicit `Governing-Issue`, then that issue's live snapshot; supporting issue references remain evidence only | `verification_dispatch_request.v1` JSON/Markdown artifact | GitHub Actions artifact upload only; ambiguous or mismatched governing-issue authority emits no request; no agent, merge, issue, label, comment, or dispatcher mutation | [issue #3602](https://github.com/RasmusTho/agentic-pkm-mvp/issues/3602), [`.github/workflows/verification-dispatch-request.yml`], [scripts/build_verification_dispatch_request.py] |
