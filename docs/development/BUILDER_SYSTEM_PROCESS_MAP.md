...
4. Dispatch/routing layer: Builder Ops Vault queue/claims, dispatcher transition fallback, labels, skill routing, model/reasoning policy, and worktree isolation select work and prevent collisions [docs/development/BUILDER_OPS_VAULT_QUEUE.md], [docs/AGENT_ISSUE_DISPATCHER.md:132-180], [AGENTS.md:171-182].
...
| Is ticket agent-ready? | vault status + claim file | yes | yes | no | folder/YAML status and `.builderops/claims` | queue eligible | validation error | [docs/development/BUILDER_OPS_VAULT_QUEUE.md] |
