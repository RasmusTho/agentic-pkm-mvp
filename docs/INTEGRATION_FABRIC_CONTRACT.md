...
1. Name the **integration classes** Mimer composes with, so attachment work is bounded and legible.
...
3. State the **authority rule** that separates "external thing providing capability, transport, inference, or interface" from "external thing being promoted into Mimer's semantic authority."
...
Mimer composes with eleven integration classes. Each class is a category of external component that crosses a Mimer boundary. A given concrete integration is typically an instance of one class (for example, a specific LLM vendor SDK is a Model provider).

A concrete integration may legitimately participate in **more than one class** when it plays more than one role at the same Mimer boundary. Obsidian is the canonical example: it is both the **Human surface** the human writes in (the durable surface where vault Markdown is authored) and the **External UI shell** that hosts Mimer's in-note Panel surface and consumes the runtime API. When an integration spans classes this way, every applicable class's contract fields (allowed role, authority limits, persistence class, provenance requirement, event boundary, health/observability expectation, replacement strategy) must be answered for the integration's behavior in that role. The classes do not blend; the integration answers each contract surface in turn.
...
8. **External UI shell** — external interactive shell that hosts Mimer surfaces or consumes its API (Obsidian as host, companion-UI implementations, future operator dashboards, third-party UIs). Distinct from "human surface" because the UI shell is the runtime container, not the human itself.
...
| External UI shell | Interface (runtime container) | Not semantic authority; container does not decide meaning | None inside Mimer (shell-local state is opaque) | Through the HTTP API and event envelope | `docs/INTERACTION_SURFACES_AND_AUTHORITY/README.md`, `docs/COMPANION_UI_PRODUCT_SPEC.md` |
| Observer / telemetry source | Interface (read-only) | Read-only; never writes back to vault or runtime state | None inside Mimer (telemetry destination owns its own storage) | Through status/health endpoints and emitted observability events | `docs/OBSERVABILITY.md`, `docs/HEALTH.md`, `docs/STATUS.md` |
...
The authority rule is the kernel-level constraint that distinguishes the integration fabric from the rest of Mimer.
...
- an `Authority rule` section that states external components may provide capability, transport, inference, or interface but must not become semantic authority unless promoted through Mimer contracts.
