...
7. **Multi-agent reads becoming coordination truth.** Agents may later consume the same read-only Workspace Orientation projection under ADR-0012, but those reads must not create shared mutable workspace state, A2A routing, orchestration state, leases, checkpoints, agent messages, hidden subscriptions, or per-agent semantic truth.
