...
when a child re-enters through the wrapper it must prove that the inherited file descriptor itself
owns the lock, not merely that some descriptor for the same path is locked.
