...
- UI → `POST /api/companion/vault/settings` (surface=`'api'`)
- CLI → existing `app.cli vault` commands (surface=`'cli'`)
- File edit → watcher-detected `settings/local.md` delta (surface=`'file'`; durable store is
  a follow-on)
...
## Audit blind-spot closed

A direct hand-edit of `settings/local.md` previously produced no receipt — only the watcher
picking it up at next start. The receipt must cover the **file-originated** path (watcher-detected
settings delta) in addition to the UI/API path, or we instrument only one of two equivalent doors.
The file-originated receipt is currently logged at INFO level; a durable store is a follow-on.
