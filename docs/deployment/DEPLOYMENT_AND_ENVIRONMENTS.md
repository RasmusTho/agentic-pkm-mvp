...
4. **Health gate: liveness first, readiness second.** Block until the channel's API `/healthz` returns `{"ok": true}` (`app/api/routes/health_contract.py`) and then verify readiness evidence via `/readyz` and/or `/api/health` on the channel's ports. `/healthz` is only a liveness probe; deploy completion requires readiness evidence that startup dependencies, DB connectivity, and the deployed code path are actually usable. The gateway's own `/healthz` must also respond. A deploy is not "done" until both liveness and readiness pass; a failing gate triggers §Rollback.
...
4. **Health gate + record.** Re-run the §Deploy liveness/readiness gate, confirm `/readyz` and/or `/api/health` return the expected healthy state, and confirm `/version` now reports the rolled-back SHA. Record the rollback receipt.
