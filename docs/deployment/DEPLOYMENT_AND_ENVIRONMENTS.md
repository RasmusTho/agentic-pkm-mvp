...
**Decision: only a loopback-local reverse proxy may supply trusted `X-Forwarded-For` for Companion vault-route auth decisions; untrusted callers stay rejected.** A raw docker bridge/gateway or other NAT hop is not a trusted proxy boundary. Host networking, or an equivalent authenticated/sanitizing proxy boundary, is the accepted alternative where a loopback-local proxy is not in front. This preserves the #2223 intent that state-changing companion API routes reject untrusted non-loopback callers.

Why it is needed: the API trusts loopback callers (`require_loopback_or_api_key` in `app/auth.py`), but a raw bridge/gateway peer is only a transport hop, not an HTTP proxy that overwrites or strips inbound `X-Forwarded-For`. Treating that bridge address as trusted would let a direct caller spoof `X-Forwarded-For: 127.0.0.1` and bypass the non-loopback guard on vault browse/select/initialize.

How it is implemented: `_effective_client_host()` in `app/auth.py` reads the first `X-Forwarded-For` hop **only when the immediate peer is loopback**. That preserves the same-host Companion UI proxy path, while a direct external caller or bridge/gateway peer is judged by its own address and cannot spoof loopback by setting the header. Issue **#2706** hardens and documents this posture explicitly:
- The trusted-proxy boundary must be a loopback-local proxy, host networking, or an equivalent authenticated/sanitizing proxy boundary. A raw bridge/gateway hop is not sufficient.
...
- S6 verification is locked by `tests/api/test_auth_proxy_topology.py` and `tests/api/test_companion_auth_loopback_behind_proxy.py`: a loopback-local proxy may assert the client via `X-Forwarded-For`, while a genuinely non-loopback caller or unsafe bridge peer with forged `X-Forwarded-For` remains rejected.
...
- **S6 — Verify/formalize auth↔topology.** Verify and lock the loopback-only forwarded-client trust boundary; add/confirm tests that exercise the proxied path and assert untrusted non-loopback callers and unsafe bridge peers are still rejected (#2223, #2706). Target: `app/auth.py` (formalize/comment), `tests/**` covering `require_loopback_or_api_key` + `_effective_client_host` on the runtime path.
