...
After the channel pin is mutated, every terminal failure — image pull, migration execution, service recreate/liveness, readiness, version identity, fleet-model fitness, Companion UI smoke, capture-watch health, or receipt creation — uses one fail-closed recovery path. That path preserves the failing gate's original non-zero status and diagnostics. Before forward-only migration execution begins it restores the previous pin and attempts to recreate the prior service set. Once execution begins, a non-zero or lost container result is treated as possibly committed: automatic code rollback is forbidden unless unchanged database revision or prior-image compatibility is explicitly proven; the hot path retains the target pin for a compatible forward fix rather than booting a potentially stale producer against a possibly migrated database.
