...
After the channel pin is mutated, every terminal failure — image pull, service recreate/liveness, readiness, version identity, fleet-model fitness, Companion UI smoke, capture-watch health, or receipt creation — uses one fail-closed recovery path. That path preserves the failing gate's original non-zero status and diagnostics. Without an applied forward-only migration it restores the previous pin and attempts to recreate the prior service set. Once an acknowledged forward-only migration has been applied, automatic code rollback is forbidden unless prior-image compatibility with the new schema is explicitly proven; the hot path retains the target pin for a compatible forward fix rather than booting a potentially stale producer against the migrated database.
