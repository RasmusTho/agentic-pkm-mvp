...
|---|---|---|---|---|---|
| 1 | [READINESS_REFLECTS_DEPENDENCIES](READINESS_REFLECTS_DEPENDENCIES.md) | OBSSTAB-01 | Fold a live DB ping into `HealthContract.evaluate()` (forcing `unhealthy`, not `degraded` — `degraded` is a ready state) so `/readyz` flips on a Postgres outage; repoint the container healthcheck from `/healthz` to `/readyz` across all three URL sources. Active-LLM-as-readiness-gate deferred to #2621. | R1,R5 | P0 |
...
| 4 | [SCHEDULED_PROBE_AND_PUSH_ALERT](SCHEDULED_PROBE_AND_PUSH_ALERT.md) | OBSSTAB-04 | A host launchd job that probes `/readyz` + `/api/health required_ok` and emits one down alert on the outage transition, then one recovery signal on the first healthy run; relabel `verify-*-channel`. | R3,R20 | P0 |
...
1. **Health-truth invariant.** After `OBSSTAB-01` and `-02` merge, **no always-on signal** (container status, `/readyz`, the worker/watcher container probe) may report healthy while a hard dependency is down: **Postgres** for any service. (`/healthz` is intentionally exempt — it is the unconditional *liveness* probe; `/readyz` is the dependency-aware *readiness* probe.) Active-LLM reachability is **not** a Fas 0 readiness gate (deferred to #2621); LLM-down is surfaced via the Minne group, not `/readyz`. The false-green register in `OBSSTAB-06` must enumerate every always-green HTTP surface (`/healthz`, `/agent/health`, the UI `/healthz`) and stay consistent with `-01`/`-02`.
...
- [ ] A sustained prod outage emits one down alert and then one recovery signal on the first healthy run. Verify: `tests/ops/test_prod_probe_alert_state.py::test_sustained_outage_alerts_once_and_recovers`
