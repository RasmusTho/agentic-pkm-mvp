...
| Mirror census remains synchronized after outbox-related edits | `tests/properties/test_event_completeness.py::test_mirror_census_is_closed` | Current AST census has no unregistered or stale production sites; promotion consumer is registered at line 98 and watcher hydration at line 358 (`103` census + selector tests passed) |
| Cross-module status readers remain test-isolated | `tests/api/test_status_api.py`, `tests/observability/test_status_service.py`, and `tests/observability/test_status_bounded_reads.py` | Full selection exposed a leaked module-global outbox path; tests now restore the patched path with `monkeypatch`, and the ordered regression set passes (`30 passed`) |
...
| `e976725fbfb3145296d3f9aace8b6d723a6f356e` | Current-head CI failed before pytest selection because four changed diagnostic surfaces were unowned by `scripts/select_pr_tests.py`; the same run also reported the mirror-census fault-injection gate failure. | Routing repair is staged in the next candidate: shared `app/cli.py` selects the broad suite, events-doctor/legacy outbox select `events_receipts`, and orientation selects `observability`. The remaining P-2 stale anchors are separately repaired at the next candidate and both gates must be rerun before closure. |
| `f5f857ffd5fa00d903dc6e99a8c9af7f2d325b31` | Local full non-PG selection reached pytest and reported `14,476 passed`, `50 skipped`, `18 xfailed`, and 8 failures: one status-path isolation defect and seven environment/platform-dependent subprocess failures (`python`/`docker` unavailable or required optional runtime). | Status test isolation is repaired in the next candidate and passes its ordered 30-test regression set. The seven environment-dependent cases remain non-claims until current-head CI or a matching runtime environment proves them. |
| `51d862248e656b63e8bc1f7e825ae64d65765201` | Current-head CI reached the full non-PG suite but the three health API tests reported `required_ok=False`; the payload identified the required `index_outbox` check as missing because the fixture only created its parent directory. | The fixture now creates the explicitly expected empty outbox. The read-only health behavior remains unchanged, the missing-path regression remains fail-closed, and the health API/CLI regression set passes locally (`22 passed`). A fresh current-head CI run is required after publication. |
