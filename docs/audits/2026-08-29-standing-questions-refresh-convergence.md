...
- Receipt event IDs are globally unique across all JSONL writers and event types; conflicting
  duplicates fail closed under one shared append lock, including the legacy index writer and
  real-path/symlink aliases. The seam refuses unreadable or malformed existing JSONL and repairs
  only an unambiguous complete final record that lacks its delimiter.
...
- The deterministic proposal pair carries an integrity-first sidecar and an atomically replaced
  ...
  replayable.
- JSONL receipt read/check/append is serialized per canonical outbox path, so the global event-id
...
| Focused SQ/Create regression set | Standing Questions, evidence matching, Create lifecycle, QuestionStore, and JSONL outbox contract tests | `165 passed` in the current run; the preceding packet recorded `148 passed` before these additions |
