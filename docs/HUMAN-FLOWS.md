...
- Markdown mutations (panel cleanup, receipts, promotion frontmatter) flow through the note writer path; agents emit intents and the writer applies deterministic transforms.
...
- Auto-panel policy (v5.3 delivered in CLI): watcher-driven panel runs now treat any note containing an AI panel fence (`%% ...ai... %%`, case-insensitive) as a candidate once the global arm switch `WATCHER_AUTO_EXEC=1` is set. Frontmatter only opts out via `ai_panel_auto_run: never` or the nested `ai_panel: { auto_run: never }` form, while other modes (`watcher`/`manual`) remain for manual CLI metadata without gating watcher eligibility. Manual CLI (`panel run` / `panel run-many`) is still always allowed.
- Hardening (v5.4): `vault-watcher-run` supports `--dry-run` (no ingest/panel, shows what would run) and `--max-notes` with optional `--force` to prevent storming on large change sets. Summaries report changed notes, ingest attempts, panel candidates/runs, skips, and errors. Watchers stay bounded by the global arm switch and the per-note opt-out policy.
