State: **UNREVIEWED DRAFT.** Specification directory (design + bounded tasks), authored 2026-07-27 from accepted ADR-0064 by a decomposition pass whose review was interrupted and never completed. Published 2026-07-29 to preserve the work and its backlog reconciliation, not because it is accepted. No child issue is filed and no implementation is authorized by this directory. Decomposes migration steps 1-5 only. **Reconcile these four changes before acting on it:** (1) HSP-02 was already delivered (#3846 / PR #4008) — the stale task-order row this directory reacted to was corrected at source in PR #4190, so MAS-03's README writeback is largely done; (2) `docs/audits/MODEL_ACCESS_SUBSTRATE_2026-07-27.md` §8.1 is superseded — the CKM migration order does **not** swap and stays at step 5; (3) ADR-0057 A1 admits CKM-driven orchestration, so this directory's assumption that CKM must not orchestrate during the migration window no longer holds; (4) the confirmed-only gap-detection claim it may rely on was withdrawn in PR #4255 and #4261.
