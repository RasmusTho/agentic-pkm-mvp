State: Accepted (owner decisions, 2026-07-04; RESEARCH-08 D1 + Heimdall structure decisions OD-1/OD-3/OD-7). Ratifies the acknowledged System-of-Systems structure from the 2026-07-04 Fable-5 structure pass (`docs/architecture/ECOSYSTEM_STRUCTURE_PROPOSAL.md`, PR #2914): the ecosystem apex is named **Yggdrasil** (the whole); its constituents are **Mimer** (knowledge-and-cognition, undivided — the current system, reverting to its original name), **Heimdall** (sensor), and a thin **private-bindings** constituent. **Supersedes the naming half of ADR-0043**: ADR-0043's Munin (knowledge/memory) / Hugin (agent-runtime) two-constituent split is replaced by the single undivided **Mimer** constituent, with **Hugin/Munin reserved**; ADR-0043's Heimdall-sensor and observability→OEF decisions are retained. Enactment (the `yggdrasil_runtime/`→Mimer code/doc rename, glossary reconciliation, entity-register + substrate contracts) is deferred to follow-ups; this ADR performs no rename.
Doc role: Decision record (ADR)
Authority: Authoritative for the ecosystem's top-level structure and constituent-naming decision (the apex, the constituent set, and the "what does Yggdrasil denote" question), superseding ADR-0043 on constituent naming only. It does not redefine any constituent's internals (Mimer keeps the 14 control boundaries + CES + correctness kernel; Heimdall keeps its A-doc charter). SoS scope and naming are owner-reserved (R-SOS/R-NAME) and are recorded here as the owner's locked decision, not an agent's.
...
Source of truth: This ADR plus `docs/architecture/ECOSYSTEM_STRUCTURE_PROPOSAL.md` (the Fable-5 pass + locked OD-1..9), `docs/adr/ADR-0043-heimdall-naming-and-norse-name-register.md` (superseded in part), `docs/HEIMDALL/**`, and `docs/architecture/ecosystem-federation.md` § Owner decisions (D1).
...
- **#2886** (`docs/HEIMDALL/**` + ADR-0043) established the Heimdall sensor constituent and captured
  the owner's first-pass naming (**D-NAME-WHOLE**): **Yggdrasil = the whole**, constituents **Munin**
  (knowledge/memory), **Hugin** (agent-runtime), **Heimdall** (sensor); "federation" avoided (SFC
  owns it).
...
### 2. Constituents: Mimer, Heimdall, private-bindings (Hugin/Munin reserved)
...
- **Heimdall** — the sensor / event-capture constituent (unchanged from ADR-0043 + `docs/HEIMDALL/**`).
...
constituent; Hugin/Munin are reserved. **Retained from ADR-0043:** Heimdall = sensor, the
...
- No constituent's internals are redefined; Mimer keeps the 14 boundaries + CES + correctness kernel;
  Heimdall keeps its A-doc charter.
...
- The ecosystem has one coherent structure: **Yggdrasil** (whole) ⊃ { **Mimer**, **Heimdall**,
  private-bindings }, on a Layer-2 platform substrate. RESEARCH-08 D2/D3/D4 (ADR-0045/0046/0047) read
  through it.
- ADR-0043 is **superseded in part** (naming / constituent register); its Heimdall/OEF half stands.
...
- `docs/adr/ADR-0043-heimdall-naming-and-norse-name-register.md` — superseded in part (naming); Heimdall/OEF half retained.
- `docs/HEIMDALL/ECOSYSTEM_SOS_MODEL.md` (A1), `docs/HEIMDALL/OWNER_DECISIONS.md` (A4).
