State: Accepted (owner decision, 2026-07-10). Formalizes the Event Horizon decay model: relevance decay as the retrieval consequence of Episode closure. Downstream of ADR-0051 (Episode primitive) and ADR-0054 (Episode Resolution Engine); locks the model *shape* while leaving the decay curve parameters owner-open runtime tuning (RQ3). Implementation home: ERE-06 (#3181, `docs/EPISODE_RESOLUTION_ENGINE/EMIT_CLOSURE_AND_DERIVE_DECAY.md`). **Amended 2026-07-11** (owner-directed definitional iteration — Fable-5 investigation + external GPT-5 review; recorded in `docs/research/EPISODE_AS_ONTOLOGICAL_PRIMITIVE.md :: Addendum 2026-07-11`): four refinements folded in, each marked inline — episode-near class gate (§2), credible-binding veto (§3), derived closed-by-ancestor (§1), resumes-over-reopen engine policy (§4). The accepted model shape (closure-triggered, non-destructive, derived) is unchanged; the class gate narrows what is dampenable and routes its concrete list to the ADR-0055/T2 note-class table (#3131).
...
Closure propagates **downward through nesting, never upward** — and (amended 2026-07-11) as a *derived* effective status, never a rewrite: closing a parent episode (`the workday`) gives its open children (`the standup within it`) the effective state `closed-by-ancestor`, computed at read time from the ancestor chain; each child's own `closed` field is untouched, so re-opening the parent restores its children with no ceremony and no history rewrite. A child closing says nothing about its parent, and re-opening a child re-opens nothing above it. Retrieval still reads one bit per episode — resolved through the ancestor chain, consistent with §4's derived-never-stored rule (a destructive child rewrite would make re-opening and history illegible).
...
- **Dampening applies to episode-near artifact classes only** (amended 2026-07-11): raw and working material whose relevance genuinely lives and dies with its situation — sensor/Sources-zone captures (#3413), transcripts, chat/working notes, ephemeral plans. Canonical knowledge artifacts (accepted decisions, evergreen knowledge notes, project status) are exempt (factor 1.0) even when episode-bound: their liveness follows their own properties, per ADR-0051 commitment 5 (episodic → semantic is transformation, and the transformed artifact carries its own standing). Without this gate, every closed meeting would dampen the still-valid decisions authored inside it — closure would leak from the situation onto its durable products. This makes Edge case 4's reasoning concrete rather than aspirational. The concrete class list is decided in the ADR-0055/T2 note-class table (#3131) and referenced here (§Owner-open Q6).
...
- **Only a credible binding keeps an artifact hot** (amended 2026-07-11): the MAX rule counts an *open* binding at full strength only when the binding is accepted or a matured candidate. A lone speculative machine hypothesis — fresh, low-confidence, never matured — must not indefinitely block dampening of large material sets; binding confidence/type weighs into whether an open episode holds its artifacts at 1.0. The maturation threshold is engine tuning (ERE design space). This is the mirror of the fail-open row above: unresolvable refs fail open to visibility, but speculative refs do not get to *manufacture* permanent visibility.
...
6. **The episode-near class list (added by the 2026-07-11 amendment) — open, decided in #3131:** which artifact classes are dampenable at all (§2 gate). Direction: sensor/Sources-zone captures (#3413), transcripts, chat/working notes, ephemeral plans — dampenable; accepted decisions, evergreen knowledge, project status — exempt. The concrete list lands in the ADR-0055/T2 note-class table (#3131), referenced from here; it is a classification decision, not a model change.
