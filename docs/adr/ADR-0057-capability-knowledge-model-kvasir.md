...
**Resolved 2026-07-30 (MAS-06, #4292).** The CKM contains exactly one LLM-assisted step: `builderops ckm associate` (`app/builderops/ckm/semantic.py`, CLI `ckm_associate`), operator-invoked, never automatic. MAS-06 replaced `FabricSemanticAssociator` and Product policy routing with `BuilderSemanticAssociator`, which submits provider-free intent through the Builder-owned resolver and declares `fallback_forbidden`. Product policy, Model Inquiry's subscription session, mock identities, unavailable credentials, and degraded Builder results are fail-closed states that write zero proposals, zero semantic edges, and no semantic watermark. Under ADR-0064's 2026-07-30 owner-cost amendment, the metered provider credentials remain intentionally unprovisioned, so the current production outcome is a visible skip and **no active provider-backed CKM inference is claimed**. If a conforming Builder result becomes available later, inferred edges remain `candidate` with provider/model provenance and the confirmation policy above unchanged. This closes the former Product-routing data-integrity defect and ADR-0064 migration step 5 without changing the authenticated approval boundary described above.
