...
- **Gap detection consumes confirmed material only.** `app/builderops/ckm/gaps.py` filters to `lifecycle == "confirmed"`, so work selected from CKM-surfaced gaps never originates in unconfirmed inference. §6 therefore survives in force, not merely in wording.
