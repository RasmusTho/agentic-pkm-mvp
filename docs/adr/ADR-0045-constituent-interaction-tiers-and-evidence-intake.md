...
In the acknowledged SoS (ADR-0043: Yggdrasil the whole; Munin/Hugin/Heimdall constituents),
...
§2 already fixes the rule — Heimdall owns the fact stream; Munin projects events into durable
knowledge under its own governed-promotion rules; Hugin reads events as candidate evidence, never as
authority. This ADR names the interaction taxonomy that classifies that flow, per boundary audit §13
...
A constituent that **emits data toward** another (Heimdall's published-event stream → Munin/Hugin) is
...
Each constituent holds domain authority in its own domain (Heimdall owns the fact stream; Munin owns
durable knowledge/memory; Hugin owns reasoning/orchestration); the human is apex. Published events are
