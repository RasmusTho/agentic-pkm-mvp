...
1. The active identity is shared/<logical-secret> for openai.api-key, anthropic.api-key, github.token, and discord.webhook, and <channel>/<logical-secret> for heimdal.raw-store-key, heimdal.archive-pass, and postgres.password. Shared items mirror the same value in prod and non-prod. These provider credentials are issued outside BWS: BWS import updates the store only and does not create or rotate the provider credential.
...
- [ ] PostgreSQL receives its already-active role credential through a file-backed Compose secret; changing POSTGRES_PASSWORD_FILE alone never claims to rotate an initialized database role.
