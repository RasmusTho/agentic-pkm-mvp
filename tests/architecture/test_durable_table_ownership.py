...
def test_no_durable_ddl_has_two_owners() -> None:
    ...
    objects_path_ddl = re.findall(
        ...
        r"|create\s+table[^;]*\bpublic\.objects\b\s*\([^;]*?^\s*path\s+text)",
        bootstrap_sql,
        flags=re.MULTILINE,
    )
    ...
    # 2. Exactly one Alembic revision owns the file_state table body.
    file_state_creators = sorted(
        name
        for name, text in sources.items()
        if re.search(r"(?is)create\s+table\s+if\s+not\s+exists\s+public\.file_state", text)
    )
    assert len(file_state_creators) == 1, (
        f"expected exactly one Alembic revision to create file_state, got {file_state_creators}"
    )
    assert file_state_creators[0].startswith(FILE_STATE_OWNING_REVISION), file_state_creators
...
def test_file_state_is_reachable_from_the_alembic_revision_chain() -> None:
    ...
    could observe the table. Assert positively that a revision now references it.
    """
    referencing = sorted(
        name for name, text in _revision_sources().items() if "file_state" in text
    )
    assert referencing, (
        "no Alembic revision references file_state; MVR-05A's AC-1 becomes "
