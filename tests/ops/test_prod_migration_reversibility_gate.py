...
def test_production_overlay_owns_gate_and_nonproduction_overlays_clear_stale_controls() -> None:
    ...
    assert prod_environment["DATABASE_URL"] == f"${{DATABASE_URL:-{default_dsn}}}"
    assert prod_environment["DB_DSN"] == f"${{DB_DSN:-{default_dsn}}}"
    ...
    for service_name in ("migrate", "api", "worker", "watcher", "heimdal-capture-watch"):
        service_environment = yaml.load(prod_text, Loader=_ComposeLoader)["services"][
            service_name
        ]["environment"]
    ...
    assert yaml.load(prod_text, Loader=_ComposeLoader)["services"]["api"]["environment"][
