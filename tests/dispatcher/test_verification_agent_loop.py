...
def test_progressing_repair_rounds_have_no_numeric_stop_and_require_rereview(tmp_path) -> None:
    ...
    for index in range(1, 7):
        ...
        assert loop.repair(
            ...
            outcome="fixed",
            strongest=strongest,
        ) == (index - 3 if strongest else index)
...
def test_event_batch_allows_repairs_beyond_legacy_two_plus_two_budget(tmp_path) -> None:
    state = ledger(tmp_path)
    run = state.ingest(request())
    claimed = state.claim(run.run_id, "host")
...
def test_tcd_can_select_strongest_capability_before_two_standard_attempts(tmp_path) -> None:
    state = ledger(tmp_path)
    run = state.ingest(request())
    claimed = state.claim(run.run_id, "host")
    loop = VerificationAgentLoop(
