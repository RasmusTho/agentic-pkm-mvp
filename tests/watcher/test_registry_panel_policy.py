...
def test_registry_panel_policy_rejects_symlinked_source_before_emit_or_processing(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    vault_root = tmp_path / "vault"
    initialize_test_vault(vault_root)
    target = vault_root / "Sources" / "panel-source.md"
    _write_note(
        target,
        "uuid: source-panel",
        "%% AI:Start %%\n- [x] Do thing\n%% AI:End %%",
    )
    alias = vault_root / "Notes" / "source-alias.md"
