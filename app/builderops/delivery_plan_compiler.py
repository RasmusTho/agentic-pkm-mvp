...
COMPILER_VERSION: Final[
    Literal["builderops.delivery-plan-compiler.v2"]
] = "builderops.delivery-plan-compiler.v2"
PLANNING_SNAPSHOT_VERSION: Final[
    Literal["builderops.delivery-planning-snapshot.v2"]
...
_CANONICAL_AGENT_STATE_LABELS: Final[frozenset[str]] = frozenset(
    {
        "agent:blocked",
        "agent:needs-human",
        "agent:ready",
...
_CANONICAL_TYPE_LABELS: Final[frozenset[str]] = frozenset(
    {
        "type:bug",
        "type:refactor",
        "type:task",
...
_CANONICAL_PRIORITY_LABELS: Final[dict[str, Priority]] = {
    "prio:high": "high",
    "prio:med": "medium",
    "prio:low": "low",
}
...
class DeliveryDependencySatisfactionEvidence(_CompilerModel):
    """Minimal immutable proof for a satisfied dependency outside the snapshot."""

    schema_version: Literal[
        "builderops.delivery-dependency-satisfaction-evidence.v1"
    ]
    dependency_authority_id: str
    dependency_contract_hash: Sha256
    delivered_state: Literal["closed"]
    ...
    @classmethod
    def _validate_evidence_identity(cls, value: str) -> str:
        normalized = value.strip()
        if not normalized:
            raise ValueError(
                "dependency satisfaction evidence identities "
                "must not be empty"
            )
        return normalized
...
class DeliveryDependency(_CompilerModel):
    ...
    satisfaction_evidence: (
        DeliveryDependencySatisfactionEvidence | None
    ) = None
...
def compile_delivery_plan(
    ...
    for issue in initiation.requested_scope:
        ...
        if fact.scope_role == "validation_parent":
            ...
            continue
        if fact.authority.observed_state != "open":
            refuse(
                issue,
                "not_ready",
                "live issue authority is not open",
            )
        _validate_pickup_authority(fact, refuse=refuse)
        if (
        ...
        for dependency in fact.dependencies:
            ...
            _validate_dependency_satisfaction(
                dependency,
                dependent=fact,
    ...
    plan = DeliveryPlan(
        ...
        provenance=Provenance(
            ...
            created_by=ActorIdentity(
                actor_type="service",
                actor_id="builderops:delivery-plan-compiler.v2",
...
def _validate_pickup_authority(
    fact: DeliveryIssuePlanningSnapshot,
    *,
    refuse: _Refuse,
) -> None:
    labels = fact.authority.observed_labels
    agent_state_labels = tuple(
        label for label in labels if label.startswith("agent:")
    )
    if agent_state_labels != ("agent:ready",):
        refuse(
            ...
            "invalid_agent_state_labels",
            (
                "executable Issue authority must contain exactly one "
                "agent-state label and it must be agent:ready"
            ),
        )

    type_labels = tuple(
        label for label in labels if label.startswith("type:")
    )
    if (
        len(type_labels) != 1
        or type_labels[0] not in _CANONICAL_TYPE_LABELS
    ):
        refuse(
            fact.issue,
            "invalid_type_labels",
    ...
    priority_labels = tuple(
        label for label in labels if label.startswith("prio:")
    )
    if (
        len(priority_labels) != 1
        or priority_labels[0] not in _CANONICAL_PRIORITY_LABELS
    ):
        refuse(
            fact.issue,
            "invalid_priority_labels",
        ...
        return
    if _CANONICAL_PRIORITY_LABELS[priority_labels[0]] != fact.priority:
        refuse(
            fact.issue,
            "priority_label_mismatch",
            (
                "snapshot priority does not equal the live canonical "
                "priority label"
            ),
        )


def _validate_dependency_satisfaction(
    dependency: DeliveryDependency,
    *,
    dependent: DeliveryIssuePlanningSnapshot,
    facts_by_scope: dict[
        tuple[str, int],
        list[DeliveryIssuePlanningSnapshot],
    ],
    ...
    if not dependency.satisfied:
        if dependency.satisfaction_evidence is not None:
            refuse(
                dependent.issue,
                "contradictory_dependency_satisfaction",
                (
                    "unsatisfied dependency cannot carry delivered "
    ...
    if same_snapshot_facts:
        if (
            len(same_snapshot_facts) != 1
            or dependency.satisfaction_evidence is not None
        ):
            refuse(
                dependent.issue,
                "contradictory_dependency_satisfaction",
                (
                    "same-snapshot dependency satisfaction must use one "
                    "unambiguous live authority fact"
                ),
                related_issues=(dependency.issue,),
            )
            return
        dependency_fact = same_snapshot_facts[0]
        if (
            dependency_fact.issue != dependency.issue
            or not _authority_binds_issue(
            ...
            or any(
                label in _CANONICAL_AGENT_STATE_LABELS
                or label.startswith("agent:")
                for label in dependency_fact.authority.observed_labels
            )
        ):
            refuse(
                dependent.issue,
                "contradictory_dependency_satisfaction",
                (
                    "same-snapshot dependency satisfaction contradicts "
                    "its live delivered and closed authority"
                ),
                related_issues=(dependency.issue,),
            )
        return

    evidence = dependency.satisfaction_evidence
    if evidence is None:
        refuse(
            dependent.issue,
            "unproven_dependency_satisfaction",
            (
                "external satisfied dependency lacks immutable "
                "hash-bound delivered-authority evidence"
            ),
            related_issues=(dependency.issue,),
        )
        return
    if (
        evidence.dependency_authority_id
        != dependency.issue.authority_id
        or evidence.dependency_contract_hash
        != dependency.issue.contract_hash
        or evidence.delivered_state != "closed"
        ...
        refuse(
            ...
            "contradictory_dependency_satisfaction",
            (
                "external dependency satisfaction evidence does not "
                "bind the declared dependency and closed authority"
            ),
            related_issues=(dependency.issue,),
        )
