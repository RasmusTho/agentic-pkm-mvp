...
def _rollback_refs(receipt: Mapping[str, Any], prerequisites: Mapping[str, Any]) -> list[str]:
    ...
    for kind, prior in (("deploy", deploy), ("health", health)):
        _shape_and_fingerprint(prior, kind)
...
def validate_receipt(
    ...
    try:
        ...
        for row in rows:
            proof = proofs[row["component_id"]]
            expected_proof = {
                key: value
                for key, value in row.items()
                if key not in {"evidence_digest", "source_identity_digest"}
            }
            if proof != expected_proof or row["evidence_digest"] != canonical_digest(proof):
            ...
            expected = expected_rows[row["component_id"]]
            if any(row[key] != expected[key] for key in ("owner", "placement_class")):
                raise ReceiptValidationError(
        ...
        if control_row["source_identity"] != {
            ...
            raise ReceiptValidationError(
                "control-plane topology must bind the activation candidate"
            )
        if kind in {"deploy", "health"}:
            activation = prerequisites["activation"]
            if any(
        ...
        if kind == "deploy":
            ...
            if receipt["rollback_baseline_state"] == "no_baseline":
                if previous is not None or receipt["refusals"] != ["no_compatible_baseline"]:
                    raise ReceiptValidationError(
                        "first deployment cannot claim a rollback baseline"
                    )
            else:
                if previous is None or receipt["refusals"]:
                    raise ReceiptValidationError(
                        "available rollback requires a complete compatible identity"
                    )
                _candidate(previous)
    except ReceiptValidationError:
        raise
