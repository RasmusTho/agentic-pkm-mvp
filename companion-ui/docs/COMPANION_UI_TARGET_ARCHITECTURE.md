...
| CodeMirror 6 source editor (body-edit panel) | Shipped (dev + production profiles) — `serve_dev_page.py` body-edit panel replaces the plain textarea with a CodeMirror 6 editor, pre-populated with raw Markdown. Write path unchanged (`POST /api/companion/workspace/body`). No autosave. **Self-hosted, no runtime CDN**: the editor loads from a vendored, self-contained ESM bundle served same-origin at `/static/vendor/codemirror-6.0.1.mjs` (`companion_ui/workspace/vendor/`, see its `README.md`) via a *dynamic* `import()`, so a failed load can never gate `DOMContentLoaded`; the panel degrades to a plain textarea offline (originally #1329 deferred bundling to `esm.sh`; self-hosted after the CDN-availability trap surfaced in #2884). Mermaid (`esm.sh/mermaid@10.9.1`) is still CDN-loaded but is a non-fatal dynamic import that already degrades to source. Decision (#2897): mermaid remains CDN-loaded — an accepted, documented external dependency; the product assumes internet access (recorded in `docs/DEPENDENCIES.md`). Not self-hosted (~3.5 MB + diagram-type registration risk); revisit only if fully-offline operation becomes a requirement. |
...
- `companion-ui/docs/WORKSPACE_ORIENTATION_CONTRACT.md` — shipped read-only orientation
...
| `companion-ui/docs/WORKSPACE_ORIENTATION_CONTRACT.md` | Shipped read-only contract for note-independent workspace orientation |
