...
| `WORKSPACE_ORIENTATION_CONTRACT.md` | Note-independent orientation projection | Aligned. Defines `GET /api/companion/orientation` as a shipped read-only projection; its leave-point cursor is operational trace only, and its MemoryCandidate intent seam is a bounded, trace-backed handoff hint only, never memory, durable workspace state, session truth, receipt, UI-owned state, or vault artifact. |
