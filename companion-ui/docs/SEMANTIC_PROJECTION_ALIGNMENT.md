...
| `WORKSPACE_ORIENTATION_CONTRACT.md` | Note-independent orientation projection | Aligned. Defines `GET /api/companion/orientation` as read-only target-state projection; its leave-point cursor is operational trace only, never memory, durable workspace state, session truth, receipt, UI-owned state, or vault artifact. |
