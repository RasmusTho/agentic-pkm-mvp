...
What is **new** (defined by this spec, implemented via `docs/SYSTEM_ENTRY_POINT/`): the entry-point state machine as a declared wrapper over the existing orientation/workspace branch, the re-entry mist latency-ladder treatment, the ⌘K Panel command palette presentation, the Settings drawer UI, the Capture UI, the memory candidate review drawer, the user-facing system map overlay, the opt-in guidance layer, and the keyboard map.
...
|---|---|---|---|---|---|
| Orientation re-entry | Projection (read-only; emits intents, never applies) | the entry substrate (`orienting`) | becomes the shell on resume | shipped (re-entry surface); **new**: mist shapes + state-machine wrapper | `WORKSPACE_ORIENTATION_CONTRACT.md` |
