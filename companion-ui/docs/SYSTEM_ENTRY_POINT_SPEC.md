...
- **Route swap (NAV-3b, #2640).** A System Map route is one history entry **swapped**, not a back-then-push: `overlayHost.replace` closes the current topmost occupant and opens the destination at the same history depth via `history.replaceState`, so a single Back closes the routed overlay and the stack never desyncs.
...
(History alignment for an **already-stacked** route destination is a tracked follow-up, NAV-3d #2644, and is not yet shipped.)
