...
| `cold_start` | Cold trajectory (> 7 days — beyond the leave-point cursor TTL, ADR-0008): no admissible `leave_point`. Renders the **intent-declaration threshold** (vault chip + honest headline + verb-line Find/Map + inline governed capture — the capture verb was collapsed into the single inline capture line, #2562). Does **NOT** render the orientation grid or any re-entry overlay; those are gated to `state in ('orienting', 'shell_active')`. Does **NOT** render the ambient-refresh / System-map bar (the vault is already resolved; that bar is the `orienting`/`shell_active` ambient-refresh and the `no_vault` `entry.retry` affordance, and its standalone System-map button duplicates the verb-line `map.open`), the always-on vault-browser panel (the vault is reached only via the verb-line `vault.open`; the orientation vault browser is the declared landing but renders collapsed/`hidden` until the verb pulls it open — pulled, not always shown), or the raw provenance line (relocated behind the System map — see below) (#2525). | `leave_point.status: absent` (and no warm/dormant signals; vault is bound) |
...
### Overlay browser-history + deep-link contract (NORMATIVE)

Overlays participate in browser history and are deep-linkable by URL. This is the shipped NAV-3 cluster (`docs/ui-audit.md :: NAV-3`); the pure model lives in `companion-ui/companion-app/companion_ui/workspace/overlay_host.py` and the page wiring in `serve_dev_page.py`.

- **Browser history (NAV-3a, #2639).** Mounting an overlay pushes exactly one history entry carrying an overlay-host marker; browser **Back** fires `popstate` and dismisses the **topmost** overlay (one entry popped per dismiss) instead of navigating the page away. `Esc` and scrim still dismiss to the document anchor (the dismiss grammar above is preserved alongside the history mechanism).
- **Route swap (NAV-3b/#2640, NAV-3d/#2644).** A System Map route to a destination that is not already open is one history entry **swapped**, not a back-then-push: `overlayHost.replace` closes the current topmost occupant and opens the destination at the same history depth via `history.replaceState`, so a single Back closes the routed overlay and the stack never desyncs. A route to a destination already open below the topmost overlay reuses that destination's existing history entry: occupants above it close, the host guarded-traverses back to the existing marker, immediate-below routes close in one Back press, and deeper stacks leave the correct lower overlay open on Back.
- **Deep link (NAV-3c, #2641).** `?overlay=<id>` on page load auto-mounts a **declared AND shipped** overlay — on both the note shell and the orientation shell. Resolution is server-side and pure (`resolve_deep_link_overlay`): only a declared id with a registered occupant is honoured; an unknown id, a declared-but-unshipped id, or an empty value is a **calm no-op** (no boot script, never a dead surface). Untrusted query input is resolved internally and never embedded raw. The auto-mount boot script is emitted **after every occupant-registration script** on each shell, so no surface — Settings included — is mounted before its occupant registers. An occupant that is not registered on a given shell stays an inert host no-op (the deep link degrades calmly). The `vault` deep-link honours the browse-surface duality established in NAV-3b: it routes through `vaultBrowser.focus()` (the canonical inline left pane on desktop; the modal fallback only on narrow viewports), not a raw modal mount, so it never opens a duplicate browse surface.
...
| `data-surface-role="fast-path\|ambient"` | ⌘K palette / agent rail | Explicit rail-vs-palette role model (CUIDR-09 E2, #2453): palette = keyboard-first fast path; rail = ambient/peripheral path. Same server-declared proposals; neither invents authority. The rail `<aside>` also carries an `aria-label`/`title` so the role is conveyed in the ambient (header-hidden) state. | (new; #2453) |
...
| `data-region="cold-start-verbs"` | `cold_start` verb-line | Non-overlay structural region for the **navigation** entry-action row, rendered as ranked design-system affordances — one `btn--primary` (Find a note, `vault.open`) plus one `btn--secondary` (See the map, `map.open`), on-palette, not inline browser-blue links (#2448 D2). The capture verb ("Jot something down") was dropped from this row (#2562 Ask 2): capture is a single affordance, the inline capture line below (`data-region="cold-start-capture"`, the one `capture.open` entry point), so the door does not carry two visible capture controls. NOT registered with `overlay_host`; carries no continuity claim. | (ranked affordances; #2171, #2448, #2562) |
...
| ⌘K Panel command palette | same authority as Panel rail — a **presentation**, not new authority. Explicit roles (CUIDR-09 E2, #2453): palette = keyboard-first **fast path** (`data-surface-role="fast-path"`), rail = **ambient/peripheral** path (`data-surface-role="ambient"`); one model, identical server-declared proposal set. | command overlay (`cmd.open`) | dismisses to anchor | shipped (#1786, SEP-04; roles #2453) | `PANEL_COMPANION_UI_CONTRACT.md` + this spec |
...
| System map overlay | Projection (index; renderer/router only) | bottom-bar Map launcher + cold-start/no-vault affordance (`map.open`) | dismisses to anchor; nodes route to surfaces | **shipped** (#1787, SEP-05); `system_map_overlay.py`, `tests/companion_ui/test_system_map_overlay.py` | this spec §Resolved Q4 |
| Settings drawer | Mixed by section: Local UI preferences + read-only Connection + server-write Vault settings | right drawer overlay (`settings.open`: bottom bar + System Map) | dismisses to anchor | shipped (#1675 display prefs; drawer consolidation + listening prefs #1789, SEP-07; Vault settings section #2590; bottom-bar launcher #2609) | `DISPLAY_PREFERENCE_LOCAL_STATE_CONTRACT.md` + `docs/SYSTEM_ENTRY_POINT/SETTINGS_DRAWER.md` (§#2590 Vault exception) + §Resolved Q19 |
...
The system map is an **always-available overlay**, reachable from the bottom-bar Map launcher on live shells and offered as a calm affordance in `cold_start` and `no_vault`. It is **pull-based and never shown unbidden** — no first-run auto-display, no periodic surfacing. It is a renderer/router index (Projection): each node shows the surface's mode (`docs/COMPANION_UI_PRODUCT_SPEC.md` Find/Reorient/Resurface/Act), how it is reached, and how it returns; clicking routes to the surface. It re-classifies nothing.

The map's **entry-point center node** also carries the relocated operator diagnostics as **read-only projections** (`data-authority="read-only-projection"`, pull-only, no zero-state): the orientation freshness / as-of / trace-id telemetry (#2174/#2245), and — since #2525 — the `cold_start` **provenance** the door no longer surfaces (the cold trajectory and the server-declared `leave_point` status). These are projections of the server-declared orientation payload; the map re-classifies nothing and the values are omitted entirely when the runtime does not supply them.
...
Picker implementation (guided create-new + open-existing UI) depends on #1867 and #2312 and is **out of scope for this spec**. This section encodes the decided human intent; the runtime contract for the `vault_selection_required` response lives in `docs/VAULT_OPTIONAL_RUNTIME/COMPANION_NO_VAULT_ROUTING.md`.
