...
| `2026-06-09-system-entry-point/` | 2026-06-09 | **B+ (normalized spec: `companion-ui/docs/SYSTEM_ENTRY_POINT_SPEC.md`)** | — | System Entry Point: the companion's front door — entry-point state machine (boot / no_vault / cold_start / orienting / shell_active), unified shell (document anchor + overlay layer + Chat rail + Panel command surface), and a system map composing all existing surfaces. Includes settings/read-back/capture/receipts surfaces. The context lane (time+place) and place band are excluded from the Crossing-B promotion scope and remain at Crossing A pending Q15–Q16. |
| `2026-07-07-uat-design-audit/` | 2026-07-07 | A (review input) | *(remediation issues created from this audit — see package README)* | **Live-UAT design audit + redesign mockups.** Journey-based audit (J1–J7) of the real dev-channel runtime from 24 captures: journey verdicts, calm/trust audits, confirmed bugs B1–B8, copy replacement table, top-10 leverage ranking. `redesigns.html` carries hi-fi before/after mockups for the four worst offenders (posture single-source, Receipts v2, panel-rail resting state, note-shell status slot). Follows up `2026-06-22-companion-ui-deep-review/` against post-delivery reality. Screenshots remain in the Claude Design project (bulky evidence stays out of git). |
| `2026-06-22-companion-ui-deep-review/` | 2026-06-22 | A (review input) | #2443 | **Deep design-review request** (not a design artifact — a handoff *into* a review). 28 screenshots of every entry state, shell state, and overlay/drawer captured from the real `render_index_html` renderer via project fixtures. Framed around end-to-end journeys (J1–J7) and a two-axis rubric (workflow intuitiveness × implementation quality), not per-screen aesthetics. Includes `OBSERVED_ISSUES.md` (topbar overflow clips overlay launchers; rail/topbar overlap; non-responsive top band; narrow bottom-edge collisions) and a paste-ready Claude Design prompt. Supersedes the framing of `2026-05-26-companion-ui-design-review/`. Screenshot tooling lands via the #2443 code-lane PR (see the package README). |
