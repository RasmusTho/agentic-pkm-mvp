...
| `2026-06-22-companion-ui-deep-review/` | 2026-06-22 | A (review input) | — | **Deep design-review request** (not a design artifact — a handoff *into* a review). 26 screenshots of every entry state, shell state, and overlay/drawer captured from the real `render_index_html` renderer via project fixtures. Framed around end-to-end journeys (J1–J7) and a two-axis rubric (workflow intuitiveness × implementation quality), not per-screen aesthetics. Includes `OBSERVED_ISSUES.md` (topbar overflow clips overlay launchers; rail/topbar overlap; non-responsive top band; narrow bottom-edge collisions) and a paste-ready Claude Design prompt. Supersedes the framing of `2026-05-26-companion-ui-design-review/`. Reproducible via `_tools/{generate,screenshot}.py`. |
