...
6. **(Optional, operator-adopted) Recents-anchor** — if the runtime declares a most-recently-edited target on the cold payload, a labeled sub-affordance "Open your most recent note" renders below the action row (`data-testid="cold-start-recents-anchor"`, `data-intent="recents.open"`) routing via `/workspace?note_path=…`. Omitted when the field is absent. This is a Find/recency link, distinct from the returning-user E1 "resume the thread?" line (`data-testid="cold-start-resume-line"`, `data-intent="entry.resume"`, #2453), which is gated on a separate `leave_point` continuity signal (`status == "present"`); the two coexist when both fields are declared.
