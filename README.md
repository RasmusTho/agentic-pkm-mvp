...
- **Settings Spine** has a single owner document and a target two-scope resolution spine. The current runtime mechanisms and any remaining convergence work are recorded in `docs/SETTINGS.md`; the canonical `<vault>/settings/` root, watcher-fed reload, visible degradation, and durable write receipts remain governed there.
